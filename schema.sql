...
-- Tracked referrals — mirrors migrations/2026-08-25-referrals.sql, so an
-- environment built from this file (scripts/staging-setup.mjs) is not missing
-- the two columns the Referrals screen writes.
--
-- The code itself is not here because it is not stored: it is
-- HMAC(accountId.clientId), recomputed on verify (src/lib/referral.ts). Neither
-- is the attribution, which rides in leads.triage.referredBy — createLead's
-- insert already names triage, so capture needed no migration at all. What is
-- left is the money-shaped half, and it is a column for exactly that reason.

-- The owner's referral promise, in their own words ("$50 off your next
-- service"). Text and not a number: there is no client-scoped ledger in this
-- schema to spend a number out of. Doubles as the per-account on switch — empty
-- means no referral link goes out.
alter table accounts add column if not exists referral_reward text;

-- "I have thanked this person." A column rather than a triage key because
-- getLeadTriage rebuilds that blob field by known field, and a key it does not
