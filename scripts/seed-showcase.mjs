...
function build(startJobNumber, startInvoiceNumber) {
  ...
  ONE_OFF_PLAN.forEach((status, i) => {
    // A quarter of one-off work is a repeat customer — that is what makes the
    // client record worth having, and the "3rd job" badge worth showing.
    const repeat = clients.length > 14 && chance(0.25);
    const client = repeat ? clients[intBetween(0, clients.length - 1)] : newClient();
