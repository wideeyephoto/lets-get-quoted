...
// A descending cabin-style pair: rounded, nearly pure tones with a relaxed decay.
...
const notes = [
  { start: 0.025, frequency: 659.255, length: 0.38, gain: 0.78 },
  { start: 0.355, frequency: 523.251, length: 0.61, gain: 1 },
];
for (let i = 0; i < samples.length; i++) {
  ...
  for (const note of notes) {
    ...
    samples[i] += envelope * (Math.sin(phase) + 0.018 * Math.sin(phase * 2));
  }
}
...
const output = new URL('dispatch-connected-v2.wav', directory);
