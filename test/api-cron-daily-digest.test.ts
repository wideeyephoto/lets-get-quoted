...
describe('Daily Digest Cron Route', () => {
  ...
  beforeEach(async () => {
    ...
    runDailyDigestsMock = (await import('@/lib/daily-digest')).runDailyDigests;
    runDailyDigestsMock.mockResolvedValue({ emailsSent: 30 });
  });
  ...
  it('runs cron if authorized and returns summary', async () => {
    ...
    expect(runDailyDigestsMock).toHaveBeenCalled();
    expect(data.emailsSent).toBe(30);
  });
});
