...
describe('AI Voice Emergency SMS Alerts (Tier 1)', () => {
  it('formats owner emergency alert text with clear caller context and direct link', () => {
    ...
    expect(text).toContain('🚨 EMERGENCY CALL for workspace BrokePipes Plumbing from +12485550199: Natural gas leak reported in basement.');
  ...
  it('detects emergency hazards and passes detection to notifyEmergencyCall', async () => {
    ...
    expect(sent.p_body).toContain('🚨 EMERGENCY CALL for workspace BrokePipes Plumbing');
  ...
  it('formats owner ordinary call notification text with caller name and inquiry summary', () => {
    ...
    expect(text).toContain('📞 New call answered for workspace BrokePipes Plumbing from Jane Smith (+12485550199): Caller requested quote for tankless water heater installation.');
  ...
  it('dispatches ordinary call notification SMS when enabled', async () => {
    ...
    expect(sent.p_billing_category).toBe('owner_alert');
    expect(sent.p_sender_purpose).toBe('lgq_shared');
    expect(sent.p_body).toContain('📞 New call answered for workspace BrokePipes Plumbing');
