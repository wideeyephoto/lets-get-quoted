...
describe('Appointment Reminders Cron Route', () => {
  let runAppointmentRemindersMock: any;
  ...
  beforeEach(async () => {
    ...
    runAppointmentRemindersMock = (await import('@/lib/reminders')).runAppointmentReminders;
    runAppointmentRemindersMock.mockResolvedValue({ reminded: 15 });
  });
  ...
  it('runs cron if authorized and returns summary', async () => {
    ...
    expect(runAppointmentRemindersMock).toHaveBeenCalled();
    expect(data.reminded).toBe(15);
  });
});
