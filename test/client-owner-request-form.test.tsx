...
vi.mock('@/app/client/jobs/[token]/actions',()=>({requestJobFollowupAction:mocks.submit}));
import FollowupRequest from '@/app/client/jobs/[token]/FollowupRequest';
...
it('keeps the form content and request ID after an uncertain response, and uses a fresh ID after reopening',async()=>{
  ...
  await act(async()=>root.render(<FollowupRequest token="token" businessName="Builder" hasWarranties={false} isComplete={true}/>));
  await click(/follow-up/i);
  container.querySelector('textarea')!.value='Please check the repaired tap';
  await submit();
  expect(container.textContent).toContain('Could not confirm your request. Retry this form.');
