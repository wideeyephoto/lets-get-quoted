...
function fixture(options: { rejectPrepare?: boolean; rejectSnapshot?: boolean; rejectFinish?: boolean; messaging?: boolean; jobless?: boolean; quickStop?: boolean; paymentRefund?: boolean; connect?: boolean; dispute?: boolean } = {}) {
  const notice = { id: 'notice-1', account_id: 'account-1', source_id: 'feed-1', source_type: options.dispute ? 'payment_dispute' : options.connect ? 'account_connect' : options.paymentRefund ? 'payment_refund' : options.quickStop ? 'quick_stop' : options.messaging ? 'messaging_registration_event' : 'job_feed', event_kind: 'client_question', source_payload: { title: 'Question', body: 'Help', job_id: options.jobless ? undefined : 'job-1', recipient_email: 'application@example.test', business_name: 'Application Business' }, attempted_at: '2026-09-14T12:00:00Z' };
  let state = 'pending';
