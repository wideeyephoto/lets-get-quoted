...
describe('Multi-Channel Ads Engine', () => {
  it('provides 3 high-converting Weekly Drip Smart Growth Bundles', () => {
    ...
    expect(launch.weeklyAmountDollars).toBe(168);
    expect(launch.weeklyAdSpendDollars).toBe(160);
    expect(launch.weeklyFeeDollars).toBe(8);
    expect(launch.monthlyAverageDollars).toBe(728);
    expect(launch.channels).toContain('google_search');

    const growth = getSmartBundle('growth');
    expect(growth.name).toBe('Growth Engine');
    expect(growth.weeklyAmountDollars).toBe(315);
    expect(growth.weeklyAdSpendDollars).toBe(300);
    expect(growth.weeklyFeeDollars).toBe(15);
    expect(growth.monthlyAverageDollars).toBe(1365);
    ...
    expect(scale.weeklyAmountDollars).toBe(588);
    expect(scale.weeklyAdSpendDollars).toBe(560);
    expect(scale.weeklyFeeDollars).toBe(28);
    expect(scale.monthlyAverageDollars).toBe(2548);
  ...
  it('calculates multi-channel budgets with 5% platform management fee', () => {
    const searchOnly = calculateMultiChannelBudget({
      searchSpendDollars: 600,
    });
    expect(searchOnly.totalAdSpendDollars).toBe(600);
    expect(searchOnly.platformFeeDollars).toBe(30);
    expect(searchOnly.totalMonthlyDollars).toBe(630);
    ...
    expect(multi.platformFeeDollars).toBe(45);
    expect(multi.totalMonthlyDollars).toBe(945);
