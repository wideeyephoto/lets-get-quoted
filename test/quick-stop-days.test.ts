...
import { quickStopDayOptions, isAllowedQuickStopDay, zonedNowParts } from '@/lib/quick-stop';
...
const days = (over = {}, now = SAT_2PM) => quickStopDayOptions(settings(over), { now, timeZone: ZONE });
...
describe('quickStopDayOptions', () => {
  ...
  it('drops TODAY once the last arrival time has passed', () => {
    ...
    expect(quickStopDayOptions(settings(), { now: late, timeZone: ZONE }).map((d) => d.label)).toEqual(['Tomorrow']);
  });

  it('keeps today right up to the cutoff', () => {
    const justBefore = new Date('2026-08-01T23:59:00.000Z'); // 19:59 New York
    expect(quickStopDayOptions(settings(), { now: justBefore, timeZone: ZONE })[0].label).toBe('Today');
  });
...
describe('isAllowedQuickStopDay — the server-side re-check', () => {
  it('accepts a day that was on offer', () => {
    expect(isAllowedQuickStopDay('2026-08-02', settings(), { now: SAT_2PM, timeZone: ZONE })).toBe(true);
  });

  it('refuses a day past the horizon', () => {
    expect(isAllowedQuickStopDay('2026-08-05', settings(), { now: SAT_2PM, timeZone: ZONE })).toBe(false);
  });

  it('refuses a day the contractor does not work, however it was submitted', () => {
    expect(
      isAllowedQuickStopDay('2026-08-02', settings({ weekdays: [1, 2, 3, 4, 5] }), { now: SAT_2PM, timeZone: ZONE }),
    ).toBe(false);
  });

  it('refuses yesterday', () => {
    expect(isAllowedQuickStopDay('2026-07-31', settings(), { now: SAT_2PM, timeZone: ZONE })).toBe(false);
  });
});
