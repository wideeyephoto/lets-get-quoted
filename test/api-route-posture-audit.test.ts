...
describe('All-Route Security & Auth Posture Audit', () => {
  ...
  it('finds all routes across src/app and asserts each has an established security/auth posture', () => {
    ...
    for (const filePath of routeFiles) {
      ...
      const isPublicUtility = rel.startsWith('api/tools/') || rel.startsWith('api/rebates/') || rel.startsWith('api/permits/') || rel.startsWith('api/health') || rel.startsWith('api/email/track') || rel.startsWith('api/og') || rel.startsWith('demo/') || rel.startsWith('api/demo/') || rel.includes('robots.txt') || rel.includes('sitemap.xml') || rel === 'client/portal/route.ts' || rel.startsWith('blog/feed.json') || rel.startsWith('blog/rss.xml');
