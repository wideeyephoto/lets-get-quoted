...
describe('quickStopNoShowLock — escalation ladder', () => {
  const now = new Date('2026-07-01T12:00:00Z');
  const daysAgo = (n: number) => new Date(now.getTime() - n * 86_400_000);

  it('first no-show (no priors) → 10-day lock, tier 1', () => {
    const lock = quickStopNoShowLock([], now);
  ...
  it('second within 90 days → 30-day lock, tier 2', () => {
    const lock = quickStopNoShowLock([daysAgo(30)], now);
  ...
  it('third within 180 days → disabled pending review, tier 3', () => {
    const lock = quickStopNoShowLock([daysAgo(20), daysAgo(120)], now);
  ...
  it('old priors outside the windows fall back to tier 1', () => {
    const lock = quickStopNoShowLock([daysAgo(200), daysAgo(400)], now);
    expect(lock.tier).toBe(1);
  });

  it('a prior between 90 and 180 days counts toward tier 3 only with a second, else tier 1', () => {
    expect(quickStopNoShowLock([daysAgo(120)], now).tier).toBe(1); // one old-ish prior, not within 90 → still first-level
    expect(quickStopNoShowLock([daysAgo(120), daysAgo(150)], now).tier).toBe(3); // two within 180
  });
});

describe('quickStopSettingsFromAccount — safe defaults on a bare row', () => {
  it('degrades an empty/pre-migration row to coherent defaults, feature off', () => {
    const s = quickStopSettingsFromAccount(null);
  ...
  it('parses category CSV to a deduped lowercase list', () => {
    const s = quickStopSettingsFromAccount({ extra_stop_categories: 'Leak Repair, faucet, LEAK REPAIR ,' });
...
describe('money helpers', () => {
  ...
  it('clampFeeCents keeps the fee within the band, even if the band is inverted', () => {
    const s = quickStopSettingsFromAccount({ extra_stop_min_fee_cents: 5000, extra_stop_max_fee_cents: 25000 });
    expect(clampFeeCents(1000, s)).toBe(5000);
    expect(clampFeeCents(99999, s)).toBe(25000);
    expect(clampFeeCents(12000, s)).toBe(12000);
    const inverted = quickStopSettingsFromAccount({ extra_stop_min_fee_cents: 25000, extra_stop_max_fee_cents: 5000 });
