...
| **CLM-012** | *"Live technician ETA sharing — Give customers an expiring map link, updated arrival window, and delay notices."* | `/features/crew`, Field App, `/track/[token]` | Product Capability | **Product Truth:** Server-side trip token hashing (`job-tracking.ts`), privacy street-level rounding (`applyPrecision`), 90-minute location expiry backstop, dynamic ETA recalculation with traffic awareness (`recalculateLiveArrivalTimes`), and automated SMS delay templates (`DEFAULT_DELAY_TEMPLATE`). Verified in `test/arrival.test.ts` and `test/live-technician-eta-sharing.test.ts`. | ✅ **VERIFIED**<br/>Product Eng |
