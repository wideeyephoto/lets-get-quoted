...
| **CLM-014** | *"paid enterprise API tiers with strict zero-data-retention and non-training guarantees (verified: Google Cloud Billing active on Gemini API project)"* | `src/app/privacy/page.tsx` | Data Privacy & Security | **Third-Party Proof:** Google Cloud Platform paid billing accounts disable data use for model training by default. OpenAI zero-data-retention (ZDR) is configured via organizational agreement. Evidence script `npm run inspect:ai-tier` verifies API key routing to paid tiers. | ✅ **VERIFIED**<br/>Security / SRE |
