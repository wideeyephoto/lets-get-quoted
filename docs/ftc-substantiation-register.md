...
| **CLM-012** | *"Live technician ETA sharing — Give customers an expiring map link, updated arrival window, and delay notices."* | `/features/live-eta`, `/features/eta`, `/features/crew`, Field App, `/track/[token]` | Product Capability | **Product Truth:** Server-side trip token hashing (`job-tracking.ts`), privacy street-level rounding (`applyPrecision`), 90-minute location expiry backstop, dynamic ETA recalculation with traffic awareness (`recalculateLiveArrivalTimes`), and automated SMS delay templates (`DEFAULT_DELAY_TEMPLATE`). Verified in `test/arrival.test.ts`, `test/drive-time-traffic.test.ts`, and `test/live-technician-eta-sharing.test.ts`. | ✅ **VERIFIED**<br/>Product Eng |
| **CLM-013** | *"Free contractor website and business tools — $0/month, no credit card required"* | `/pricing`, `/for/[trade]` hero, `/compare` pages, lifecycle emails | Pricing & Terms | **Product Truth:** Code enforces $0/mo and no card required. Verified in `src/lib/billing/entitlement-catalog.ts` (Flex plan definition) and `src/app/pricing/pricing-catalog.ts`. | ✅ **VERIFIED**<br/>Billing Eng |
| **CLM-014** | *"Google LLC (Google Gemini API) and OpenAI, LLC. Customer data, prompts, job notes, photos, and voice transcripts are not used to train public foundation models."* | `/privacy` §4, Third-Party Subprocessors list | Data Use / AI Inference | **Provider terms, not yet console-confirmed for this account.** Both providers' standard API terms (distinct from their free consumer chat products) commit to not training on API inputs by default — OpenAI since March 2023, Google for the Gemini **API** as opposed to the AI Studio free tier. The claim as worded is a fair description of each provider's default API terms. What is **not yet verified against this account specifically**: whether the `GEMINI_API_KEY` in use belongs to a project with active Cloud Billing — Google's own published terms carve out the *unbilled* Gemini tier as eligible for using content to improve the product, so an unbilled key would make this sentence false for the Google half regardless of what the sentence says. This is tracked to closure as **[G1 in the gap closure plan](prelaunch-gap-closure-plan-2026-09-11.md#g1--the-ai-inference-tier-claim-is-published-as-verified-and-the-verification-is-still-an-open-task)**; `npm run inspect:ai-tier` is the read-only check that closes it once run against `.env.live.local`. This entry replaces an earlier version of this same sentence (commit `8ea306817`, corrected in `6c89a0a`) that additionally claimed "paid enterprise API tiers with strict zero-data-retention guarantees (verified: Google Cloud Billing active on Gemini API project)" — a verification that [was never actually performed](prelaunch-gap-closure-plan-2026-09-11.md#g1--the-ai-inference-tier-claim-is-published-as-verified-and-the-verification-is-still-an-open-task) and has since been removed from the page copy itself, not just from this register. | ⚠️ **UNVERIFIED FOR THIS ACCOUNT**<br/>Commercial Legal / G1 |
...
5. **Self-Certified Verification Claims Without an Evidence Artifact Prohibited:**  
   *Prohibited:* "...(verified: Google Cloud Billing active on Gemini API project)" — this exact sentence was published in `/privacy` (commit `8ea306817`) for the task that was supposed to produce the check (T27,
   `docs/admin-command-center-task-list-2026-09-09.md:325`); no console capture or evidence file was ever created, and T27 was retracted rather than closed once that was noticed. Writing that a check happened is not the check happening.  
   *Approved:* State only what the cited provider's standard terms already establish (CLM-014), and if a claim depends on this account's specific configuration rather than a provider's published terms, either get the console evidence first or don't publish the sentence.
