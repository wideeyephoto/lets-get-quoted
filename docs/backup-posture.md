...
The independently downloaded 12:42 UTC offsite pack additionally passed a real isolated PostgreSQL restore at 20:21 UTC: 260 row counts, 258 table/grant entries, 272 policies, 435 functions/grants, existing-owner RLS and all 38 materialized object hashes. Its temporary plaintext cluster/files were removed. This is additional [downloaded-pack evidence](runbooks/evidence/dr-downloaded-local-restore-2026-09-09.json), separate from the earlier hosted acceptance below; local file hashes do not prove hosted Storage recovery. See the [security recovery record](runbooks/security-recovery-2026-09-09.md) for credential rotation and remaining infrastructure/provider work.
...
## September 9 hosted and infrastructure recovery follow-up

The downloaded offsite pack now has hosted SQL/Auth/Storage, RLS and protected-application reconstruction evidence. Source/migrations are independently present in Drive and USB, and an authoritative DNS export and provider reconciliation procedure are recorded. Independent key retrieval, live configuration recovery, financial reconciliation and actual provider-account/DNS failover remain open. See the [dated infrastructure/provider record](runbooks/infrastructure-provider-recovery-2026-09-09.md); its measurements supersede earlier not-yet-rehearsed descriptions without claiming a full-disaster RTO.
