...
- [ ] Complete independent lifecycle suppression scans with pagination or bounded exact-recipient queries. Platform campaign suppression checks now use bounded exact-recipient RPCs. Any incomplete lookup must stop the affected send/batch visibly.
...
Completed locally: a disabled-by-default recovery worker for the two existing ledgers; final workspace checks for crew/theme/merchandise sends; complete shared suppression scans; and platform campaign preference storage, unsubscribe flow and final checks. Next complete lifecycle suppression scans and the policies for other platform sender families, along with the outstanding capacity review. M2 remains open. Keep scheduling/enabling in the hosted environment within M5 acceptance.
