...
Local domain failure progress: submissions require the existing incident UUID and stable provider key. The prepared snapshot migration saves exact rendered content and a fingerprint of the actual sending credential before submission, with one winning preparation per incident and immutable message/identity guards. Current recipient blocks are checked again after saving. Existing one-attempt/manual-review behavior remains in place. This does not complete M3: verified provider workspace/region scope, signed callback acceptance reconciliation, retention review and bounded recovery remain open. Restoration and other owner notices remain separate work. See the [notice runbook](runbooks/email-domain-failure-notices.md) for migration order and limits.
