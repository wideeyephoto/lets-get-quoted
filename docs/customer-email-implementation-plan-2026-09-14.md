...
- [ ] Repair platform campaign scope end to end: add appropriate platform opt-out persistence, reconcile footer/header tokens (including test previews), avoid unioning preferences across workspaces, and apply the final eligibility check to live/test paths. The current literal `platform` cannot be stored in the workspace UUID foreign key. Test an opt-out arriving after audience selection.
...
- [ ] Complete independent lifecycle/platform suppression scans with pagination or bounded exact-recipient queries. Any incomplete lookup must stop the affected send/batch visibly.
