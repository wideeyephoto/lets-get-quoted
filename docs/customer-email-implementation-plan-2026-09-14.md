...
- [ ] Turn the [sender inventory](runbooks/email-delivery-audit-and-recovery-monitoring.md) into a maintained registry of purpose, recipient scope, transport and suppression behavior. Review application call sites plus operational scripts before claiming complete coverage.
  - Raw HTTP and script scan added the previously omitted support auto-responder and independent deliverability seed script. The support sender is now gated and reports actual acceptance/resolution; seed-script policy, indirect/external transports and provider evidence remain open.
