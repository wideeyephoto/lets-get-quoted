...
- [ ] Review indirect transports/external automations and reconcile operational callback/provider evidence before claiming complete enforcement. Verify the emergency responder channel separately.
  - Matched unscoped callbacks now bind to the operational ledger's provider ID and exact saved recipient before recording platform delivery blocks. Unknown/early callbacks and emergency messages without ledger rows still require reconciliation; saved payloads and retry keys are unchanged.
