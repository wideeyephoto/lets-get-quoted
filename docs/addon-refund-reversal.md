...
This historical evidence does not claim production delivery, real-money settlement,
customer UI debt presentation or full provider-period reconciliation.

## September 9 continuation

The implementation is integrated with the current main baseline in the prelaunch
payment review branch. The expanded portable PostgreSQL regression passes 28/28
checks across all six SKUs, including refund-before-grant retry, expired leases,
base-plan changes and no repeated debit. The checkout page now explains how used
or reserved refunded minutes offset future grants, and how a full refund ends a
monthly add-on. That explanation renders only when the refund rollout flag is on.
