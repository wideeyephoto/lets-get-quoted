...
**Current release checkpoint, September 10 13:10 UTC:** PR #70's durable owner-notice recovery and PR #71's direct settings link/failure explanation are live on both aliases as **`dpl_4r5THCtG18aDrULMbbLUrLLbvdRP`**, SHA **`a19356d3b671fdd0fd49628220bef6e9cd3479c1`**. Full final CI passed **14,619 tests** and the production build. Actual provider deletion, deployed downgrade, one authenticated Gmail platform owner alert, matching callback resolution without resend, and product reconnection passed. The replacement binding below is verified. Both September 10 test-inbox allowances are now used. F03 product fallback receipt, failed-notice escalation, other lifecycle acceptance, website DNS/device approval and certificate/deletion drills remain open. The seven-day clock has not started. See the [dated release and drill evidence](contractor-domains-canary-2026-09-09.md).

## September 9 verification update

**September 10, 13:30–14:12 UTC update:** DNS/passkey and the website certificate-watcher gate passed: the scheduled watcher connected the disposable fixture, strict TLS validation passed, and its one approved extra Gmail alert arrived with a matching signed delivery callback. The existing website remained healthy. The supported account-closure drill exposed missing provider cleanup and invalid account-column references; a tested candidate and hosted staging rollback check now cover those defects, but production release/fixture deletion remain open. Today's normal test allowances and extra Gmail alert are used. The seven-day clock remains unstarted. See [current evidence](contractor-domains-canary-2026-09-09.md) and the [closure runbook](runbooks/account-closure-domains.md).
