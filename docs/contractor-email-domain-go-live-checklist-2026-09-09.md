...
Prepared September 9; updated September 12, 2026 at 16:42 UTC. Status: **Observation remains active from September 11 at 16:23:14.731206 UTC, with 1/7 qualifying scheduled checks and one completed 24-hour observation day. The 16:41 UTC database/provider checks remain healthy. September 12 Gmail and September 11 Outlook product receipts passed delivery, authentication and matching-link checks within the window. Gmail's September 12 allowance is used; Outlook remains unused. The supported empty website-fixture closure is queued with an October 12 recovery deadline after a narrowly tested database-function repair. Actual disposal/provider/DNS cleanup, remaining message paths and lifecycle gates, replies and final canary acceptance remain open.** See the [current checkpoint](contractor-domains-canary-2026-09-09.md#september-12-scheduled-check-gmail-receipt-and-fixture-closure).
...
- [ ] **I03 — Observe seven consecutive scheduled runs with the canary present.** **1/7 passed**: September 12 run `27eccf15-459d-4852-ae9a-e1dc5aa34661` checked the active domain with zero errors/backlog and refreshed `last_checked_at` to 06:23:21.190 UTC. Each remaining run must successfully check the active domain with current timestamps, no unexplained backlog, and no unresolved permission/provider error. Track actual sends, provider delivery results, bounces, complaints, reply receipt, and support incidents daily. A zero-send week does not pass; extend observation until representative normal usage exists. Seven elapsed days are also required; the original earliest review remains September 18 at 16:23:14.731206 UTC.
...
| September 11, 16:43–16:46 UTC | No post-start email run yet | Same verified binding; provider DNS verified | 1 Outlook product send / 1 delivery | Authentication/link passed; actual in-window reply pending | PR #78 quote path passed; original start retained; both inbox allowances used |
