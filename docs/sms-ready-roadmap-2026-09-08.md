...
Current status: **single-workspace dispatch handset session complete; broader operational closeout and expansion pending; customer session gated on registration.** Employee welcome/assignment/schedule messages contain no links. Subcontractor secure links and workflow branches remain untested. The calendar's explicit **Notify** action sends the crew schedule message; **Move this job** alone changed the schedule without queuing a crew text. Cross-workspace evidence is currently a suppression-helper check, and second-sender campaign evidence is local testing; neither closes an eligible second live-workspace/carrier test. Private evidence is recorded at `C:\dev\sms-live-test-evidence-20260908.md`.
