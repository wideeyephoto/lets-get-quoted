...
- [x] **E1 — Point protected Vercel hosting at the restored database.** September 9 follow-up: independently recovered source deployed to the separate `lgq-recovery-20260909` project; all deployments require Vercel login. An existing restored owner opened the dashboard and job data. The concurrent staging grant change required a documented client adapter. [Evidence](evidence/dr-reconstructed-hosting-app-2026-09-09.json).
...
- [x] **F1 — Secrets inventory.** Vercel Sensitive vars are write-only and unreadable by anyone, including the owner. A total-loss scenario needs every secret re-derivable from its source of truth. **PASS =** a list of every required env var with, for each, where its value can be re-obtained. Any var with no answer is an unrecoverable dependency and a finding.
- [x] **F2 — Stripe divergence.** Money state lives at Stripe. A restore to time T leaves the database believing it is T while Stripe is at now: subscriptions, invoices, charges and refunds in between exist only at Stripe. Plan the reconciliation — `scripts/reconcile-stripe-live-ledger.mjs` is the starting point. **PASS =** a documented procedure for closing the gap, and an explicit statement of which direction wins on conflict.
- [x] **F3 — Replay hazard on restore.** Bringing back a restored database with crons live re-processes events already processed. Idempotency keys are not automatically safe here — a stable key can replay a 24h-old response. **PASS =** a written pre-cutover step that pauses the money-touching crons (`billing-subscription-projection`, `connected-payment-projection`, `top-up-projection`, `direct-payment-settlement`, `overage-settlement`, `ad-wallet-refill`, `dunning`) before traffic resumes.
- [x] **F4 — Provider-side pointers go stale.** Custom domains and TLS certs, email sending domains at Resend, SignalWire numbers and campaign assignments, QuickBooks OAuth tokens, Google LSA and Meta ad credentials. Each is a row pointing at a resource whose real state moved on. **PASS =** a per-provider note on whether restore-then-reconcile is automatic (the reconciler crons cover email and custom domains), manual, or impossible.
- [x] **F5 — DNS and deploy recovery.** The Vercel rollback drill covers deploy; confirm DNS records and their zone are recoverable independently. **PASS =** a zone export or a documented source of truth.
- [x] **F6 — Repo and migration recovery.** The 320 migrations under `migrations/` are the schema's source of truth. Backups exist (hourly USB, twice-daily offsite) but GitHub silently refuses some refs. **PASS =** confirm the migration directory and `schema.sql` are present in at least two independent backups.
...
- [x] **G4 — Ransom / malicious-actor case.** Backups reachable with the same credentials as production are not backups. **PASS =** a statement of whether the offsite copy is separately credentialed, and whether it is immutable or merely a copy someone with the service-role key can also delete.
...
## September 9, 22:15 UTC acceptance update

The downloaded pack completed hosted restore, Auth, all 38 Storage hashes and 35 RLS tests. Independently recovered source now runs in a new all-login-protected Vercel project and renders dashboard, jobs, clients, schedule, messages, payments and account settings. The initial HTTP smoke rendering claim was withdrawn after a streamed error boundary was discovered; the later browser check used the compatibility adapter required by another task's staging grant changes.

F1–F6 now have the required inventory/procedure/backup evidence in the [infrastructure/provider recovery record](infrastructure-provider-recovery-2026-09-09.md): environment names with recovery sources and explicit missing verification, Stripe provider-wins conflict handling, pre-cutover worker pause, provider-specific recovery paths, authoritative DNS export, and 327 migrations plus schema in both the downloaded source and USB mirror. These checked procedure gates do not certify original-key retrieval, live configuration parity, completed Stripe reconciliation, lost-provider-account recovery or DNS cutover. Stripe event inventory is blocked by the available live key's missing `event_read` permission. G4 is documented: Drive uses separate Google credentials but remains mutable and accessible from this PC; it is not immutable storage. G1/G2 selective recovery and a complete G3 RTO remain open. E3 invoice/PDF and E4 Plan & usage acceptance were not completed in this protected hosting run; the plan surface is disabled by its recovery configuration.
