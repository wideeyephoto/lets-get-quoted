...
**Remaining at this record's initial commit:** application deployment and production native-device enrollment, fresh-session verification, backup verification, cancellation and recovery acceptance. A real Apple Passwords/Dashlane/device ceremony requires the owner to operate the native prompt. Do not mark that gate complete from the protocol fixture.
...
**Remaining:** hosted Auth/Storage and application recovery from this exact pack, independent Dashlane-key retrieval, infrastructure reconstruction/DNS failover, and provider-account recovery/reconciliation. Production and shared staging were not overwritten during the isolated drill. Supabase Free/PITR-disabled and the 12-hour offsite target remain the owner's chosen posture.
