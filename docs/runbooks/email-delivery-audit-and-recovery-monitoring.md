...
| `ai-operator/support-auto-responder.ts` | Platform support recipient; shared final platform check, bounded direct Resend HTTP request and callback tag; requires HTTP success plus acceptance ID before resolving; verifies persisted resolution separately | No production caller found in the source scan; durable reply identity and ambiguous-acceptance recovery required before enabling repeated execution |
| `scripts/run-deliverability-seed-test.mjs` | Preview by default; explicit controlled recipients and live flag; bounded HTTP submission through shared platform checks; acceptance IDs and partial outcomes retained | Controlled manual tool only; provider capacity/scope, durable deduplication and actual receiver evidence remain open. See [seed runner](deliverability-seed-runner.md) |
...
The subsequent raw-HTTP/script scan searched provider URLs, SDK sends and transport constructors in `src` and `scripts`. It found the support auto-responder omission above and the independent seed script. Recovery-worker HTTP submission uses the existing ledger contract; provider-domain API calls do not send email. This scan expands the inventory; it is not proof that indirect or external transports are exhausted. Provider workspace/region, hosted credentials and historical suppression evidence remain unverified.
