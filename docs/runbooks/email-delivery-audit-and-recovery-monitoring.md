...
The subsequent lifecycle pass replaces sweep/approval suppression scans with exact workspace/address batches. See the [lifecycle runbook](contractor-lifecycle-email-sends.md) for its 500-recipient bound and migration prerequisite. Reassessment of `magic-link.ts`, `founder-alerts.ts`, `ai-operator/digest.ts` and the public email-report route confirms recipient scope remains platform login, platform staff/founder or explicitly requested report recipients. Campaign opt-outs must not be applied as transactional delivery blocks. These families still need reviewed provider-scope evidence, final delivery-block checks, callback correlation and durable request/event identities; the campaign preference table alone does not complete their protection.
