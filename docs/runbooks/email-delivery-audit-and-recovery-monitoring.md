...
| `scripts/run-deliverability-seed-test.mjs` | Independent SDK sends for synthetic login/quote/invoice samples; dry-run option; no shared suppression gate | Controlled manual tool only; guard delivery policy and verify actual receiver evidence before live use. Synthetic links and printed authentication assertions are not proof of working login or receiver authentication |
