...
Existing saved payloads gain no tags or replacement keys. The subsequent [durable callback evidence migration](operational-callback-evidence.md) retains early negative evidence and reconciles automatically when acceptance is recorded, including concurrent arrival. Emergency notifications without ledger rows and unknown provider IDs remain unmatched rather than establishing scope by address. Historical/provider evidence reconciliation remains open. Do not alter saved payloads to add tags under an existing idempotency key.
