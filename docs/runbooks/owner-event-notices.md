...
### Recovery after interrupted cost evaluation

Apply 20260914201537_margin_evaluation_requests.sql after the margin owner-notice migration and before deploying the updated owner cron. Cost inserts, changes to amount/burden/type/account/job, and deletes request evaluation in the same transaction. Jobless overhead stays outside this job queue. Job transfers request both affected jobs; new links must belong to the cost account. Failure to save the evaluation request rolls back the cost mutation. Existing costs are not backfilled.

Each job has a revision and optional five-minute lease. New cost edits rotate the revision while retaining an active lease. The worker claims at most five jobs; successful completion removes only that claimed revision, while a newer revision releases for another pass. Failed evaluations retain a fixed error and release for retry. Expired leases can be reclaimed; the old lease cannot finish the replacement. Account/job cascade cleanup removes pending work. The current-margin check also excludes soft-deleted jobs.

The existing owner cron runs these evaluations with direct owner dispatch disabled, then executes the usual bounded owner sender. A margin failure does not suppress the owner queue but marks the run unhealthy; the summary includes evaluated, failed and remaining counts. LGQ_OWNER_EVENT_NOTICES_ENABLED still controls background execution and was not changed. Re-evaluation may append history again, but cannot bypass the saved four-hour owner cooldown or authorize automatic retries of uncertain email submissions.

Local evidence: 23 application tests, 179 PostgreSQL checks, full type checking, lint, sender registry and clean local advisor. Verify capacity, backlog clearing, cost-writer ordering and actual receiver evidence during hosted acceptance. This closes the new-cost-write interruption gap locally; the overall ten-step goal and final push remain open.
