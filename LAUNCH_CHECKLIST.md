...
- [ ] **Daily reconciler cron (plan §10) — not built.** Deferred to `docs/custom-email-domains-post-launch-tasks.md`. Until it exists, a contractor who removes their DKIM record after verification keeps a `verified` row while their mail fails at the provider — the one failure mode here that is silent and customer-visible.
