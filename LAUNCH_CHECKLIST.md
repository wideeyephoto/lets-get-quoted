...
- [ ] **Dispatch number provisioning and live dispatch matrix**: campaign `19e7c875-3611-4b40-8429-7dae3b5e6553` is active but has no assigned number. Replacement candidate `+1 (947) 262-4739` was quoted at **$0.50/month**; purchase awaits explicit approval. Assignment, application sender readiness, and controlled crew send/reply are still unverified.
...
- [ ] **Remaining live carrier matrix**: correctly registered dedicated customer delivery; dispatch delivery/reply; booking and missed-call text-back through the released producer; quiet-hours deferred release; controlled rejection/retry/dead-letter recovery; duplicate/out-of-order callbacks; and full usage reconciliation. Historical or mocked coverage does not close these checks.
...
- [ ] **Re-verify the two active crew phones before enabling crew-origin voice dispatch**: the database-first rollout deliberately defaulted existing crew to unverified, and the hosted verifier audited the two active crew rows awaiting step-up OTP challenge proof (`+12485630746` - Chelsea Landry, `+18103042061` - Brett Arnold). Confirmed both have `phone_verified: false`, keeping crew voice dispatch fail-closed (`status: 'unavailable'`, `apply_voice_contractor_action` raises `42501`) until step-up OTP challenge proof is completed from their physical handsets; then canary assigned-job allowance plus unassigned, revoked, coworker-attribution, and office-record denial in production.
...
- [x] **Complete the live AI Voice matrix only after hosted database verification, exact paid-number authorization, READY deployment, deliberate entitlement, and canary-only enablement (Canary Completed 2026-09-04 at 19:41 UTC / 15:41 ET)**:
...
- [ ] **Clean-Slate Onboarding E2E (Harness & Protocol Ready 2026-09-04)**: Established operational runbook in `docs/runbooks/clean-slate-onboarding-rehearsal.md` and automated database/Stripe verification auditor in `scripts/verify-clean-slate-onboarding-journey.mjs`. Auditor checks 8 distinct lifecycle gates: account existence, valid terms acceptance timestamp & version, active owner membership, Stripe Connect merchant readiness, quote/job creation, customer public portal/quote access, payment settlement, and dashboard-issued fee-protected refund. Requires human operator with unseeded email, physical cellular phone, and valid payment card to execute live KYC and card authorization.
- [ ] **Stripe↔Application Ledger Reconciliation & Money-Rail Rehearsal (Reconciler & Harness Ready 2026-09-04)**: Built production ledger reconciler in `scripts/reconcile-stripe-live-ledger.mjs` to cross-examine Stripe charges, refunds, subscriptions, and open failures against database `payments`, `invoices`, and `billing_subscriptions`. Verified connected destination refund loss protection (`reverse_transfer: true` + `refund_application_fee: true` in `src/lib/payments.ts`). Created and verified automated dispute webhook lifecycle test `test/dispute-webhook-lifecycle.test.ts` (passing) and dispute playback rehearsal runner in `scripts/simulate-dispute-playback.mjs`. Operational execution protocol codified in `docs/runbooks/stripe-money-rail-rehearsal.md`.
