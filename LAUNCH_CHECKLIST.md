...
- [ ] **Website watcher read-retry candidate:** One bounded retry of transient site SELECT failures preserves permanent-error reporting and avoids replaying promotion or owner emails. Six regression cases failed before the change; 74 focused tests and targeted ESLint now pass. Full typecheck passes; PR #85 CI, Preview validation and production acceptance remain pending. The candidate is published as draft [PR #85](https://github.com/wideeyephoto/lets-get-quoted/pull/85) and remains undeployed; this does not close the upstream timeout or lifecycle gates.
