...
- [x] **Spoken-quote release and fallback recovery:** PR #57 (`cf92c60cc`) is included in production with passing CI and health. Actual number fallback captured a 14-second synthetic voicemail; provider-verified manual recovery restored its attribution, signed-in playback and zero AI charge with no remaining hold.
- [x] **Admission and native recording recovery release:** PR #61 (`3e7466a6b`) passed full CI; both domains were verified on production `3fa18307a` at 21:44 UTC. A controlled customer call reached AI, used one measured minute and cleared its hold. Signed-path callbacks returned HTTP 200 and automatically persisted a nine-second voicemail. Homeowner office-review wording passed in a synthetic caller test. Startup still needed a retry; see [evidence and limits](docs/voice-acceptance-2026-09-09.md).
- [ ] **Voice acceptance:** The confirmed 21:52 Dispatch retest reached AI but failed; its log shows job-reference redaction, repetitive quote follow-ups and 1.143–4.950-second first-audio waits. No note write ran; three AI minutes settled with no hold. Resolve these issues and verify save/readback/Stop. The actual unanswered-transfer retest automatically saved 43 seconds of voicemail, but final call status reconciliation remains open. Complete admitted fallback termination/hold recovery, active voicemail/in-flight-tool boundaries and remaining on-call/staff behavior. The late probe ended at 598.321 seconds without active-recording proof and was cleaned up. The prompt-derived false transfer-history correction is committed separately. Provider-period billing reconciliation remains open; exhaustion blocking stays OFF.
- [ ] **Remaining operator and recording lifecycle acceptance:** Retry an attributable failed receipt through the supported operator action and prove no duplicate effects; verify a completed provider deletion. Authorized playback passed for the manually recovered fallback and both automatically persisted recordings, including the 43-second unanswered-transfer voicemail. An empty healthy queue does not close the remaining drills.
...
Latest status: [September 9 handset and provider acceptance](docs/voice-acceptance-2026-09-09.md). Spoken-quote and PR #61 admission/recording fixes are live. The confirmed 21:52 handset call reached Dispatch but failed conversation acceptance; no note write ran and settlement cleared its hold. Automatic recording persistence and playback passed, including a 43-second unanswered-transfer voicemail. Spoken-reference handling, repetitive answers, delay, final transfer status, remaining boundaries and role acceptance stay open.
