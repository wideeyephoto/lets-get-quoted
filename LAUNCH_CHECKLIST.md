...
- [x] **Quote approval:** Rebuilt quote acceptance to use atomic request receipts. Add-on choices, quote signatures, lead/job stage updates and the feed history commit together with the same request ID pattern. Deposit-on-approval attempts are executed safely after the transaction, recovering properly if retried.
...
- [x] **Local signed website callback recovery:** Saved binding checks repair lost acceptance without resending and preserve stronger delivery outcomes across worker races. Verification: 97 application tests, 57 PostgreSQL checks, full type checking, lint and a clean local security advisor; see the [execution record](docs/customer-email-ten-step-execution-2026-09-14.md).
- [ ] **Hosted website-notice acceptance:** Verify actual provider workspace/region, apply migrations before the worker, and retain signed callbacks and receiver evidence. Accepted notices lacking final evidence still become manual review after the deadline. No live rollout or automatic retry is included in local verification.
...
- [x] **Local sending-domain restoration notices:** Atomic recovery events, immutable owner messages/provider keys, signed callback recovery and persistent reviews are implemented. Verification: 153 application tests, 75 PostgreSQL checks, lint, registry checks and a clean local security advisor. See the [restoration runbook](docs/runbooks/email-domain-restoration-notices.md). Other owner event families remain open.
- [x] **Local owner-event dispatch protection (first sources):** Client questions and follow-up/more-work events now save their owner notice atomically, retain immutable messages and reconcile signed delivery outcomes. Verification: 192 application tests, 93 PostgreSQL checks, lint, registry checks and clean local security advisor. See the [owner-event runbook](docs/runbooks/owner-event-notices.md). Request IDs for these migrated forms are now verified below; other owner events and hosted background pickup remain open.
- [x] **Local request-retry protection for questions and follow-ups:** Explicit request IDs, atomic scoped receipts, changed-content rejection, deletion tombstones and stable attachment paths are implemented. Verification: 50 application tests, 99 PostgreSQL checks, type checking, lint without errors and a clean local security advisor; see the [owner-event runbook](docs/runbooks/owner-event-notices.md). Other owner request families remain open.
...
- [x] **Local review-link private feedback protection:** Request receipts, invite update, internal job event and owner notice commit together; jobless reviews use the dashboard. Checks: 59 application tests, 116 database checks, type checking, lint and clean local security advisor. Hosted acceptance and remaining owner families stay open.
...
- [x] **Local saved Quick Stop cancellation refund attempts:** Cancellation and refund intent commit before provider submission; one caller uses a stable provider key, saves provider evidence and records completion only after verified accounting. 61 application tests, 135 database checks, full type checking, lint and clean local security advisor passed.
- [x] **Local late Quick Stop refund reconciliation:** Verified provider outcomes close matching attempts after accounting; duplicates and SDK/webhook races cannot submit another refund or duplicate the owner notice. Terminal negative outcomes require review. 78 application tests, 139 database checks, full type checking, lint and clean local security advisor passed.
- [x] **Local connected-account payment setup notices:** Webhook and onboarding-return observations save one owner alert with the account transition, fence stale checks and cancel obsolete pending notices after recovery/reconnection. 106 application tests, 144 database checks, type checking, lint, registry and local security advisor passed.
- [ ] **Connected-account hosted acceptance:** Apply the notice migration before new callers, drain legacy writers, verify background pickup and real owner receipt; deployment and receiver evidence remain open.
- [x] **Local verified full-amount dispute owner notices:** Current provider evidence and guarded payment transitions save opening/lost owner notices atomically; won outcomes cancel obsolete pending instructions. 217 application tests, 149 database checks, type checking, lint, registry and local security advisor passed.
- [ ] **Dispute completion and hosted acceptance:** Resolve partial/currency-adjusted amounts, incompatible payment states, inquiry notification policy and interrupted invoice/feed follow-ups; verify migrations, real provider outcomes and owner receipt. Formal full-amount handling alone does not close this family.
- [x] **Local recurring failure notices:** First and terminal recurring charge failures save their owner notice atomically with one recording per lifetime attempt. 104 application tests, 154 database checks, lint, registry and local security advisor passed; full type checking passed.
- [ ] **Recurring-charge recovery and hosted acceptance:** Save/reconcile pre-provider attempts and unknown responses, cover terminal exits outside the failure recorder, recover interrupted client/feed effects, and verify deployment and real owner receipt. This owner-notice slice does not close recurring payment safety.
- [x] **Local quote-approval notices:** Client-link approval feed entries save owner notices atomically; duplicate insert races reuse the scoped winner, changed evidence stops pending mail and job promotion preserves later stages. 66 application tests, 158 database checks, lint, registry and local advisor passed; full type checking passed.
- [x] **Local portal-message receipts:** Message, optional job/inbox history and owner notice commit together; matching retries reuse one ID, failed saves preserve form text, and jobless messages remain readable. 69 application tests, 163 database checks, type checking, registry and local advisor passed; lint has no errors.
- [x] **Quote-option read failures:** Failed prerequisite reads and invalid payment/quote totals stop customer option changes before writes or notices. 54 tests, type checking and lint passed. Concurrent changes and atomic notice storage remain open.
- [x] **Local atomic quote-option notices:** Quote items/total, financial history and owner notice commit together after a locked snapshot and current financial checks. Stale changes are rejected; later edits cancel pending obsolete notices. 65 application tests, 167 database checks, type checking, lint, registry and local advisor passed.
- [x] **Local quote-option request receipts:** Stable form request IDs and displayed quote versions prevent delayed retries from replacing newer choices. Receipt failure rolls back the entire change, and deleted jobs retain replay protection. 127 application tests, 171 database checks, type checking, registry and local advisor passed; lint has no errors.
- [x] **Local margin owner notices:** Margin history and eligible owner notice save together with a serialized four-hour cooldown; recovery cancels pending mail and explicit zero floors are respected. 26 application tests, 175 database checks, type checking, lint, registry and local advisor passed.
- [x] **Local margin evaluation recovery:** Cost changes atomically leave versioned work for a bounded worker. Failures and expired leases remain recoverable, newer edits survive old completion, and account cleanup removes pending work. 23 application tests, 179 database checks, type checking, lint, registry and local advisor passed.
- [ ] **Margin release acceptance:** Verify cost-writer concurrency, migration order, background capacity, recovery pickup and actual inbox delivery.
- [ ] **Quote-option release acceptance:** Apply the transaction migration before callers, drain old writers, verify payment/plan concurrency plus hosted delivery.
- [ ] **Portal-message rollout:** Apply the receipt migration before callers, drain legacy senders, verify real portal/inbox delivery and background pickup. SMS recovery, legacy history cleanup and hosted acceptance remain open.
- [ ] **Quote acceptance completion and hosted acceptance:** Bind approvals to accepted quote revisions/request identities and recover add-on/signature/job/lead/deposit steps across interruptions and competing requests. Verify rollout and real owner receipt; the saved approval notice alone does not close this workflow.
- [ ] **Refund recovery and remaining request identities:** Add operator review/closeout and recovery when callbacks never arrive, stable identities for non-Quick-Stop refund requests, and interrupted invoice/customer-message recovery. Hosted provider/receiver evidence remains open.
...
- [ ] **Observe the four newly scheduled cron jobs firing:** partial as of 2026-09-12. `db-guard` (*/5) and `webhook-heal` (*/15) have completed `cron_runs` rows; the healer escalated one unresolved failure for human review. `smart-dunning` (hourly) and `activation-autopilot` (daily 15:00 UTC) have no run rows yet — next scheduled runs 6 a.m. and 11 a.m. Eastern. Cadences were chosen by this audit, not by product: review them against intent. Timestamps, run IDs and source references in the owner's acceptance evidence file, `cron-acceptance-evidence-20260912.md`. **Note:** the two observed runs predate the summary fix below, so "completed" for them means the route returned, not that the worker was error-free.
...
- [ ] **A1 — The quiet-hours gate covers one billing category out of five:** `src/lib/sms-delivery.ts:113` defers only `customer_message`. `payment_message` — the pay links, card-setup and dunning card-update texts that go to homeowners — is not deferred, nor are `verification` or `crew_message`. Close by making the exemptions one reviewed table with stated reasons, deferring consumer payment traffic, and asserting the table in a test.
- [ ] **A2 — Quiet hours are decided at enqueue and never re-checked at send:** `availableAt` is computed once when the row is written; `src/lib/sms-delivery-worker.ts` has no quiet-hours logic. A task enqueued at 8:50pm that fails retryably can be re-attempted after 9pm. Close by re-evaluating recipient-local time at the final egress gate.
...
- [ ] **B1 — Provider error codes are stored and never read:** `extractStatusWebhook` parses `ErrorCode` and persists it, but nothing branches on the value (`21610`, `30007`, `30003`, `30006`). Close by mapping terminal carrier codes (e.g. opt-outs reported via `21610`) to consent suppression.
...
- [ ] **Live Stripe price inspection:** Run `npm run inspect:live-top-ups` against live environment to certify all 12 SKUs have matching `active` prices, amounts, recurring cadences, and `PRICING_CATALOG_VERSION` metadata.
