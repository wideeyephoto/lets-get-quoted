...
- [x] **Resolve the live billing failure and verify rehearsal routing.** The stale live event is audited as superseded by the current processed subscription state, with zero billing/usage changes. All 185 test receipts map to 13 positively identified rehearsal subscriptions. The legacy test endpoint that routed its Preview deployment to production Supabase is disabled.
- [ ] **Activate the paging release and complete the 24-hour observation.** Verify the tested application/workflow release, three consecutive successful scheduled monitor/projection cycles, and a full 24 hours after the latest relevant activation/containment. Inspect new test ingress, live failures, source drift, SMS retries and notification delivery. The four terminal historical SMS outcomes and bounded billing configuration case remain visible during review. Initial observation also found 12 Resend callback failures from two documented contractor-domain rehearsals; their staging-to-production routing needs an explicit disposition. Owner: Brett. **Current-production observation started 2026-09-09 21:13:17 UTC; earliest completion 2026-09-10 21:13:17 UTC (5:13 PM Eastern).** The hourly follow-up is active. Paging code is pushed but not deployed; its release acceptance requires a full window after activation. Update this list and commit the final observation evidence only after the full window passes.
