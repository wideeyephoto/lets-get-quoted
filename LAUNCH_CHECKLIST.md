...
- [x] **Finish actual signed-in browser/API verification (AUDIT COMPLETED; 33 PASS / 3 FAIL):** Three controlled identities signed in through the production magic-link callback. Verified owner/office permissions, useful client/job pages, financial redaction in browser/Focus JSON, real workspace-form switching and revoked access using retained sessions. Final production release: `8a7b129ac4dd68467532cb384da78d5203135a18` / `dpl_Cc1muyh2q6hUJNRcKHqso8DwK1jB`; run 20:06:28–20:07:23 UTC. See the [report](docs/tenant-office-browser-verification-2026-09-09.md) and [36-case HTTP/browser evidence](docs/tenant-office-browser-evidence-2026-09-09.json).
- [x] **Implement and stage-verify financial Data API confidentiality:** Protected raw job columns are denied; the permission-aware application view preserves operational reads and returns quote values only with current financial authority. Raw projections, wildcard/filter attempts and embedded queries verified. Case `FINANCE-REST`; [fix evidence](docs/tenant-office-access-fixes-2026-09-09.md).
- [x] **Implement and stage-verify protected financial writes:** Base-table guards reject unauthorized quote changes, including mixed PATCH payloads and creation/upsert attempts. Operational edits and owner quote saves still work. Case `WRITER-FINANCE`.
- [x] **Implement and stage-verify same-workspace job/client relationships:** A composite foreign key rejects foreign client assignments on insert/update/upsert, with supporting indexes and the existing relationship name preserved. Case `WRITER-FOREIGN-PARENT`.
- [ ] **Release the three access fixes to production and repeat the signed-in run:** All **44 staging browser/API cases passed at 21:58:09–22:01:52 UTC**, plus 17 isolated PostgreSQL scenario groups. Production has not received these changes. Apply the additive migration, deploy the application adapter, then revoke raw financial SELECT privileges; follow the [ordered rollout](docs/tenant-office-access-fixes-2026-09-09.md#production-rollout).
- [x] **Restore and contain the audit fixtures:** Both unauthorized mutations were restored immediately. Cleanup at 20:07:55 UTC removed four client/job records and verified zero active fixture memberships, sessions and grants. Two marked test workspaces are suspended; test identities and audit/initialization records remain for traceability. Across 189 account-scoped tables, payment/message/credit-lot counts did not change during the final run.
- [ ] **Complete the broader surface matrix before full launch sign-off:** Storage, Realtime, crew, invitations/capacity, other financial categories, all server actions and exhaustive route/table coverage still need actual evidence. The former “83/83 passed” report is **invalidated**, and its SQL-only runner is retired; `npm run verify:tenant-office` now invokes the real-session browser harness with explicit target arguments.
