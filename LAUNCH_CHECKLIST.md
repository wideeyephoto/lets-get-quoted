...
- [ ] **Remaining production sign-off:** Deploy and rehearse the patch through LGQ, prove a pending website certificate transition and disposable deletion through deployed app credentials, complete additional message and failure scenarios, then observe the controlled production canary. The seven-day clock has not started. The earlier staging-tagged Gmail probes caused production callback retry failures; their incident remains recorded. Full scope and residual gates are in the [canary record](docs/contractor-domains-canary-2026-09-09.md) and [email go-live list](docs/contractor-email-domain-go-live-checklist-2026-09-09.md).
...
- [ ] **Remaining operator and recording lifecycle acceptance:** Retry an attributable failed receipt through the supported operator action and prove no duplicate effects; verify authorized recording playback and a completed provider deletion. An empty healthy queue does not close these drills.
...
- [ ] **Live sending gate:** Gmail custom-domain delivery, aligned SPF/DKIM and DMARC PASS are proven in the September 9 rehearsal. Production onboarding/activation, Outlook coverage, remaining failure drills, and canary observation remain open; DNS propagation for the rehearsal has completed.
...
- [x] **Stage 5 Gmail header rehearsal (2026-09-09).** An actual quote from `hello@blackholeart.com` arrived in Gmail with SPF PASS for `rsend.blackholeart.com`, DKIM PASS with `d=blackholeart.com`, and DMARC PASS. Reply-To and real provider-rejection fallback were exercised. This used the actual send function with an isolated staging fixture, not deployed product onboarding; Outlook and full production acceptance remain open. See [evidence](docs/contractor-domains-verification-2026-09-09.md).
