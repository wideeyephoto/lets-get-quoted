...
- [ ] **Block unauthorized financial reads from the Data API (CONFIRMED FAILURE):** `jobs.read` without financial grants returned nonzero `quoted_amount` and confidential `quote_items` from the real Data API. Browser redaction passed; the data boundary remains open. Case `FINANCE-REST`.
- [ ] **Block protected financial-field writes by operational office users (CONFIRMED FAILURE):** `jobs.write` without `quotes.write` changed a fixture's price through PATCH; independent database read confirmed persistence. Case `WRITER-FINANCE`.
- [ ] **Enforce same-workspace job/client relationships (CONFIRMED FAILURE):** An A-office writer attached A's job to B's client through PATCH. Case `WRITER-FOREIGN-PARENT`.
...
- [ ] **Prove what the now-sellable office seat actually buys (AUDITED; THREE LAUNCH BLOCKERS 2026-09-09):** Real signed-in browser/API audit completed with 33 passes and 3 failures. Usable office pages, Focus redaction, workspace switching and revoked access passed; direct quote disclosure, unauthorized price writes and cross-tenant client assignment remain open. See [dated report](docs/tenant-office-browser-verification-2026-09-09.md).
