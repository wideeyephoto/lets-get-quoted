...
- [ ] **Repeat the owner-field PostgreSQL gate on the immutable release SHA**: application release `bd25aa7` is committed, CI-green, and includes the broader **122-file / 1,295-test** voice/message selection plus typecheck and schema mirror/order checks. A separate post-commit rerun of the PostgreSQL 17 owner-field harness remains open; do not treat the Voice provisioning/step-up database harnesses as a substitute for that shared-number field-routing proof.
...
- [ ] **Production-prove AI Voice dispatch before calling it launch-ready**: hosted migrations, green CI, and the exact READY app deployment are complete on `bd25aa7`; the controlled owner call is not. After exact paid-number authorization, correlate dedicated-number ingress → caller admission → `/api/voice/swaig` exact action → `/api/voice/receipt` settlement/history → recording callback/playback and allowance usage. Replay the same provider payload to prove no duplicate action, confirm a staff call creates no customer lead, and inspect runtime/webhook/operator-review failures before marking the canary complete.
...
- [ ] **Provision and verify a separate BrokePipes AI Voice number before the live call (production inventory checked 2026-09-03)**: the hosted database currently has **0** account voice numbers, **0** voice settings rows, **0** `contractor_dedicated` sender rows, **0** purchase authorizations, and **0** `ai_voice_route_verified` events. SignalWire owns only shared `***2323`; its independent voice handler is `ai_agent` with no request/status URL into this app, while SMS correctly remains bound to `/api/sms/inbound`. Do not repurpose or reclassify that live shared SMS number. The spend-policy/provisioning schema is hosted but empty and dark, no provider purchase has occurred, and a new purchase must remain blocked until the owner gives action-time authorization for one exact E.164 number and its exact recurring/per-minute charges. The operator-observed SignalWire Dashboard candidate rental was **$0.50/month** (Dashboard evidence, not an API price authority); published usage pricing remains $0.16/min AI Agent runtime plus $0.0066/min inbound transport. After approval, bind the exact provider resource/account, configure signed POST callbacks to `/api/voice/ai` and `/api/voice/provider-status`, activate settings and allowance, enable the feature only for the controlled canary, and require fresh provider-readiness evidence before exposing the number as ready.
