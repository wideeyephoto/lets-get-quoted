...
- [x] **Prove a live charge refunded through the LGQ engine, including both reversals.** Re-read the September 7 $1.00 partial refund of a $150.00 payment. Stripe request (identifier retained privately) used LGQ's exact idempotency key, `reverse_transfer=true` and `refund_application_fee=true`; refund (identifier retained privately) succeeded. Stripe shows transfer reversal (identifier retained privately), $1.00 of the transfer reversed and $0.01 of the $0.75 application fee refunded. Production payment, admin audit and current LGQ page reconcile. This was a programmatic LGQ rehearsal; a new dashboard-click or full refund was not executed.
- [x] **Finish the local six-SKU unpaid, ordering and refund-recovery matrix.** Corrected grants on unpaid/no-payment-required provider sessions; 12 new cases failed before the fix. All 24 six-SKU negative cases now pass. PostgreSQL: 28/28, including refund before fulfillment, all six partial/full reversals, future-minute debt, expired claims, repeat cancellation intent, base-plan-change attribution, mode isolation and denied browser access. These are local/database checks; prior real sandbox lifecycle evidence remains separately recorded.
- [x] **Prepare current-source add-on refund and renewal-date fixes.** Integrated the pending PR #33 refund implementation and current Stripe item-period correction into this review branch. Preserved operational-alert scheduling. Added customer-visible refund/debt explanation behind the refund flag and removed the unverified paid claim on checkout return. Full suite: 14,374 tests / 1,120 files passed. This code is not yet production acceptance.
- [ ] **Release and verify paid add-on refund handling in production.** September 9 production catalog reads found no refund jobs/reversals tables or apply RPC. Apply the reviewed migration, deploy the code, configure the platform refund webhook and worker in the documented order, then verify the deployed revision and actual processing.
- [ ] **Execute the remaining live paid add-on journeys.** Brett authorized payer the designated payer and up to $248 total; this task spent $0. The designated Flex workspace is eligible for Voice Flex, the minute pack and storage ($119 before tax). Designate eligible Solo/Growth workspaces for the remaining three SKUs. Reconcile real purchases, usable benefits and refunds; verify natural renewal and effective cancellation when their actual periods elapse. Extra base-plan or renewal charges must fit separately approved spending. Sandbox and local checks do not close this item.
