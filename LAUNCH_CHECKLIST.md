...
- [x] **Terminal Tap-to-Pay fail-open on Stripe error, fixed 2026-09-11:** `confirmTerminalPayment` (`src/lib/stripe-terminal.ts`) initialized `isPaid = true` and only ever set it `false` inside an explicit branch; a `stripe.paymentIntents.retrieve()` call that **threw** — a network blip, a rate limit, an expired PaymentIntent — fell through its `catch` with `isPaid` still `true`, and the code then wrote `status: 'paid'` to the payment row and marked the linked invoice paid on the strength of an error, never a confirmation. This is a real-money, card-present collection path, not a demo. `isPaid` now defaults `false` and a failed retrieve returns `status: 'processing'` without touching the payments row or the invoice — the UI already handled that case correctly (`PaymentModals.tsx`'s existing `confirmRes.data?.status === 'succeeded'` check), it just never reached it. New regression test in `test/stripe-terminal.test.ts` forces the retrieve to reject and asserts no write and no invoice mutation; proven to bite by reverting the fix and observing the test fail. Full suite 15,081/1,181 files, typecheck, 0 lint errors pass. Found and fixed while surveying code/UX quality for launch; not previously tracked anywhere.
...
- [ ] **Customer-facing incident channel:** 7 alert categories page the operator; nothing informs a
  ...
  `admin_actions`, and a rehearsed incident cycle. **Source work complete 2026-09-11; nothing
  deployed.** The reviewed publishing migration was unsafe to apply and is rewritten: its lone
  policy had no `TO` clause and no column restriction, so `select *` on a published row would have
  returned `root_cause`, `owner`, `created_by` and `external_url` anonymously, and it left
  Supabase's default write grants in place — **TRUNCATE is not subject to RLS, so this table has
  been truncatable by any browser session since 2026-08-06**. The page reported "All Systems
  Operational" whenever it could not read the log; unknown is now its own state.
  `togglePublishIncidentAction` had no caller anywhere in the app, so `/status` could only ever
...
Fixed in the same pass, verified by re-running the same tool after each change:
