...
- [x] **Prepare a fresh encrypted recovery pack after rotation:** 22:03 UTC production snapshot, native passkey schema, all 38 Storage objects, released source, committed recovery/DNS evidence and updated local configuration authenticate locally. [Pack evidence](docs/runbooks/evidence/dr-post-rotation-pack-2026-09-09.json).
- [x] **Upload the approved fresh recovery pack and verify cloud presence:** encrypted publication and mounted authentication passed at 22:24 UTC; Google Drive web shows the pack and receipt, private to the owner.
- [ ] **Independently download and authenticate the fresh pack:** distinct from mounted readback and web presence. The earlier downloaded pack already passed full hosted restore; this new post-rotation pack needs its own download check.
