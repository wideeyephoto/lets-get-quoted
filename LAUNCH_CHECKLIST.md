...
- [ ] **P0 — Finish staff account-export authorization and auditing verification**: deployed `304b2b06` calls `requirePermission('account.export')` before service-role access, and the anonymous nonexistent-account probe now returns an opaque empty 404 instead of the prior account-lookup JSON oracle. The two route tests still mock authorization; add inactive-staff and missing-permission denial cases, prove an authorized export plus persisted audit row, and verify revocation behavior.
...
- [x] **Deployed narrow state guard**: production release `304b2b06` includes the source guard making `executeWalletRefillCharge` refuse inactive, paused, and cancellation-scheduled campaigns. Live/provider and concurrency verification remain open.
- [x] **Deployed narrow invoice guard**: production release `304b2b06` no longer blindly reactivates an unprovisioned or paused campaign on `invoice.paid`. Out-of-order event/version handling and Google-state reconciliation remain open.
...
- [ ] **Disposable-Account Deletion & DSAR Relational Reconcile Drill**: reconcile the 25-entry registry against all 111 schema tables and run a real disposable-account export/delete drill covering relational rows, nested Storage, Stripe/vendor cleanup, audit evidence, retries, partial failures, and recovery.
...
- [x] **Deployed owner-phone OTP rate-limit source guards**: production `304b2b06` uses strict distributed buckets for send and verify; live concurrency/fail-closed behavior remains part of the abuse audit.
- [x] **Abuse & Cost-DoS Distributed Limits & Spend Parameter Clamping (Remediated 2026-08-31)**:
  - Added strict distributed rate limiting across public permit diagnostics (`/api/permits/health`, `/api/permits/public-estimate`, `/api/permits/preview`).
  - Added distributed rate limiting (`checkRateLimitStrict`) on ad budget checkout creation (`/api/stripe/ad-budget`).
  - Enforced server-side canonical platform fees (`AD_PLATFORM_FEE_RATE`) and strict amount clamps ($50–$15,000/wk, $100–$50,000/mo, $50–$10,000 deposit) in `createAdBudgetCheckoutSession`, eliminating client-controlled fee tampering.
...
- [x] **Local Cold-Cache Mobile Media & Interaction Diagnostic**: exact code-equivalent release source was built and tested at 390×844 in fresh cache-disabled Chromium contexts. All four routes loaded without overlays, console/page errors, failed responses or CLS; Pricing, AI Intake and Website Builder target images deferred until scroll and decoded; the homepage control, Pricing lightbox, AI replay/payment flow, Website Builder keyboard/touch slider and video tab worked.
  - Homepage requested five hero trade WebPs before scroll (78,062 B); a decorative tape-measure image became the observed local LCP candidate at 1.908s despite lazy/low priority.
  - Website Builder fetched a 997,925 B autoplay MP4 before scroll. Completing the AI Intake sample flow fetched three raw JPEGs totaling 2,586,425 B with default loading/decoding. These are new optimization targets.
  - Local single-run LCP/CLS values are diagnostic only; Next also warned that image quality `80` is absent from `images.qualities` for a future Next 16 requirement.
- [ ] **Throttled Performance & Field Validation**: run at least three cold samples per route under the established 1.6 Mbps / 150 ms / 4× CPU profile, compare request bytes and LCP/CLS to the pre-change baseline, test Safari/iOS decode/memory behavior, and confirm production field Web Vitals before accepting the image changes.
