...
- [x] **5-Tier Hardening Backlog Completion & Verification:** Completed audit, automated gating safeguards, and test coverage across all 5 tiers of [`docs/hardening-backlog-2026-09-14.md`](docs/hardening-backlog-2026-09-14.md).
  - **Full Test Suite & Typecheck:** 564 test files, 9,649 tests passed (0 failures, 0 regressions in 34.78s). `npm run typecheck` (`tsconfig.test.json`) clean with 0 errors.
  - **Automated Hardening Readiness Audit (Tier 1 & Sequencing):** Built and verified `scripts/verify-hardening-readiness.mjs` (`npm run verify:hardening`) validating 17 critical deployment and structural invariants (subscription lifecycle flag ordering in `.env.example`, top-up webhook before purchase, capacity lifecycle sweep before recurring SKU activation, overage period close & settlement ordering, legacy rail dependency sequences, status of 12 unconfirmed migrations, and demo payment containment). Commit `1c6a5401`.
  - **Metering Prerequisites & Storage Carve-out (Tier 2):** Added `test/metering-prerequisites.test.ts` verifying `LGQ_USAGE_RESERVATION_EXPIRY_ENABLED` mapping, storage cap independence from usage sweep, and fail-open/measure-only baseline configurations across all 4 meter types (Text credits, Marketing email, AI writing drafts, AI Intake). Commit `12ecb92c`.
  - **Messaging Carrier Boundaries (Tier 3):** Added `test/messaging-carrier-hardening.test.ts` enforcing 10DLC callback route remains unsupported/blank, fail-closed lead verification when secrets are absent, dedicated number provisioning fail-closed protections, and default dark SMS traffic lanes. Commit `2ce4d554`.
  - **Voice Lot Tail Coupling (Tier 4):** Exported `VOICE_RESERVATION_TTL_MS` (90 min) in `src/lib/billing/voice-minute-usage.ts` and added `test/voice-minute-lot-tail-coupling.test.ts` verifying Postgres `voice_minute_lot_tail()` (120 min) >= 90 min reservation TTL with positive safety margin to prevent end-of-period call rejections. Commit `3447c257`.
  - **Worker & Gate Observability (Tier 5):** Added `gateEnvVar` mappings for 15 gated background workers in `src/lib/cron-jobs.ts` with `cronGateStatus()`, and exposed real-time Gate status badges (`Live`, `Dark`, `Always on`) on `/admin/health` (`src/app/admin/health/page.tsx`), providing operational visibility before flipping rollout flags. Covered by `test/cron-jobs-gate-observability.test.ts`. Commit `c28dfed9`.
