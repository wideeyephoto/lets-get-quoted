...
- [x] **Triage the historical failure backlog (2026-09-09).** All **221** baseline records have individually reviewed dispositions: 185 verified obsolete billing rehearsals, 1 superseded live billing receipt, 29 terminal voice authentication rejections, 2 reconciled/superseded registry callbacks and 4 terminal SMS failures with matched accepted usage. The unsupported aggregate replay claim is corrected without erasing its audit. No webhook or customer SMS was replayed. See the [cleanup report and evidence](docs/operational-cleanup-2026-09-09.md).
- [x] **Prove paging when the email provider is unavailable (2026-09-09).** An isolated Resend HTTP 503 drill delivered one SignalWire SMS to the approved owner phone within **22.32 seconds**; the owner confirmed receipt. Reconciliation reused its provider ID with zero additional submissions. Production Vercel and GitHub paging secrets are configured. Runtime release and observation remain tracked below; simultaneous database/email failure is outside this proof.
- [x] **Resolve the live billing failure and verify Stripe rehearsal routing.** The stale live event is audited as superseded by the current processed subscription state, with zero billing/usage changes. All 185 test receipts map to 13 positively identified rehearsal subscriptions. The legacy test endpoint that routed its Preview deployment to production Supabase is disabled.
- [x] **Retire the owner-confirmed queued voice-rehearsal SMS (2026-09-10).** Cancelled the September 9 BrokePipes missed-call SMS and its delivery task at 15:27 UTC with a separate audit record. Preserved all 66 sender-not-ready deferrals; zero provider requests, SMS reservations or SMS overage charges. Repeat cleanup changed zero rows. The original 221-record batch and existing voice usage remain unchanged. See the [cancellation evidence](docs/evidence/operational-cleanup-rehearsal-sms-cancellation-2026-09-10.json). Observation now explicitly checks queued/deferred SMS; earlier failed-only checks did not establish that all messages were delivered.
- [x] **Verify the 24 Resend rehearsal callback dispositions (corrected September 10, 22:30 UTC).** The domain task had already reviewed all 24 retained rows by September 9 at 23:44 UTC. Verified the explicit `resolved_by` notes and shared incident marker in production, including signed HTTP-202 quarantine evidence and no tenant reassignment or email resend. The earlier observer omitted those fields and incorrectly treated the absence of a separate admin audit as missing disposition. The observer and final report are corrected; general Preview isolation and the unrelated unsigned voice callback remain separate. See the [recheck evidence](docs/evidence/operational-cleanup-release-recheck-2026-09-10.json).
- [x] **Verify paging deployment and initial scheduled cycles (2026-09-10).** Production `d06454e6a` became READY at **15:55:58 UTC** with the exact tested paging module and GitHub workflow secret references. Three scheduled application monitor and billing-projection cycles succeeded at 16:00, 16:05 and 16:10 UTC. Independent scheduled watchdog run `34509437024` succeeded on this revision at **17:38 UTC**; its 225.93-minute start-to-start gap leaves timing assurance open. See the [activation evidence](docs/evidence/operational-cleanup-paging-activation-2026-09-10.json) and [watchdog follow-up](docs/evidence/operational-cleanup-observation-20260910T181407Z.json).
- [ ] **Repair the overage deployment/database mismatch (2026-09-10).** The 15:37 UTC settlement run returned HTTP 500 / `worker_errors=1`. Both that deployment and current production call recovery functions absent from the production catalog; `overage_recovery_guards` is not in applied migration history. Zero settlement rows and zero new overage accruals were found. Coordinate the database rollout with the deployed worker using `docs/overage-recovery.md`, then verify scheduled recovery. No migration or financial replay was performed by observation. See the [failure and catalog evidence](docs/evidence/operational-cleanup-observation-20260910T161329Z.json). The 22:28 UTC recheck still found the three required functions and migration absent; the 21:37 UTC run failed again.
- [ ] **Investigate recurring worker timeouts and incomplete cron evidence (2026-09-10).** Voice allowance reported Gateway Timeout at 17:30 and 18:15 UTC. Direct-payment settlement failed at 17:55, then recovered; its separate 18:10 request returned HTTP 200 but lacks a database completion record. `finishRun` does not inspect returned update errors. Preserve the unknown outcome fields and verify recording reliability; do not infer missed credits, a stuck worker or a successful business effect from these gaps. Current direct-payment task and SMS queues are empty. See the [dated worker and runtime evidence](docs/evidence/operational-cleanup-observation-20260910T181407Z.json). Recent uncommitted changes in `prelaunch/autonomous-fixes-20260910` inspect/log returned completion and prune errors, with a regression test file present; production remains on `d06454e6a` without this change.
- [x] **Complete and report the fixed 24-hour observation (2026-09-10): operational acceptance FAIL.** Preserved September 9 21:13:17.282 UTC through September 10 21:13:17.282 UTC as the exact interval; final checks at 21:15-21:17 UTC. Recorded 288 successful billing-projection, 288 successful monitor and 1,439 successful SMS cycles, with the documented SMS start-record gap. All 221 original audits/fingerprints and the separate rehearsal cancellation remain intact; no failed live billing receipt, post-containment test ingress, pending SMS, new SMS failure, duplicate checked identity or unconfirmed alert remains at the final snapshot. Six overage failures, four voice timeouts, two direct-payment failures and seven incomplete cron records prevent a pass. See the [final report](docs/operational-cleanup-2026-09-09.md#final-fixed-window-result--september-10-2117-utc) and [bounded result evidence](docs/evidence/operational-cleanup-observation-result-2026-09-10.json). The hourly task follow-up is paused; production schedulers remain running. The report is committed locally; publication of the accumulated evidence awaits payload-specific approval after automatic push review rejected it.
- [ ] **Complete remaining operational release acceptance.** Repair the overage deployment/database mismatch and recurring worker/recording failures; classify the separate unsigned voice status rejection; establish timely independent watchdog execution. Seven scheduled watchdog runs succeeded in the fixed interval, but gaps reached 280.75 minutes. Production paging is deployed on `d06454e6a` with three initial scheduled application cycles verified, yet only 5 hours 19 minutes of its separate observation had elapsed at the final snapshot. **Paging minimum: September 11 at 15:55:58.476 UTC (11:55 AM Eastern), subject to later relevant changes.** A fresh relevant stability window and the outstanding acceptance evidence are required after authorized repairs; elapsed time alone is not a pass. Keep four historical SMS outcomes and the bounded billing configuration case visible. Owner: Brett. No production migration, replay or new drill is authorized by this observation result.
...
- [ ] **Remaining staging billing and customer presentation cases.** Complete initial failed-payment, remaining out-of-order/worker-recovery and plan-change scenarios, plus customer-visible explanation of refund debt before production acceptance.
