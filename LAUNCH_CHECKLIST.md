...
**Current status, September 15 16:50 UTC:** The seven-day canary remains on attempt 2, started **September 14 at 17:06:28.748793 UTC (1:06 PM Eastern)** after the quote-send repair. **1/7 qualifying scheduled runs and zero complete days** are recorded. The September 15 06:23 UTC run checked the active domain and refreshed verification with zero errors or notification backlog. The first full observation day ends September 15 at 17:06:28.748793 UTC; the next daily check is September 16 at 06:23 UTC. The earliest seven-day review remains **September 21 at 17:06:28.748793 UTC**, subject to all acceptance gates. Actual in-window product quotes now passed both receivers: **Gmail September 15 at 04:49:48.005 UTC** and **Outlook at 10:45:08.772 UTC**. Each reached Inbox (Outlook Focused), passed SPF/DKIM/DMARC with exact-aligned DKIM for blackholeart.com, retained the correct Gmail Reply-To, and opened the matching unsigned J-1004/$0 quote page. Outlook also passed Microsoft composite authentication. **Both September 15 inbox allowances are used.** No reply was sent; new-window actual replies remain pending. The fixture retains its approved Outlook recipient, null phone, $0 amount and unsigned/unscheduled state. The September 14 Outlook recovery receipt is baseline evidence only; the retired attempt retains three successful runs and two complete days without carryover. The customer-facing app remains on passing **8126f4e0**, confirmed by a successful deployment-scoped health request. A newer main build, **c6be758b**, is READY but failed three release tests; later database/typecheck/lint/CI-build stages were skipped. The actual 15:27 website check ran successfully on that newer build, reopening app/scheduled-worker release consistency after the earlier 12:49 alignment. The changes add navigation preferences and leave custom-email transport, claims, quote actions, callbacks, domain/closure workers and schedules unchanged. No active email-path failure was observed, so the current canary start stands; the next daily email check remains September 16 at 06:23 UTC. Closure-registry review now confirms seven account-owned messaging tables are classified as global and skipped by the configured cleanup action. An in-memory execution of the deployed code requested completion without touching them; no hosted closure was performed. Their ownership, disposition and export rules must be repaired and accepted before closure sign-off. The empty disposable fixture has no rows in those tables; its deadlines and the email canary remain unchanged. The verified domain retains all three verified provider records, zero open notices and zero unresolved Resend callbacks. Healthy empty website scans establish normal completion recording, not hosted fault recovery or pending-site promotion. PR #80 remains at remote **84aa41fbe**, with 14 files, conflicts and no full CI for that head. A separate local reconciliation against main **8126f4e0** resolves those conflicts, preserves already-released workers and the canonical repair, and removes the equivalent duplicate migration. The remaining seven-file difference contains rollout records, closure regression coverage and mailbox guidance. Local validation now passes all 17,553 application tests across 1,467 files, the 433-page production build and a dependency audit with zero vulnerabilities. The matching 10 PostgreSQL closure checks, full application/test type checking and lint (warnings, no errors) also pass. The previously approved metadata is already published; a revised description for the reconciled scope is prepared locally. Branch publication, hosted CI and Preview acceptance, and release remain pending. The disposable closure fixture retains both October 12 deadlines and pending acceleration approval. Remaining lifecycle, message-path, direct-to-From mailbox, cleanup and operational gates still block launch. Keep enrollment restricted to BrokePipes. [Full current evidence](docs/contractor-domains-canary-2026-09-09.md#september-15-passing-production-release-and-continuity).
...
- [x] **First Outlook receipt in the new canary window (September 15, 10:45 UTC):** One actual product quote reached Outlook Focused Inbox with SPF/DKIM/DMARC and Microsoft authentication PASS, expected Gmail Reply-To and the matching unsigned J-1004/$0 page. Provider, send claim and canonical delivery agree with one attempt and no duplicate. Both September 15 inbox allowances are used. Actual replies remain pending; the canary remains 1/7 qualifying scheduled checks and zero full days.

- [x] **First qualifying scheduled check in attempt 2 (September 15, 06:23 UTC):** The run checked the active domain, refreshed verification and recorded zero errors/backlog. Database completion and deployment-scoped HTTP 200 agree. Attempt 2 is **1/7 qualifying checks and zero full elapsed days**; first complete day is due at 17:06 UTC. The existing app/worker release-validation issue and remaining acceptance gates stay open.

- [x] **First Gmail receipt in the new canary window (September 15, 04:49 UTC):** One actual product quote reached Gmail Inbox with SPF/DKIM/DMARC PASS, the expected Reply-To and the matching unsigned J-1004/$0 page. Provider, send claim and canonical delivery agree with one attempt and no duplicate. The test contact is restored to Outlook. Gmail allowance used; Outlook and replies remain available/pending under daily limits. This does not count as a scheduled domain check.

- [x] **Production quote-send revision repair:** The new document sender failed before provider submission because the guarded job view omitted its revision. The scoped database repair is applied; 14 office-boundary and 14 document-ledger PostgreSQL checks pass. A single actual recovery quote passed Outlook delivery/authentication and its matching J-1004/$0 link. PR #90 retains the repair and is approved/merged with full candidate CI passed; the combined release is live and its required merged-head CI passed.

- [ ] **Website watcher run-record recovery:** PR #87 head `ad6799aa7` passes 145 focused tests, full CI (17,368 tests, configured PostgreSQL suites, typecheck/lint/security audit and 433-page build), and its exact Preview homepage/console smoke check. It is approved, merged and live in `fa6e6908` after required merged-head CI passed. The first post-release scheduled website run at 17:30 passed with HTTP 200 and a durable empty summary. Hosted injected-fault acceptance remains pending; normal completion does not establish recovery from a failed start write. Preserve the two historical missing records separately from the three failed scans; no backfill is claimed.

- [x] **Third website watcher failure investigated (September 14, 12:43 UTC):** The 12:00 ownership-inventory read failed with Gateway Timeout/HTTP 500 and recovered on scheduled 12:15 and 12:30 runs. Both website certificates and verification remain valid. Retain three failed website scans and zero failed email-domain runs; the email canary remains 3/7 qualifying checks and two completed days. PR #85 remains undeployed with its existing merge approval pending. See [incident evidence](docs/contractor-domains-canary-2026-09-09.md#september-14-third-website-inventory-timeout).

- [x] **Second website watcher failure investigated (September 14, 09:42 UTC):** The 09:00 orphan-ownership read failed with Gateway Timeout/HTTP 500; scheduled 09:15 and 09:30 runs recovered. Both website certificates and verification remain valid. Retain two failed website runs separately from the email canary's **3/7 successful checks and two elapsed days**. No mail or domain mutation occurred. See [incident evidence](docs/contractor-domains-canary-2026-09-09.md#september-14-orphan-ownership-read-timeout-and-retry-candidate).

- [x] **Website watcher read-retry deployment:** PR #85 merged September 14 at 14:59 UTC and is present in current production `b1c4bdd5`. Its bounded transient SELECT retries are deployed. Scheduled empty scans at 16:30, 16:45 and 17:00 have durable successful summaries. Hosted fault injection and the upstream timeout cause remain open; this does not repair historical telemetry gaps.

- [x] **Third qualifying scheduled check (September 14, 06:23 UTC):** Run `de510d4a-746d-423c-b5a9-1c8e9e05ea7c` checked the active domain, refreshed verification and reported zero errors/backlog. Database/provider checks at 06:43 UTC retain the same verified binding and production release, with no open notices or unresolved callbacks. This brings the canary to **3/7 scheduled checks and two completed 24-hour days**; the third full day is due at 16:23 UTC. Retain the earlier recovered website failure and all remaining acceptance gates. No test message or reset occurred. See [scheduled-run evidence](docs/contractor-domains-canary-2026-09-09.md#september-14-third-scheduled-check).

- [x] **In-window Gmail reply (September 14, 04:42 UTC):** Reply on the actual September 12 product receipt automatically selected the configured Gmail address. The single marked reply appeared in Inbox with matching original/reply Message-IDs. This is same-mailbox receipt evidence, without a new external authentication claim. Both Gmail and Outlook reply-routing/receipt subchecks now pass; E04 remains open for direct-to-From mailbox behavior. September 14 Gmail allowance is used. The 04:49 UTC checkpoint retains the original start, **2/7 scheduled checks and two elapsed days**, plus the previously recovered website failure. See [reply evidence](docs/contractor-domains-canary-2026-09-09.md#september-14-in-window-gmail-reply).
...
- [x] **Deployed reply guidance and continuity (September 13, 20:46 UTC):** READY `a773f17a` adds reply-address input validation and verified-domain reply/alias guidance. Actual BrokePipes settings display the new guidance and existing Gmail reply address; database/provider checks remain healthy with **2/7 scheduled checks and two elapsed days**. Sender/reply selection and domain lifecycle paths are unchanged, so the original start stands. No message or settings save occurred. A06/E04 and the separate undeployed no-inbox/subtitle candidate remain open. See [hosted guidance evidence](docs/contractor-domains-canary-2026-09-09.md#september-13-deployed-reply-guidance-and-continuity).

- [x] **Production monitoring release continuity (September 13, 19:47 UTC):** READY `7a356f28` changes founder pages and operational monitoring; contractor sending, domain actions/reconciliation, quote routes, closure-domain worker and domain schedules are unchanged. Database checks retain the verified binding, **2/7 scheduled checks**, two completed days and zero failed domain runs, open notices or unresolved callbacks. The hourly canary is active with its original start; final acceptance and staffed-alert verification remain open. See [continuity evidence](docs/contractor-domains-canary-2026-09-09.md#september-13-monitoring-release-continuity).

- [x] **Production release continuity (September 13, 17:41 UTC):** READY `cf8bfad3` adds blog/operator features while retaining contractor sending, domain management/reconciliation, quote routes and closure-domain worker behavior. Shared host, cron and disposition changes were reviewed; live database/provider checks remain healthy. The canary retains its original start, **2/7 checks and two completed days**. No new mail or domain mutation occurred. See [release-continuity evidence](docs/contractor-domains-canary-2026-09-09.md#september-13-blog-and-operator-release-continuity).

- [x] **Second complete observation day (September 13, 16:23 UTC):** The 16:42 UTC database/provider checkpoint confirms two full 24-hour days, **2/7 qualifying scheduled checks**, the same verified binding and production release, and no open notices, unresolved callbacks or failed domain runs. No new send or scheduled email check was triggered. Earliest seven-day review remains September 18 at 12:23 PM America/New_York; final acceptance stays open. See [elapsed-day evidence](docs/contractor-domains-canary-2026-09-09.md#september-13-second-complete-observation-day).

- [ ] **Mailbox guidance candidate (September 13):** Local settings copy explains that domain connection creates no inbox, identifies the Customer reply email setting and explains the separate mailbox/alias requirement for new messages addressed directly to From. The verified subtitle describes sending verification without claiming receiver authentication. Targeted ESLint passed; deployment and hosted copy acceptance remain pending. A06/E04 remain open. See [candidate evidence](docs/contractor-domains-canary-2026-09-09.md#september-13-mailbox-guidance-candidate).

- [x] **Second qualifying scheduled check (September 13, 06:23 UTC):** Run `0644860a-85f7-4486-95a6-379c7f1cc66d` checked the active domain, refreshed verification and reported zero errors/backlog. Database/provider checks at 06:42 UTC remain healthy on the same production release and binding. This brings the canary to **2/7 scheduled checks**, with one full 24-hour day elapsed. No new test message or reset occurred; remaining acceptance gates stay open. See [scheduled-run evidence](docs/contractor-domains-canary-2026-09-09.md#september-13-second-scheduled-check).

- [x] **In-window Outlook reply (September 13, 04:43 UTC):** Reply on the actual September 11 product receipt automatically selected the configured Gmail address. Gmail received the single marked test reply after two seconds; original/reply Message-IDs correlate, and receiver SPF/DKIM/DMARC checks passed for the Microsoft reply sender. September 13 Gmail allowance is used. E04's in-window Gmail reply and visible From mailbox behavior remain open. The 04:49 UTC database check confirms the original start and **1/7 qualifying scheduled checks**. See [reply evidence](docs/contractor-domains-canary-2026-09-09.md#september-13-in-window-outlook-reply).
...
- [x] **Start single-workspace observation (September 11, 16:23 UTC):** At Brett's request, the healthy, already-live BrokePipes domain now enters seven-day observation while remaining drills continue. The app resolves to the accepted READY release; the sole contractor email-domain row and all three provider DNS records are verified, with no unresolved callbacks or open notices. Hourly follow-ups track the window from **16:23:14.731206 UTC**, with no backdated days or runs. First qualifying scheduled check is due September 12 at 06:23 UTC; earliest seven-day review is September 18 at 12:23 PM America/New_York. Active-domain disruptions restart the healthy window after recovery. This completes starting observation, not the remaining launch criteria. See [baseline and daily log](docs/contractor-domains-canary-2026-09-09.md#september-11-observation-start).

- [x] **Production Gmail quote acceptance (September 11, 05:14 UTC):** [PR #76](https://github.com/wideeyephoto/lets-get-quoted/pull/76) fixes quick-edit HTTP 500 caused by nonexistent `jobs.updated_at`; full CI passed **14,884 tests**, nine PostgreSQL checks and build. Both production aliases serve merge `7f32e44c7c`, deployment `dpl_Uk5c8x51awwv2cXU5Dhnac2ErMD2`. The repaired contact edit and actual LGQ quote action passed. Gmail Inbox received J-1004 from the current custom-domain binding with SPF/DKIM/DMARC PASS and the correct Reply-To; the received button opened the matching $0 test quote. The original test recipient was restored without another send. Today's Gmail allowance is used; Outlook remains unused. See [dated receipt and release evidence](docs/contractor-domains-canary-2026-09-09.md#september-11-release-and-gmail-product-acceptance). Remaining lifecycle drills, supported fixture cleanup and the seven-day observation remain open.
...
- [x] **Outlook and quote-link follow-up:** Outlook received the custom-domain probe with SPF/DKIM/DMARC PASS; its reply arrived in Gmail. Corrected the probe's homepage-link setup by creating and emailing real $0 quote J-1004 through production LGQ; the received button opens that exact quote. Added atomic domain reservation and durable foreign-workspace webhook quarantine; 172 focused tests and 17 PostgreSQL checks passed. Both databases have the reservation index. See [follow-up evidence](docs/contractor-domains-canary-2026-09-09.md).
- [x] **Controlled release and callback recovery:** Owner-approved Production settings admit only BrokePipes. Deployed LGQ created its pending `blackholeart.com` binding; the required return-path DNS records are published. Another workspace's settings omit enrollment. Four authentic provider replays and four later retries returned 202 and persisted quarantine; at 23:44 UTC all 24 known rehearsal incident records remain retained with explicit reviewed disposition and zero unresolved. The website watcher also checked and attached one real pending disposable domain through deployed credentials.
- [x] **Production domain verification:** Owner-approved [PR #67](https://github.com/wideeyephoto/lets-get-quoted/pull/67) is merged as `1f095c1f0c` and deployed as `dpl_8SxWnaFVGvJasG8mMoSrFhKCe5ar`; both public app/webhook aliases were verified after promotion. Full CI passed 14,593 tests and build. The released Check Connection button recognizes completed verification; BrokePipes reached Verified through the production UI and database at 22:48:18 UTC.
- [x] **Production disconnect/reconnect (September 10 UTC):** [PR #68](https://github.com/wideeyephoto/lets-get-quoted/pull/68) is merged as `72b3a4662e` and live as `dpl_Pmo6VeDnWAv9WKy1kiodGUHP8Jim`; both public aliases were verified. Full CI passed 14,597 tests and build. The on-page confirmation passed cancel and confirmed-disconnect checks. The old DB/provider binding was removed and its slot released. Production reconnect created a new binding and restored `hello@blackholeart.com` to Verified at 02:13:28 UTC. F08 evidence is in the [canary record](docs/contractor-domains-canary-2026-09-09.md).
...
- [x] **Active-domain scheduled reconciliation (September 10–11, 06:23 UTC):** September 10 run `93219051-6b33-40be-84fd-b68ce8f588d5` checked the original product binding. September 11 run `59b08397-67fb-420f-b856-6765d8d3a459` checked the replacement binding after provider-loss recovery, retained verification and refreshed `last_checked_at` to 06:23:20.954 UTC. Both had zero errors, provider orphans or backlog; independent provider verification passed. These preparation checks predate the September 11 16:23 UTC observation start and are not counted in its seven scheduled runs. See the [scheduled checkpoint](docs/contractor-domains-canary-2026-09-09.md#september-11-scheduled-reconciliation-checkpoint).
...
- [x] **Account-closure domain safeguards released (September 10, 14:41 UTC):** [PR #73](https://github.com/wideeyephoto/lets-get-quoted/pull/73) merged as `83e5663274` and serves both public aliases as `dpl_YZRey9qLGxen8BFNZyYPM1SWwrWG`. The three ordered migrations and hosted rollback drill passed in production. Full CI passed **14,643 tests**, nine PostgreSQL checks, type checking, lint and build. The release captures provider targets, guards retries/completion and 30-day recovery, repairs the hosted request contract and corrects website-alert copy. See [procedure](docs/runbooks/account-closure-domains.md).
- [ ] **Supported production fixture cleanup acceptance:** The deployed form correctly describes the 30-day recovery period. Its fixture-only submission reached LGQ's staff second-factor requirement before mutation; database inspection confirms zero closure jobs and unchanged fixture routing. Complete staff verification, schedule the empty fixture, then exercise eligible deployed cleanup and confirm provider release. Actual cleanup and DNS fixture removal remain open.
- [ ] **Remaining production sign-off:** Gmail/Outlook product quote coverage and scheduled certificate promotion are complete. Finish remaining message/recovery/hold/rollback drills and supported fixture deletion alongside the observation started September 11 at 16:23 UTC. Require seven healthy elapsed days, seven consecutive post-start active-domain scheduled runs, representative in-window mail/authentication/replies and all remaining launch gates. Active-domain disruption requires a fresh healthy window after recovery. Hourly follow-ups are ACTIVE; preserve the one-test-per-approved-inbox daily limit and check current inventories before each send. LGQ staff security verification is still required for fixture closure. Full scope and evidence are in the [canary record](docs/contractor-domains-canary-2026-09-09.md) and [email go-live list](docs/contractor-email-domain-go-live-checklist-2026-09-09.md).
...
- [x] **Finish the local six-SKU unpaid, ordering and refund-recovery matrix.** Corrected grants on unpaid/no-payment-required provider sessions; 12 new cases failed before the fix. All 24 six-SKU negative cases now pass. PostgreSQL: 47/47 in each LF/CRLF direction, including all six reversals, future-minute debt, expired claims, mode isolation, denied browser access, duplicate delivery and the production mixed-line-ending regression. Paid Voice grants against its verified invoice even when a base-plan allowance exists; refunds cannot debit another period's lot. These are local/database checks; prior real sandbox lifecycle evidence remains separately recorded.
...
- [ ] **Live sending gate:** Gmail and Outlook transport delivery/authentication and replies are proven. The BrokePipes-only release is deployed; product enrollment, DNS verification, clean disconnect/reconnect, a real custom-domain Outlook product quote with the correct link, scheduled website TLS promotion and its Gmail owner notice passed. Gmail product coverage, remaining failure drills, deployed account-closure cleanup, and seven-day observation remain open; see the [current canary record](docs/contractor-domains-canary-2026-09-09.md).
...
- [x] **Certificate watcher & release on deletion (`103097369`):** Added certificate status watcher and Vercel binding release on deletion. Production scheduling and a disposable provider release were verified September 9; full deployed deletion and certificate transition acceptance remain open. Provider cleanup can fail and must be observed/retried.
...
## Six-SKU release and private test evidence — 2026-09-08

**Release decision: all six original SKUs are sellable on eligible plans.** Voice launched with minute measurement and recurring allowances enabled, exhaustion enforcement disabled, and unmetered usage absorbed by LGQ under the owner's approval. Storage cap enforcement is enabled. This dated update supersedes older withholding and pending cutoff/voicemail claims below; the remaining acceptance checks stay open.

**Private evidence record:** `C:\dev\voice-live-test-evidence-20260908.md` (local operator record, intentionally outside this public repository). It contains the dated provider/application observations and test identities supporting the results below. Its final release and follow-up entries supersede earlier pending observations. Customer identities, call identifiers, and private recording details remain in that record. The public [pre-release readiness snapshot](docs/six-sku-release-readiness-2026-09-08.md) contains price/metadata verification; its earlier withholding and pending-release statements are superseded by this update. The local follow-up plan is `C:\dev\six-sku-post-launch-plan-2026-09-08.md`.

### Completed — private evidence recorded

- [x] **All six purchase paths released**: `ai_voice_flex`, `ai_voice_solo`, `ai_voice_growth`, `voice_minutes_100`, `storage_100gb`, and `office_user` are enabled for eligible plans. [Storage/office release #28](https://github.com/wideeyephoto/lets-get-quoted/pull/28), [inbound retry fix #29](https://github.com/wideeyephoto/lets-get-quoted/pull/29), [recording callback fix #31](https://github.com/wideeyephoto/lets-get-quoted/pull/31), and [voice release #30](https://github.com/wideeyephoto/lets-get-quoted/pull/30) merged. Release #30's [hosted CI](https://github.com/wideeyephoto/lets-get-quoted/actions/runs/34246266080) passed; the recorded production deployment was ready and served the application domain.
- [x] **Authenticated checkout handoff verified without payment**: the Flex AI Voice subscription, 100-minute pack, and storage add-on opened live Stripe checkout with the expected product, amount, and billing cadence. Checkouts were exited without submitting payment. Live catalog price/metadata verification covers the six SKUs; this does not claim a paid transaction for each SKU.
- [x] **Storage and office capacity checks recorded**: hosted rollback-only checks verified capacity grants, cancellation restoration, and repeated-cancellation idempotency, with no synthetic ledger rows left. Flex includes two counted seats: the owner and one office user. The authorized office invitation was accepted; workspace selection and permitted leads, clients, jobs, and job-detail reads worked. A further invitation was refused at the seat limit. These observations prove the recorded read permissions, not unrestricted office access or all write workflows.
- [x] **Conversational AI and inbound retry recovery verified live**: the caller confirmed responsive AI conversation. Repeated inbound delivery reused the admission without the previous self-concurrency refusal or callback-to-caller loop; the completed call settled once with no recorded receipt error.
- [x] **Ten-minute cutoff verified at the provider**: the controlled call ended through the provider's maximum-duration limit; application settlement recorded the capped call once. This closes the observed cutoff check, not the full concurrency or failure matrix.
- [x] **Voicemail fallback and signed recording callbacks verified live**: the final controlled fallback saved a ready, non-provisional voicemail, accepted pending/completed recording callbacks, and retained unmetered status. A media response check returned audio content through the existing trust policy; full browser playback/retention acceptance remains separate. AI answering was restored after the test, with no temporary pause left.
- [x] **Launch settings and cleanup recorded**: the final production observation verified storage caps, top-up purchases, voice measurement, and the recurring allowance worker enabled, with the voice exhaustion gate disabled. Rollback-only test rows were absent and the deployed source matched the merged release tree.

### Outstanding — retain on the launch list

- [ ] **Correct partial-balance behavior in measurement mode**: a remaining balance of one to nine minutes must not shorten calls while exhaustion enforcement is disabled. Verify admission, provider retries, call-duration instructions, and accurate absorbed-usage accounting. The September 8 private release evidence does not close this subsequently identified defect.
- [ ] **Complete paid billing lifecycle acceptance**: record authorized paid activation, recurring renewal, period-end cancellation, and applicable refund behavior for the released products, including exactly-once fulfillment and capacity/allowance changes. Existing checkout handoffs, automated tests, and rollback-only database checks are not real paid lifecycle evidence.
- [ ] **Prove successful human transfer live**: use a distinct authorized destination and verify connection, resulting call state, and settlement. Same-caller fallback and voicemail recovery do not prove a successful human transfer.
- [ ] **Reconcile a complete provider billing period before exhaustion enforcement**: compare the SignalWire invoice and provider usage with application admissions, receipts, settlements, allowance grants, and absorbed usage; resolve discrepancies before enabling the gate. Confirm the provider's actual invoice boundary rather than treating the application subscription date as that boundary.
- [ ] **Finish the remaining permission, storage, and voice operations checks**: retain scoped office write/denial checks, documented storage measurement/upload limitations, and the unproven recording/playback/retention, concurrency, failure, and out-of-order callback cases in the broader matrix below. These remain separate from the passed controlled calls. Unrelated SMS, dispatch, dedicated-number, and quote-price editing gates are unchanged by this evidence.
...
### September 8 SMS acceptance status — reconciled with this task's evidence

**21:22 UTC closeout (latest):** [Cancellation fix PR #39](https://github.com/wideeyephoto/lets-get-quoted/pull/39) merged as `c6937034b` after [CI passed](https://github.com/wideeyephoto/lets-get-quoted/actions/runs/34278045513), including 1,104 test files and build/security checks. Production deployment `dpl_2sFrc2cgEGFt121Pw5cme38pEnFw` is READY and verified on the application domain; it includes the preceding live voice fixes. The inline confirmation cancelled a fresh unanswered offer through the production UI: request `cancelled`, offer `expired`, cancellation text delivered once at 21:18:21 UTC. Brett confirmed the cancellation text and original link's closed state with no Accept button. The earlier declined offer also remains declined on its cancelled request. Job and subcontractor were archived through the UI; all six extended-session delivery tasks completed in one attempt. Nine business messages independently match provider segments to committed application units (22 total). Private identifiers remain in the local operator evidence. This closes the stated cancellation and cleanup checks and supersedes the historical pending observations below; broader suppression/recovery and customer gates remain open.

**20:29 UTC decline retest:** the delivered decline offer opened from the received link; the public page confirmed unavailability, and staff/database views recorded one declined offer with its response note. Brett also confirmed declining. Browser and handset activity overlapped, so this is one completed workflow, not two independent response tests. Cancellation is still pending at a browser confirmation dialog; no cancellation or fixture cleanup is claimed.

**20:20 UTC continuation:** subcontractor projector permission migration applied to production (`20260908201549`); service-role EXECUTE verified, browser roles excluded. The accepted offer was linked to its original delivered event, retaining accepted/claimed state. A fresh decline-test offer delivered once with automatic event-link/timestamp projection, closing the live permission-fix retest. Handset decline response and cancellation remain pending. Six earlier business sends matched SignalWire's actual segment counts to committed application reservations (17 segments total). A rollback-only hosted callback test verified duplicate-receipt reuse and late-sent non-regression of delivered/accepted state, task attempts, and accounting; zero synthetic receipts remained afterward. This is database-layer evidence, not an HTTP signature replay. Creating a second open request was correctly rejected, but its generic error page needs a usability follow-up.

**Follow-up verification:** [SMS activation gap audit](docs/sms-activation-gap-audit-2026-09-08.md) checks every proposed remaining item against saved evidence and fresh production state. It distinguishes confirmed pending work from already-passed local checks awaiting live acceptance. The clean-schema failure is a separate reproducibility issue: its named table exists in production, so this is not a demonstrated production SMS blocker. Self-service registration has implementation/staged work; current production policy activation and full lifecycle acceptance remain pending.

- [x] **Dispatch handset acceptance, single workspace**: automatic welcome, assignment and schedule notifications, ordinary reply routing, HELP acknowledgment after the approved production fix, STOP acknowledgment, queued/fresh send blocking without provider submission or usage reservation, START acknowledgment, resumed delivery, and repeated-action duplicate prevention passed.
- [x] **Subcontractor mobile offer and staff isolation**: offer link loaded without sign-in, concealed customer identity before acceptance, and accepted successfully. A Midwest-only staff session received 404 for the BrokePipes request. This does not prove cross-workspace carrier suppression.
- [x] **Supporting local checks recorded**: 295 extended-session application checks plus 23 customer registration/readiness checks passed; local PostgreSQL delivery 27/27, webhook safety 24/24, campaign-purpose 7/7, and sender registration 3/3 passed. Earlier HELP and campaign-wide STOP checks remain recorded in the private evidence. These are local results, not additional carrier acceptance.
- [ ] **Remaining SMS work**: eligible cross-workspace/sender suppression; remaining quiet-hours, retry/dead-letter, callback and failed/indeterminate usage acceptance; correct customer registration and handset acceptance; commercial dedicated-number lifecycle; staged expansion. Permission repair, live decline/cancellation, fixture archival, and nine-message carrier segment reconciliation are complete. The baseline local full-schema failure is a separate reproducibility issue; the referenced table exists in production.

**Customer testing decision:** LUX HD ART was considered only as a way to test LGQ, not as an operating installation business. No website, registration, purchase, or customer send was performed for it. A question for SignalWire about the appropriate LGQ internal-test brand/campaign setup has been drafted; submission and provider guidance are not yet confirmed. Keep customer sends gated until that path is approved and the individual number is correctly assigned. This update covers this task's SMS evidence; concurrent voice/billing work is tracked separately.

### Outstanding — shared SMS and dispatch rollout

**Execution roadmap:** [Roadmap to SMS ready — September 8](docs/sms-ready-roadmap-2026-09-08.md) records the September 8 dispatch handset session and remaining operator, staged-expansion, and customer-registration gates. Welcome, assignment, ordinary reply, HELP (after an explicitly approved production fix), STOP, blocked queued/future probes, START, and resumed schedule delivery passed for BrokePipes. Detailed evidence is in `C:\dev\sms-live-test-evidence-20260908.md`. Midwest Glass remains a test company.

- [x] **Normal Add Crew producer verified September 8**: production UI saved crew disclosure evidence and queued one automatic welcome from dispatch 8333, confirmed by delivery callback and handset. Repeat save created no duplicate. The welcome contains no link. The temporary crew and job were archived after the session.
- [ ] **Complete the new dispatch number's live reply/compliance matrix**: verify an ordinary crew reply reaches the correct workspace, HELP and its handset acknowledgment, STOP and its acknowledgment, blocked queued/future sends, and handset START/re-opt-in. Prove campaign-wide suppression across contractors and a controlled same-campaign sender fixture; a different contractor or number must not bypass STOP. Do not overwrite opt-out state to make the test pass.
  - September 8: the single-workspace handset sequence passed. The worker canceled both pre-STOP queued and post-STOP fresh probes before provider egress, with no usage reservation. Cross-workspace suppression-helper and local same-campaign tests passed; eligible second-workspace/sender acceptance remains open.
- [ ] **Prove real crew/subcontractor business workflows**: exercise an actual job assignment/schedule notification and the applicable offer/accept/decline/cancel flows through the released producers; verify company identification, consent scope, job links, correct account routing, durable results, and duplicate prevention. A welcome delivery alone does not prove these automations.
  - September 8: actual assignment and calendar Notify schedule delivery passed, including correct handset schedule and duplicate prevention. The subcontractor offer arrived, opened without sign-in, hid customer identity before acceptance, and accepted successfully. Decline, cancellation after decline, and cancellation of an unanswered offer now pass; the recipient confirmed the cancelled link cannot accept. Midwest-only staff access to the BrokePipes request returned 404. Broader cross-workspace suppression and repeated/concurrent response coverage remain open, so this aggregate row stays unchecked. Move this job alone updates the schedule without texting crew; calendar Notify sends the date.
- [x] **Apply and retest subcontractor delivery-evidence permission fix**: the narrow grant is now applied and verified in production. Existing accepted-offer evidence was repaired without resending; a fresh offer sent successfully through the UI, delivered once, and projected its event link and timestamps. Decline/cancel remain separate workflow acceptance items.
  - Extended automated evidence: 295 application checks passed, plus local PostgreSQL delivery 27/27, webhook 24/24, campaign-purpose 7/7, and sender-registration 3/3. These do not substitute for outstanding live carrier checks. The initial crew/job cleanup preceded the extended session; the new subcontractor fixture and reactivated test job require final cleanup.
...
- [ ] **Obtain correct contractor-to-customer campaign coverage**: pilot `+18103202687` remains on LGQ's support campaign, whose scope excludes that traffic. Register genuine downstream businesses using their real identity, consent collection, disclosures, and samples; verify individual number assignment and SMS capability before customer sends. BrokePipes is a test workspace, not evidence of a vetted independent business. Do not substitute the crew dispatch campaign or disturb the pilot's voice routing. See the [customer coverage finding](docs/texting-release-readiness-2026-09-05.md).
  - September 8 fresh database read: neither BrokePipes nor Midwest has a `messaging_registration_applications` row. Pilot campaign remains the support campaign. Customer booking/post-call/intake/timezone checks passed locally, followed by 23 registration/readiness checks; live customer sends remain untested pending a provider-confirmed internal-test registration path or genuine downstream business application, plus correct approval/assignment.
...
- [x] **Email reconciler scheduled execution verified (2026-09-09 to 2026-09-10).** September 9 run `e7162394-5a37-45e2-8fb3-e45c44db4f36` proved empty-inventory scheduling. September 10 run `93219051-6b33-40be-84fd-b68ce8f588d5` at 06:23:20 UTC checked the active BrokePipes binding with zero errors or orphans and advanced its `last_checked_at` to 06:23:21.089 UTC. These runs predate the September 11 16:23 UTC observation start and do not count toward its seven checks; see the [canary record](docs/contractor-domains-canary-2026-09-09.md).
