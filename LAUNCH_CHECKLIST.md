...
- [ ] **Fix the STOP → ordinary-reply compliance hole before the real handset journey**: the inbound RPC correctly refuses to route ordinary traffic while the sender/account is opted out, returning a null-account `review`; the route then sends that review through `sharedNoticeTwiml`, whose egress gate checks global/canary/purpose switches but not the recipient's sender-specific STOP preference. With an empty canary allow-list, a genuine provider callback could therefore ask SignalWire to send the courtesy notice to an opted-out number. Add a recipient-level suppression check and a cross-layer regression before exercising this path against a carrier. The focused route/migration rerun on 2026-09-03 passed 3 files / 29 tests, confirming the current behavior is unguarded rather than a failing existing test.
...
| `POST /api/sms/inbound` | **Handler**: HELP/STOP/START, shared notice, signature, synchronous-reply audit, and retry-dedupe branches; focused rerun 3 files / 29 tests | **Signed production handler**: HELP plus exact replay verified on 2026-09-03; existing production rows reconcile ordinary, STOP, and START branches | Fix recipient-level STOP suppression, then exercise HELP/STOP/blocked ordinary/START and ordinary routing from a real handset; require carrier execution and device receipt |
