...
- [ ] **Remaining production sign-off:** Release and product-verify the newly found Check Connection fix: it restarted an already completed provider verification and read pending. Resend now verifies the new binding's DNS; 59 affected tests pass for the fix. Then receive a real custom-domain product send, finish the remaining message, recovery, website certificate-promotion, and deployed deletion drills, and observe seven healthy elapsed days with seven consecutive active-domain scheduled runs. Hourly follow-ups are ACTIVE with at most one test per approved inbox daily. **The seven-day clock has not started.** Full scope and evidence are in the [canary record](docs/contractor-domains-canary-2026-09-09.md) and [email go-live list](docs/contractor-email-domain-go-live-checklist-2026-09-09.md).
...
- [ ] **Live sending gate:** Gmail and Outlook delivery, aligned SPF/DKIM, DMARC PASS, replies, and the corrected production quote link are proven. The single-workspace release is deployed and product enrollment created a pending binding. Verification of its newly published return-path DNS, a custom-domain product send, remaining failure drills, and seven-day observation remain open; see the [current canary record](docs/contractor-domains-canary-2026-09-09.md).
...
- [ ] **Complete natural renewal/cancellation after the verified initial purchase/refund journeys.** The six initial $248 journeys and the separate live LGQ connected refund are verified above. The replacement renewal cohort has an approved $426 additional cap; two of five subscriptions are paid ($74), with three pending provider approval. Actual paid renewal, effective period-end cancellation and subsequent cleanup remain open; see [current evidence](docs/evidence/live-addon-renewal-2026-09-10.json).
...
- [ ] **Email sending domains — production rollout remains gated (UPDATED 2026-09-09)**: Real Gmail receiver headers now prove SPF/DKIM alignment and DMARC PASS for `blackholeart.com`; reply routing and rejected-domain fallback were exercised. See [dated evidence](docs/contractor-domains-verification-2026-09-09.md). New recovery fixes await deployment; Outlook, deployed onboarding, further failure drills, and seven-day canary evidence remain open. Production enrollment stays off.
