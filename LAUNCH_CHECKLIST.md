...
- [ ] **Repair the canonical fresh-schema routine dependency before release**: on 2026-09-03, `npm run test:pg17:messaging-schema` failed on a disposable PostgreSQL 17.10 database with SQLSTATE `42883` because `schema.sql` reaches `public.office_can(account_id, 'leads.read')` before any `public.office_can(uuid,text)` definition is available. Restore the canonical `office_can`/capability foundation ahead of its first policy or RPC reference, then require the broad fresh-schema harness, the FK-order lint, and the messaging schema/parity checks to pass. The focused owner-field migration harness below is intentionally isolated and does not close this broader blocker.
...
- [ ] **Align every dashboard “Voice & Text” hint with the implemented rail before launch**: `FieldIntakeHint` currently advertises lead/profile updates, job address/status changes, scheduling/booking/arrival-window changes, crew assignment/labor, Quick Stop and recurring-service mutations, task completion, and instant job creation that the shared-number worker cannot perform. It also tells users that shared-line text/voice memos/calls use Voice credits, while SMS/media field intake meters `ai_intake_threads` and live AI calling requires an enabled `contractor_dedicated` number. Restrict examples to supported notes, bounded costs, adding tasks, and owner lead capture; label SMS/voice memos as AI Intake usage; gate phone-call claims on dedicated-number readiness; and hide unsupported page affordances until their atomic actions exist. The current 7-test voice-hint suite proves component/source presence only, not command handling or live calling.
