...
## Google Ads Cloud-project migration results — September 14, 2026

- [x] **Remove developer-token dependency and deploy:** [PR #88](https://github.com/wideeyephoto/lets-get-quoted/pull/88) merged as `190aebe29570bd193459e6922fbe69185afb5d1e`; production deployment READY. Managed Ads, Local Services Ads, server verification and both CLI verifiers omit the header. Legacy options are ignored. OAuth and account restrictions remain enforced. Validation: 136 focused tests across 10 files, TypeScript, production build and full CI passed.
- [x] **Confirm Cloud access and notification recipient:** Cloud project `letsgetquoted` (`126766638442`) has EXPLORER access: 2,880 daily production operations and 15,000 test operations. `hello@letsgetquoted.com` is already a project Owner and eligible for future administrative notices. No IAM grants changed.
- [x] **Verify live access without a developer token:** [PR #89](https://github.com/wideeyephoto/lets-get-quoted/pull/89) added an authenticated read-only verifier; merged as `b1c4bdd52a61d593fb1aee2815322e29e695ce41` and deployed READY. At **2026-09-14 16:11:32.226 UTC**, the exact production deployment successfully refreshed OAuth credentials and received **Google HTTP 200**, with `developerTokenHeaderSent: false`, **2 accessible accounts**, and the configured manager accessible. Validation: 28 focused tests across 3 files, TypeScript, production build and full CI passed. No campaigns or conversions were modified; production secrets stayed protected.
- [x] **Resolve custom app-domain verification route and repeat the read-only check (17:20 UTC):** The same authenticated path on `app.letsgetquoted.com` now returns **HTTP 200**, Google upstream 200, no developer-token header, 2 accessible accounts and manager access. Vercel now maps the app domain to deployment `dpl_AvAHiewcmXRG8UNobdqVh2QNFHsJ` / commit `b1c4bdd52a61d593fb1aee2815322e29e695ce41`. The earlier 404 is no longer reproducible; no domain changes were necessary in this follow-up. [Dated app-domain evidence](docs/google-ads-app-domain-live-result-2026-09-14.json).

- [x] **Promote the corrected verifier and verify the app domain (17:51 UTC):** [PR #91](https://github.com/wideeyephoto/lets-get-quoted/pull/91), merge `eebbcabab7fdef3b35534ff88fbb8a232a50b939`, passed 145 focused tests, TypeScript, full CI and Vercel preview. Production deployment `dpl_55Ktz8TMDVWjNeXczWTg5RjrVz6i` was READY but STAGED with custom-domain assignment pending. Explicit promotion succeeded; the app domain then resolved to that release and the authenticated token-free check returned HTTP 200. [Promoted-domain evidence](docs/google-ads-promoted-domain-live-result-2026-09-14.json).
- [x] **Repeat live campaign writes after migration (17:51 UTC):** OAuth, advertiser discovery/account read, budget creation, PAUSED campaign creation, PAUSED status update, campaign removal and budget removal all passed through the app domain. Billing setup was APPROVED. Test campaign never activated; no ad spend was initiated. [Write and conversion results](docs/google-ads-write-conversion-live-result-2026-09-14.json).
- [x] **Migrate offline conversion uploads to Data Manager API (18:52 UTC):** PRs #92 and #93 merged and deployed. Dedicated OAuth credentials are protected Production Secrets; Data Manager is enabled for production. The app-domain validation returned HTTP 200, `ok: true`, `productionTransportEnabled: true`, zero warnings and `conversionRecorded: false`. The former UploadClickConversions restriction and missing event-source field are resolved. [Production evidence](docs/google-data-manager-production-validation-2026-09-14.json).

- [x] **Implement Data Manager transport and enable Cloud API:** PR #92 / commit `917b694da`, 157 focused tests and full CI passed. PR #93 / commit `59a65c4a6` supplies the Google-required offline event source, with 13 focused tests and full CI passed. Both previews passed.
- [x] **Finish Data Manager authorization and production cutover:** Explicit full-scope approval received; all three dedicated credentials saved securely at 18:27 UTC. Corrected release `4cbc22a6a347e591125223088ed00a5425bcc7ae` / deployment `dpl_F27bydJqQWBnsUW56MU2vRuUeQeb` promoted and confirmed on the app domain. Token-free Ads read-only access also passed at 18:52:40 UTC after cutover.
- [ ] **First genuine conversion acceptance and attribution:** User confirmed no eligible unuploaded won job is available. Await a genuine event and its provider processing/attribution diagnostics. Validation-only success does not establish a recorded conversion.
- [ ] **Broader OAuth distribution:** Cloud Audience is In production but shows Google app-verification requirements and a 100-user cap for unapproved sensitive/restricted scopes. Owner authorization succeeded; review before extending OAuth access to more users.

Full results, deployment/CI references, retry history and scope: [Google Ads migration and live verification summary](docs/google-ads-migration-results-2026-09-14.md). Original sanitized [read-only response evidence](docs/google-ads-token-free-live-result-2026-09-14.json). Migration and production API validation are **green**. End-to-end conversion attribution awaits the first genuine eligible event; general go-live and ad-spend gates retain their existing requirements.
