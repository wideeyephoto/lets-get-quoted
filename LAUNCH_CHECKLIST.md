...
**Current status, September 15 01:47 UTC:** The seven-day canary restarted September 14 at **17:06:28.748793 UTC (1:06 PM America/New_York)** after a material email release and a verified quote-send defect/repair. **0/7 qualifying scheduled runs and zero completed days** belong to this new window; the first daily check is due September 15 at 06:23 UTC, and the earliest seven-day review is **September 21 at 17:06:28.748793 UTC (1:06 PM Eastern)**. The earlier attempt retains three successful runs and two complete healthy days; its September 18 review date is retired. The repaired production quote reached Outlook Focused Inbox with SPF/DKIM/DMARC and Microsoft authentication PASS, the correct Reply-To and a matching J-1004/$0 link. This receipt is the new baseline, not retroactive in-window volume. Both September 14 inbox allowances are used. PR #85 is already merged and deployed. PRs #87 and #90 both passed full candidate CI and exact Preview smoke validation, were explicitly approved, and are merged. Both repairs remain deployed in the customer-facing app on `b725cb6c`. Newer production-target builds now execute the website watcher on shifted 15-minute slots while their full CI is failing; their READY state does not establish a passed release or app promotion. The watcher is healthy and its code is unchanged, so the email canary start remains intact. This app/worker release split is an open release-validation issue. The subsequent permit, security, language and operator release leaves custom-domain delivery, quote sending, reconciliation and completion recording unchanged. Webhook inspection now records durable read-only audit entries instead of obsolete approval cards; it does not replay or resolve failures. The canary start is retained. PR #80's actual closure repair was rechecked with 10 passing local PostgreSQL checks; its public description is corrected with explicit approval, while full CI and rollout-document refresh remain pending. The first scheduled website run after PR #87 passed with HTTP 200 and a durable completion summary. The first website run after the latest release also has a durable successful summary. This establishes normal scheduled recording, not injected-fault recovery or pending-site promotion; those acceptance gates remain open. The verified domain and both website certificates remain healthy, with zero open notices or unresolved Resend callbacks. The October 12 fixture recovery deadline and pending acceleration approval are unchanged. Remaining lifecycle, message-path, direct-to-From mailbox and cleanup gates still block launch. Keep enrollment restricted to BrokePipes. [Full current evidence](docs/contractor-domains-canary-2026-09-09.md#september-14-quote-send-repair-and-new-window).
