...
- [ ] **Apply and retest subcontractor delivery-evidence permission fix**: live offer delivery succeeded but its staff send action returned HTTP 500 (`42501`, missing EXECUTE on `apply_subcontractor_sms_event_projection`), leaving the offer unlinked to its delivery event. A narrow service-role-only grant reproduced/fixed the failure in 27 local PostgreSQL checks; production application remains pending. Recover the existing event link without resending or regressing accepted state, then finish fresh decline/cancel tests.
...
- [ ] **Obtain correct contractor-to-customer campaign coverage**: pilot `+18103202687` remains on LGQ's support campaign, whose scope excludes that traffic. Register genuine downstream businesses using their real identity, consent collection, disclosures, and samples; verify individual number assignment and SMS capability before customer sends. BrokePipes is a test workspace, not evidence of a vetted independent business. Do not substitute the crew dispatch campaign or disturb the pilot's voice routing. See the [customer coverage finding](docs/texting-release-readiness-2026-09-05.md).
  - September 8 fresh database read: neither BrokePipes nor Midwest has a `messaging_registration_applications` row. Pilot campaign remains the support campaign. Customer booking/post-call/intake/timezone checks passed locally, followed by 23 registration/readiness checks; live customer sends remain untested pending a genuine business application and correct approval/assignment.
