...
- [ ] **Resolve custom app-domain verification route and repeat the read-only check:** `app.letsgetquoted.com/api/admin/verify-google-ads/read-only` returned **HTTP 404** at 16:09:18.486 UTC, while the exact READY production deployment passed. Domain routing/deployment cause remains unconfirmed; no domain settings changed. Keep this gate open until the same authenticated check succeeds through the app domain. This result does not close general launch, advertising-spend or write-path gates.

Full results, deployment/CI references, retry history and scope: [Google Ads migration and live verification summary](docs/google-ads-migration-results-2026-09-14.md). Sanitized [live response evidence](docs/google-ads-token-free-live-result-2026-09-14.json). September 7 write/conversion checks below remain historical evidence, not fresh post-migration write verification.
