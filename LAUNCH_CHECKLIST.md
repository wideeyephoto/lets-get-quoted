...
- [ ] **BLOCKER — migrate offline conversion uploads to Data Manager API:** The fresh validation-only request using the real configured Job Won action was rejected inside an HTTP 200 partial failure: Google says new integrations must use Data Manager and restricts UploadClickConversions to existing users. The corrected combined verifier correctly returned HTTP 207 / `ok: false`. No conversion was recorded. Enable/configure Data Manager, obtain authorization for its additional `datamanager` OAuth scope, migrate the upload transport, and pass validation plus a genuine eligible event/diagnostics check before claiming conversion acceptance or attribution. This supersedes the older September 7 conversion-green conclusion; do not treat HTTP 200 alone as conversion success.

- [x] **Implement Data Manager transport and enable Cloud API (18:24 UTC):** [PR #92](https://github.com/wideeyephoto/lets-get-quoted/pull/92), commit `917b694da`; 157 focused tests in 16 files and TypeScript passed. Preview passed; full CI build still pending. Includes protected validation-only route, destination checks, hashed identifiers and provider request-ID persistence.
- [ ] **Finish Data Manager authorization and production cutover:** Dedicated OAuth client created and client ID saved as a Production Secret. Secret/refresh-token storage is not confirmed. Automatic approval review requires explicit consent to Google's full Data Manager scope, including customer-data editing/deletion across advertising and analytics products. Approval requested. After authorization, deploy and validate through the app domain before enabling the transport; genuine-event diagnostics remain outstanding.

Full results, deployment/CI references, retry history and scope: [Google Ads migration and live verification summary](docs/google-ads-migration-results-2026-09-14.md). Original sanitized [read-only response evidence](docs/google-ads-token-free-live-result-2026-09-14.json). Overall Google Ads status remains **not fully green** because offline conversion uploads are blocked; general go-live and ad-spend gates retain their existing requirements.
