...
- [x] **Checkout idempotency, return-URL validation & lifecycle idempotency**: Added optional client `idempotencyKey` forwarding to Stripe checkout sessions; implemented `validateAdReturnUrl` and `sanitizeAdAlertPhone` to prevent open redirects, protocol-relative attacks (`//`, `/\`), and SSRF; made `pauseAdCampaign`, `resumeAdCampaign`, and `cancelAdCampaign` completely idempotent.
  - **Amended 2026-09-13:** that forwarding passed the caller's string to Stripe verbatim. Stripe scopes idempotency keys per *platform* account, so every workspace shared one flat namespace and any authenticated account could occupy a key another might use. Not session disclosure — the requests differ, so Stripe answers the second one `400` rather than returning the first one's Session — but a durable cross-tenant denial. `buildAdBudgetCheckoutIdempotencyKey` now binds the caller's string to its workspace before it reaches Stripe ([ad-billing.ts:144](src/lib/ad-billing.ts#L144)); regression `test/ad-budget-idempotency-key.test.ts`.
...
- [ ] **BEFORE MANAGED ADS GO ON SALE — subscribe the four ad-budget lifecycle events (opened 2026-09-13):** `handleAdBudgetWebhookEvent` returns early unless the event is one of seven types ([ad-billing.ts:799](src/lib/ad-billing.ts#L799)). Four of them — **`invoice.paid`, `invoice.payment_failed`, `customer.subscription.updated`, `customer.subscription.deleted`** — are **not** among the eleven subscribed on the live endpoint listed under *Live Stripe Webhook Endpoints* below. Confirmed 2026-09-13 by a read-only live API read of `we_1TuE0BGqh5LFKuTCEyt5d4jh`; the eleven events it returns match this document exactly, so the document was right and the subscription is simply short.
  - **Nothing fails when they are missing.** The events are never delivered, so `invoice.paid` never credits the ad wallet on renewal, `invoice.payment_failed` never marks a campaign `past_due`, and `customer.subscription.deleted` never runs `updateAccountAdBudgetState(… status: 'inactive', stripeSubscriptionId: null)` ([ad-billing.ts:1117](src/lib/ad-billing.ts#L1117)) — a contractor cancels in Stripe, the app never learns, and the campaign shows active indefinitely. Same silent shape as the top-up flag ordering recorded under *Whole-platform go-live gates*: money moves, no cron fails, no dead letter, nothing to notice it by.
  - **Latent today, not live.** A live subscription search for `metadata['kind']:'ad_budget'` returned zero results on 2026-09-13 — all eight live subscriptions are `lgq_purpose` `top_up` or `base_plan_subscription` — so no contractor is affected right now. This is a pre-launch gate for managed ads, not an active incident.
  - **Not a code change.** The handler already implements all four branches correctly; only the Stripe endpoint subscription is short. Do not "fix" this in the repository.
  - **PASS =** the four events are subscribed on the live endpoint, the event list under *Live Stripe Webhook Endpoints* in this document is updated to match, and one real cancellation is observed flipping a campaign to `inactive`.
...
- [x] **Standard Connect Webhook**: `https://letsgetquoted.com/api/stripe/webhook`
  ...
  - Re-confirmed unchanged 2026-09-13 by read-only live API read of `we_1TuE0BGqh5LFKuTCEyt5d4jh`.
  - **This list is four events short of what `handleAdBudgetWebhookEvent` needs.** `invoice.paid`, `invoice.payment_failed`, `customer.subscription.updated` and `customer.subscription.deleted` must be added before managed ads go on sale — see the open item under *Managed Ads Billing* above. Harmless until then (zero live `ad_budget` subscriptions as of 2026-09-13).
