...
- [ ] **Website watcher run-record recovery:** The September 14 13:00 and 13:30 requests returned HTTP 200 but their initial telemetry inserts failed, leaving no run rows. The intervening 13:15 run was recorded. Keep these two evidence gaps separate from the three failed website scans. PR #85 does not repair telemetry inserts. A separate stable-invocation-ID recovery candidate is in draft PR #87, with 89 focused tests passing on current main; full CI, hosted validation and production acceptance remain pending. No email-domain scheduled record was lost. See [recording-gap evidence](docs/contractor-domains-canary-2026-09-09.md#september-14-website-run-record-gaps).
...
- [ ] **Website watcher read-retry candidate:** One bounded retry of transient site SELECT failures preserves permanent-error reporting and avoids replaying promotion or owner emails. Six regression cases failed before the change; 74 focused tests and targeted ESLint now pass. Full CI passes 17,030 tests, 20 PostgreSQL checks, typecheck, lint and the 433-page build. The exact candidate Preview homepage renders with no captured browser warnings/errors. [PR #85](https://github.com/wideeyephoto/lets-get-quoted/pull/85) is ready for review; merge/deployment approval and production acceptance remain pending. The authenticated worker and hosted fault injection remain unverified; this public-page check does not close those gates or the upstream timeout. See [validation evidence](docs/contractor-domains-canary-2026-09-09.md#september-14-retry-ci-and-preview-smoke-check).
