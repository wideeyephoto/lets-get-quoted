...
- [x] **Local owner-event dispatch protection (first sources):** Client questions and follow-up/more-work events now save their owner notice atomically, retain immutable messages and reconcile signed delivery outcomes. Verification: 192 application tests, 93 PostgreSQL checks, lint, registry checks and clean local security advisor. See the [owner-event runbook](docs/runbooks/owner-event-notices.md). Repeated-submission request IDs, other owner events and hosted background pickup remain open.
