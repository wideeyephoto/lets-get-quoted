...
- [x] **Owner-alert failure recovery implementation and staging drill (September 10):** Added durable notices so a failed owner email remains visible after the domain is already downgraded. Unknown send outcomes require review; concurrent workers cannot blindly resend them. Passed 128 focused tests, 15 PostgreSQL checks, type checking/lint, and a rolled-back hosted staging drill. Production release and owner-notice receipt remain open; see [evidence](docs/contractor-domains-canary-2026-09-09.md).
- [ ] **Remaining production sign-off:** Finish Gmail product coverage and the remaining message, recovery, website certificate-promotion, and deployed deletion drills, then observe seven healthy elapsed days with seven consecutive active-domain scheduled runs. The disposable website DNS edit currently awaits Squarespace device verification. Hourly follow-ups are ACTIVE with at most one test per approved inbox daily. **The seven-day clock has not started.** Full scope and evidence are in the [canary record](docs/contractor-domains-canary-2026-09-09.md) and [email go-live list](docs/contractor-email-domain-go-live-checklist-2026-09-09.md).
...
- [ ] **Live sending gate:** Gmail and Outlook transport delivery/authentication and replies are proven. The BrokePipes-only release is deployed; product enrollment, DNS verification, clean disconnect/reconnect, and a real custom-domain Outlook product quote with the correct link passed. Gmail product coverage, remaining failure drills, website TLS promotion/deployed cleanup, and seven-day observation remain open; see the [current canary record](docs/contractor-domains-canary-2026-09-09.md).
