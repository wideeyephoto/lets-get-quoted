...
- [x] **AI Voice contractor-dispatch release gate passed locally (2026-09-03 at 18:51 ET)**: 13 focused files / **248 tests** passed across signed caller admission, grounding, SignalWire/SWAIG schemas, exact contractor actions, booking and second-choice recovery, booking SMS, minute usage, crew verification, and post-call settlement. The disposable PostgreSQL 17.10 harness passed **22/22** checks: the migration applies twice, both mutation/admission RPCs are service-role-only, the force-RLS action ledger has no direct grants (including service role), identical provider retries produce one mutation/feed row, caller mismatch and ambiguous/revoked identities fail closed, assigned crew can perform only an allowed action, unassigned/coworker/office-record/cross-tenant attempts are denied without partial writes, and a late feed failure rolls back atomically. Typecheck, canonical 55-migration schema mirror/order, and diff checks also passed. These gates are now included in `npm run test:prelaunch`, with the database harness exposed as `npm run test:pg17:voice-contractor-dispatch`.
- [x] **AI Voice contractor-dispatch database-first rollout completed (2026-09-03 at 18:54 ET)**: applied `20260903215831_voice_contractor_dispatch_hardening.sql` transactionally to hosted PostgreSQL 17.6 in 466 ms. The read-only hosted verifier passed all **8/8 columns**, **8/8 validated constraints**, both exact RPC grants, force-RLS, and zero direct service/browser-role ledger grants. There were zero prior action rows and zero admissions in the preceding 15 minutes. Re-run with `npm run verify:hosted:voice-contractor-dispatch` after any schema or role change.
- [ ] **Deploy and production-prove AI Voice dispatch before calling it launch-ready**: with the hosted migration now verified, deploy the exact tested app SHA, require green CI and a READY Vercel deployment, then make one controlled owner call. Correlate dedicated-number ingress → caller admission → `/api/voice/swaig` exact action → `/api/voice/receipt` settlement/history → recording callback/playback and allowance usage. Replay the same provider payload to prove no duplicate action, confirm a staff call creates no customer lead, and inspect runtime/webhook/operator-review failures before marking the canary complete.
- [ ] **Add step-up authorization before broadly enabling privileged voice writes**: normalized caller ID plus a signed provider call binds the request to the admitted number, but caller ID by itself is not possession proof. Require a short PIN/OTP, verified callback, or equivalent trusted challenge for money, status/schedule, lead, and change-order mutations; keep those tools feature-gated until the challenge, retry/lockout, recovery, and audit paths are production-tested.
...
- [ ] **Provision and verify a separate BIGFATPIPEGUYS AI Voice number before the live call (production inventory checked 2026-09-03 at 19:01 ET)**: the hosted database currently has **0** account voice numbers, **0** voice settings rows, **0** `contractor_dedicated` sender rows, and **0** `ai_voice_route_verified` events. SignalWire owns only shared `***2323`; its independent voice handler is `ai_agent` with no request/status URL into this app, while SMS correctly remains bound to `/api/sms/inbound`. Do not repurpose or reclassify that live shared SMS number. There is also no messaging registration application or database-approved number-spend policy, so a new purchase must remain blocked until the owner explicitly approves the recurring carrier charge and completes the reviewed provisioning rail. Then bind the new provider resource to the exact account, configure signed POST voice/status callbacks, activate voice settings and entitlement/allowance, enable the production feature flag only for the controlled canary, and require route-verification evidence before exposing the number as ready.
...
- [ ] **Build a separate, charge-reviewed AI Voice number provisioning rail**: the live Michigan search returned voice/fax candidates but no SMS-capable candidates, so those numbers correctly remain unavailable to the 10DLC messaging purchase flow. Add a voice-specific candidate contract requiring `voice`, its own durable spend policy/typed confirmation/idempotent purchase and reconciliation operations, exact BIGFATPIPEGUYS binding, safe release recovery, and verified `laml_webhooks` POST configuration for `/api/voice/ai` plus status callbacks. Show the exact monthly rental before purchase; [current provider pricing](https://signalwire.com/pricing) also bills AI Agent runtime at $0.16/min plus inbound 10DLC transport at $0.0066/min. Keep all provider mutations blocked until the owner explicitly authorizes the charge.
...
| `POST /api/voice/ai` | Supporting: webhook auth, exact signed caller classification, fail-closed admission/entitlement, provider disclosure, and SignalWire adapter contracts; included in the 248-test voice gate | None; feature gate/top-ups remain withheld | Direct handler test and dedicated-number live admission canary; prove caller challenge before privileged tools are broadly enabled |
...
| `POST /api/voice/receipt` | Supporting: Basic-auth, replay, authoritative admitted-caller binding, staff/customer settlement separation, in-call lead recovery, and no-duplicate-lead contracts; included in the 248-test voice gate | None | Direct handler test plus a real post-call receipt, staff-call no-customer-lead proof, and idempotent replay |
...
| `POST /api/voice/swaig` | **Handler — selected tools**: booking/permit tools and signed token/auth paths; contractor tools additionally have exact target resolution, atomic RPC mutation, canonical per-call replay, crew assignment/self-attribution rails, and 22/22 PostgreSQL 17 checks | None | Controlled exact-job note canary, replay proof, booking link/confirmation, receipt correlation, and honest failure behavior during a real call |
...
- [ ] **Complete the live voice canary only after the database-first rollout is verified and the feature is deliberately enabled and entitled**: dedicated number → `/api/voice/ai` → exact-job SWAIG note → `/api/voice/receipt` → recording callback/playback. Prove AI/recording disclosures, signed caller/admission binding, exactly one action on retry, staff-call no-customer-lead behavior, answered/unanswered fallback, emergency and post-call SMS, booking, allowance settlement, retention purge, observability, and rollback. Do not broaden privileged writes until step-up caller verification is live.
