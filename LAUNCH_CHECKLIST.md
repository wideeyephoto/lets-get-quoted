...
**Cancellation follow-up (latest):** Brett confirmed cancellation; a fresh production query verifies request `cancelled` while preserving the offer's `declined` response. All four extended-fixture SMS tasks are completed. This closes cancellation of the responded request and supersedes the pending decline/cancellation observations below; cancellation of an outstanding offer and fixture archival remain open. The native confirmation hung during browser testing. A scoped inline-confirmation improvement is committed locally on `fix/subcontractor-cancel-inline-20260908` (`664db9961`), with full typecheck and 98 focused tests passing; it is not deployed.
...
- [ ] **Remaining SMS work**: production subcontractor permission fix and existing-event link repair; live decline/cancel; extended fixture cleanup; eligible cross-workspace/sender suppression; remaining operational carrier/usage checks; correct customer registration and handset acceptance; staged expansion. The fresh full-schema check also retains its pre-existing missing `inventory_tool_custody_log` failure.
...
- [ ] **Prove real crew/subcontractor business workflows**: exercise an actual job assignment/schedule notification and the applicable offer/accept/decline/cancel flows through the released producers; verify company identification, consent scope, job links, correct account routing, durable results, and duplicate prevention. A welcome delivery alone does not prove these automations.
  - September 8: actual assignment and calendar Notify schedule delivery passed, including correct handset schedule and duplicate prevention. The subcontractor offer arrived, opened without sign-in, hid customer identity before acceptance, and accepted successfully. Midwest-only staff access to the BrokePipes request returned 404. Decline/cancel and broader cross-workspace suppression remain open. Move this job alone updates the schedule without texting crew; calendar Notify sends the date.
