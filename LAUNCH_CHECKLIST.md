...
This is the definitive production deployment and launch checklist. A checked item requires dated command output or external-system evidence. A completed audit may be checked even when it found defects; every unfulfilled requirement remains explicitly tracked in an unchecked workstream. Configuration presence alone is not runtime proof.

## Current remaining work — reconciled September 10, 2026

**Launch acceptance remains open.** The 12 workstreams below are the single current set of unchecked launch entries. They group related tests; they are not a claim that only 12 tests remain. Older entries retain detailed criteria or link to superseding evidence instead of duplicating checkboxes. Dated observations are not fresh production probes. See the [reconciliation record](docs/launch-checklist-reconciliation-2026-09-10.md) for sources and scope.

<a id="launch-r01"></a>

- [ ] **R01 — Finish paid add-on lifecycle acceptance.** Initial six-SKU purchases/refunds ($248) and the LGQ connected refund with both reversals are complete. The separately approved $426 cohort has Solo/storage fulfilled ($74); Flex/Growth/office remain unpaid pending fresh Link approval. Complete one natural renewal per subscription, then actual period-end cancellation and full refund cleanup; no third cycle. Solo/storage first renew October 9 Eastern. Actual provider failed-payment/plan-change journeys and deployed refund/debt presentation also remain open. [Payment record](docs/prelaunch-payments-verification-2026-09-09.md), [renewal evidence](docs/evidence/live-addon-renewal-2026-09-10.json).

<a id="launch-r02"></a>

- [ ] **R02 — Complete live AI Voice and metering acceptance.** Verify full Dispatch and saved-state behavior, staff/customer authorization, two-way transfer and fallback, abandonment, interruption/silence behavior, cutoff/concurrency, recording/playback/deletion and replay-safe recovery. Validate each contractor's actual receptionist configuration. Obtain seven healthy daily comparisons and reconcile the actual provider invoice period, including forwarding and rounding. Metering is recorded ON; exhaustion enforcement stays OFF unless separately chosen after review. [Voice runbook](docs/ai-voice-go-live-runbook.md) and the detailed Voice entries below retain the individual cases.

<a id="launch-r03"></a>

- [ ] **R03 — Complete SMS, field-intake and dedicated-number acceptance.** Finish normal Add Crew and job/subcontractor producers, live reply/HELP/STOP/START and recovery/usage checks, and exactly-once field-intake usage, lease and authorization drills. Obtain correct customer brand/campaign coverage and complete provider registration integration, paid-number provisioning/billing/cancellation and customer workflows. Keep crew field mutations and literal job-creation claims restricted until their specific semantics and canaries pass; expand beyond the approved canary only after lane acceptance and authorization. [Carrier readiness](docs/texting-release-readiness-2026-09-05.md).

<a id="launch-r04"></a>

- [ ] **R04 — Finish custom email and website domain sign-off.** The one-workspace enrollment and Check Connection fixes have recorded production verification. Complete a real custom-domain product send, remaining message/disconnect/reconnect/recovery drills, disposable website certificate promotion and deployed deletion, then seven elapsed healthy days and seven consecutive active-domain scheduled runs. The latest dated record has not started that clock. [Committed canary evidence](https://github.com/wideeyephoto/lets-get-quoted/blob/59949e2bd/docs/contractor-domains-canary-2026-09-09.md), [email acceptance criteria](docs/contractor-email-domain-go-live-checklist-2026-09-09.md).

<a id="launch-r05"></a>

- [ ] **R05 — Verify native admin passkeys in production.** Pin the serving application revision; complete owner enrollment, a fresh-session native assertion, backup verification, cancelled-prompt denial and recovery on the canonical origin. The owner must complete the native device ceremony. Schema and cryptographic staging checks are recorded separately. [Security record](docs/runbooks/security-recovery-2026-09-09.md).

<a id="launch-r06"></a>

- [ ] **R06 — Complete disaster recovery beyond the verified restores.** Recover hosted Auth/Storage/application services from the independently downloaded pack, retrieve the recovery key independently, and rehearse deployment/DNS and provider-account recovery/reconciliation. Staging restoration, the production crew correction, pack authentication and isolated local restoration already have evidence. Retain the user's Free/PITR-disabled posture; full-disaster recovery time remains unmeasured. [Recovery scope](docs/runbooks/security-recovery-2026-09-09.md), [offsite runbook](docs/runbooks/dr-offsite-recovery.md).

<a id="launch-r07"></a>

- [ ] **R07 — Disposition the historical failure backlog.** Reconcile every record from the September 9 baseline (31 webhook, 186 billing and four SMS failures), crediting later audited dispositions without silently deleting history. Recover actionable records through supported paths and prove no duplicate payment, credit or message effects. These counts are the baseline, not a new current count. [Alert evidence](docs/operational-alerts-verification-2026-09-09.md).

<a id="launch-r08"></a>

- [ ] **R08 — Prove independent paging during an email-provider outage.** Add and drill a delivery path independent of Resend, including a primary-mailbox fallback. A controlled outage must reach the operator within 60 minutes with recovery context and no duplicate business effects. The existing five-class alert/recovery drill is already complete. [Alert scope](docs/operational-alerts-verification-2026-09-09.md).

<a id="launch-r09"></a>

- [ ] **R09 — Finish storage and capacity-reduction boundaries.** Verify production concurrent uploads, effective cap enforcement and over-limit cancellation/refund behavior while preserving existing files and memberships. Production office/tenant access has an 83-case acceptance record; it is not still a staging-only blocker. Initial paid storage/seat benefit and refund arithmetic are also complete. [Office evidence](docs/tenant-office-verification-evidence-2026-09-09.json), [paid scope](docs/prelaunch-payments-verification-2026-09-09.md).

<a id="launch-r10"></a>

- [ ] **R10 — Audit and verify one exact release.** Reconcile the chosen checkout and actual audit range, review uncovered changes, inventory current production flags with expected/actual values and deployment attribution, and preserve migration/webhook/worker/purchase ordering. Run typecheck, the full suite, lint, isolated build, schema order/parity and applied-migration checks with individual exit codes; match the deployed SHA and complete exact-release product smoke. Old ahead/behind counts, flag counts and red CI runs are dated snapshots. [Smoke protocol](docs/runbooks/target-release-smoke-protocol.md).

<a id="launch-r11"></a>

- [ ] **R11 — Finish paid-ad landing and offer acceptance before spend.** Verify deployed offer claims and the substantiation register, buyable landing-page journeys, noindex/DSA exclusions for duplicate routes, real page speed and cross-device attribution. The false trial CTA and its source regression guards are already corrected; final-release verification still applies. Flex refill/product changes remain a separate decision below. [Claims register](docs/ftc-substantiation-register.md), [source guards](test/claims-substantiation.test.ts).

<a id="launch-r12"></a>

- [ ] **R12 — Verify contractor-lifecycle email candidates and routes.** Run the supported production dry run without sending; inspect every intended recipient, step, subject and account age. Exclude test/demo accounts, prove first-message sequence correctness and resolve every CTA against the current routes. No current run has been established by this reconciliation. [Lifecycle implementation](src/lib/contractor-lifecycle-emails.ts).

<a id="launch-decisions"></a>

**Recorded scope decisions, separate from launch-test counts:** keep the Flex monthly refill unbuilt until its product contract is resolved and any changed copy/tests agree; keep telephone quote-price editing guarded unless explicitly deferred or safely restored; re-evaluate the deliberately deferred duplicate CSS issue; determine the referral engine's current integration/release status before enabling it. These entries are neither silently completed nor authorization to turn features on. Interruption-fade feasibility remains part of R02; assignment-safe crew commands and literal job creation remain restricted under R03; attribution remains in R11.

## Dated evidence and detailed acceptance criteria

Checked entries below retain their original date and verification scope. A **Tracked in Rxx** note points to the one current open workstream; **Superseded** identifies a specific obsolete finding, not blanket launch acceptance. Later evidence takes precedence over historical wording. The September 8 whole-platform audit's statement that it supersedes older sections applies only to that audit date, not to the September 9–10 evidence above.

## Contractor domains verification — 2026-09-09

- [x] **Verification audit and recovery fixes:** Real `blackholeart.com` quote received in Gmail with aligned SPF, DKIM, and DMARC PASS; configured Gmail Reply-To exercised. A real provider rejection recovered once through the platform sender and arrived in Gmail. Fixed paused-enrollment management, unsafe provider-domain adoption, disconnect recovery, and cleanup failure reporting. [PR #64](https://github.com/wideeyephoto/lets-get-quoted/pull/64) is merged and serving production as `dpl_955shMPfprmxsadkAEui6PqeaKC9`, SHA `18a412404d`. Full CI passed 14,590 tests and the production build. See [current release evidence](docs/contractor-domains-canary-2026-09-09.md).
...
- [x] **Controlled release and callback recovery:** Owner-approved Production settings admit only BrokePipes. Deployed LGQ created its pending `blackholeart.com` binding; the required return-path DNS records are published. Another workspace's settings omit enrollment. Four authentic provider replays returned 202 and persisted quarantine; all 20 known rehearsal incident records remain retained with explicit reviewed disposition. The website watcher also checked and attached one real pending disposable domain through deployed credentials.
- **Remaining production sign-off — tracked in [R04](#launch-r04).** The released Check Connection fix reached Verified in production on September 9. A real custom-domain product send, remaining message/recovery and disconnect/reconnect drills, website certificate promotion/deletion, and seven healthy elapsed days are still required. The observation clock has not started in the latest dated record. See the [later committed canary evidence](https://github.com/wideeyephoto/lets-get-quoted/blob/59949e2bd/docs/contractor-domains-canary-2026-09-09.md).
...
- **Tracked in [R01](#launch-r01); dated detail:** **Verify natural paid renewal and effective period-end cancellation.** A separate $426 gross cap is approved for five replacement subscriptions ($213) and one natural renewal each ($213), followed by period-end cancellation and full refund cleanup. At September 10, 01:41 UTC, Solo Voice and storage are paid ($74) and fulfilled: both signed receipts are processed, storage adds 100 GB, and the normal 01:37 worker granted exactly one 100-minute Solo paid-invoice lot while preserving the manual allowance. Flex Voice, Growth Voice and office seat remain unpaid pending Link approval. Exact provider period ends are October 10 at 00:51:08 and 01:11:44 UTC (October 9 Eastern). Follow-ups now surround the October and November billing boundaries; the next check is October 9 at 7:40 p.m. Eastern. No third cycle is authorized; no natural renewal/cancellation is claimed yet. See [renewal cohort evidence](docs/evidence/live-addon-renewal-2026-09-10.json).
...
- **Production native passkey acceptance — tracked in [R05](#launch-r05).** Verify the application revision serving the canonical app, then complete owner enrollment, a fresh-session native assertion, backup verification, cancelled-prompt denial and recovery. Production schema and staging protocol checks are already recorded; native-device acceptance is not.
...
- **Tracked in [R06](#launch-r06); dated detail:** **Complete hosted, infrastructure and provider recovery:** exact downloaded-pack hosted Auth/Storage/application acceptance, independent recovery-key retrieval, deployment/DNS recovery, and provider recovery/reconciliation remain open. The isolated PostgreSQL restore is not full-disaster recovery.
...
- **Tracked in [R02](#launch-r02); dated detail:** **Voice acceptance:** The post-release handset attempt hit voicemail with `admission_unavailable` before Dispatch started. Release/retest admission retry handling and signed-path recording recovery, then complete full Dispatch and saved-state checks, automatic termination/hold recovery, voiced unanswered transfer, active voicemail/in-flight-tool boundaries, and homeowner/on-call/staff behavior. Provider-period billing reconciliation remains open; exhaustion blocking stays OFF.
- **Tracked in [R02](#launch-r02); dated detail:** **Remaining operator and recording lifecycle acceptance:** Retry an attributable failed receipt through the supported operator action and prove no duplicate effects; verify a completed provider deletion. Authorized playback of the manually recovered fallback voicemail passed at 20:57 UTC. An empty healthy queue does not close the remaining drills.
- **Tracked in [R02](#launch-r02); dated detail:** **Interruption fade:** Confirm a supported native provider fade control, then implement and verify immediate listening, cancellation of queued speech and the audible fade. No documented fade control was found; a provider question is prepared but unsent. This is separate from the still-open full-call silence acceptance.
- **Tracked in [R02](#launch-r02); dated detail:** **Per-business receptionist setup:** Validate actual services, territory, timezone/hours, capacity, staff roles and transfer/alert recipients for each real contractor. Test-workspace configuration is not customer onboarding acceptance; keep registration and customer SMS acceptance separate.
...
- **Tracked in [R03](#launch-r03); dated detail:** **Remaining customer carrier gate:** Customer launch remains blocked on obtaining approved customer brand/campaign registration, number assignment, and carrier acceptance.
...
- **Live sending acceptance — tracked in [R04](#launch-r04).** Gmail/Outlook authentication, replies, the production quote link and the one-workspace release are recorded. The later [committed canary record](https://github.com/wideeyephoto/lets-get-quoted/blob/59949e2bd/docs/contractor-domains-canary-2026-09-09.md) confirms the replacement binding reached Verified; its remaining product-send and lifecycle requirements still apply.
...
- **Superseded — add-on refund rollout completed.** PR #56 is in production; the refund, paid Voice and duplicate-delivery migrations are applied, four refund events are configured, and scheduled workers processed all six real refunds. PR #66 and actual duplicate/stale HTTP-200 replays are recorded in the [payment report](docs/prelaunch-payments-verification-2026-09-09.md). This is dated payment-release evidence; final whole-platform verification remains in [R10](#launch-r10).
- **Natural renewal/cancellation — tracked once in [R01](#launch-r01).** The initial six $248 journeys are complete. The separately approved $426 cohort has two fulfilled subscriptions ($74), with three unpaid; actual renewal, effective cancellation and refund cleanup remain open. See [cohort evidence](docs/evidence/live-addon-renewal-2026-09-10.json).
- **Tracked in [R01](#launch-r01); dated detail:** **Remaining provider billing journeys.** September 9 local checks now cover all six initial unpaid/failed/expired states, out-of-order success, refund recovery and base-plan-change attribution, with gated customer refund/debt copy prepared. Actual provider failed-payment/plan-change journeys and deployed customer presentation still require acceptance; see the [current report](docs/prelaunch-payments-verification-2026-09-09.md).
- **Tracked in [R02](#launch-r02); dated detail:** **Complete human transfer acceptance.** Prove two-way audio, busy/no-answer fallback, caller abandonment, playable recovery voicemail and correct separation of AI/forwarding time with ready controlled participants. One-way audio receipt is partial evidence only.
- **Office acceptance recorded; storage boundaries remain in [R09](#launch-r09).** The [September 9 production suite](docs/tenant-office-verification-evidence-2026-09-09.json) records 83 passing tenant/office cases, superseding the earlier staging-only access finding. Concurrent production uploads and over-limit capacity reduction with existing files/memberships retained still require evidence. Final release changes remain subject to [R10](#launch-r10).
- **Tracked in [R02](#launch-r02); dated detail:** **Daily and full provider-period reconciliation, monitoring and separate enforcement decision.** Establish at least seven healthy daily comparisons and reconcile the actual SignalWire invoice period, including forwarding, rounding and absorbed usage. September 23 remains a checkpoint until the provider period is verified. Exhaustion blocking stays OFF unless Brett later explicitly chooses enforcement after reviewing the evidence.
...
- **Release checkout reconciliation — tracked in [R10](#launch-r10).** The September 8 ahead/behind counts and undeployed-commit statements described that checkout only. They are not the current release inventory. Record the chosen candidate, integration state and deployed revision when executing the final gates; do not repeat the old counts as current facts.
- **Changes since the audit frontier — tracked in [R10](#launch-r10).** Recompute the actual range from the [September 1 audit](docs/live-integrations-e2e-audit-2026-09-01.md), credit the later dated verifications, and audit uncovered changes. The old 380-commit count is a September 8 measurement.
- **Exact-release checks — tracked in [R10](#launch-r10).** Record independent exit codes for typecheck, the full test suite, lint, an isolated production build, schema order, messaging-schema parity and applied migrations, then match the deployed SHA and finish product smoke. Later payment/domain CI supersedes the old September 8 red-run snapshot but does not certify every later release.

### P0 — gates the first paying stranger

- **Superseded — all six live Prices and initial checkouts verified.** The [September 8 provider record](docs/six-sku-release-readiness-2026-09-08.md) documents all six amounts, cadences and catalog metadata. The [September 9 live journeys](docs/evidence/live-addon-lifecycle-2026-09-09.json) prove all six actually charged and fulfilled. Catalog changes require fresh verification under [R10](#launch-r10); renewal acceptance remains in [R01](#launch-r01).
- **Superseded — voice metering is recorded ON, enforcement OFF.** [Provider settings and a settled call](docs/six-sku-release-readiness-2026-09-08.md) establish the meter and allowance worker were enabled. Full billing-period reconciliation remains in [R02](#launch-r02), and current-release flag reconciliation in [R10](#launch-r10). No automatic enforcement date is authorized.
...
- **Production feature flags — tracked in [R10](#launch-r10).** Recompute the current inventory and record expected/actual values and the deployment that uses them. The 67-versus-12 count is historical. Paid add-on grants/refunds prove those exercised paths operated; they do not verify every flag. Preserve webhook/worker-before-purchase ordering.
- **Full recovery — tracked in [R06](#launch-r06).** Staging Auth/Storage/application acceptance and the downloaded-pack local restore are recorded separately. The crew-completion correction is [verified in production](docs/runbooks/evidence/dr-production-migration-2026-09-09.json). Hosted recovery from that downloaded pack, independent key retrieval, infrastructure and provider recovery remain open. PITR stays disabled by the recorded keep-Free decision.
...
- **Email-domain lifecycle — tracked in [R04](#launch-r04).** Production enrollment is enabled for BrokePipes only, and the replacement binding reached Verified. Outlook/reply and quote-link evidence also exists. Product sends from that binding, remaining lifecycle drills and seven healthy elapsed days remain open; see the [later committed record](https://github.com/wideeyephoto/lets-get-quoted/blob/59949e2bd/docs/contractor-domains-canary-2026-09-09.md).
- **Website-domain lifecycle — tracked in [R04](#launch-r04).** Valid live-site TLS, scheduled watcher execution and attachment of one real pending disposable domain are recorded. Certificate-ready promotion, owner notification and deletion through the deployed application remain open. Preserve the active website; see the [fixture record](https://github.com/wideeyephoto/lets-get-quoted/blob/59949e2bd/docs/contractor-domains-canary-2026-09-09.md).
- **Tracked in [R03](#launch-r03); dated detail:** **10DLC contractor-to-customer coverage gates the dedicated-number SKU (UPDATED 2026-09-09)**: Verified fail-closed customer sender and campaign scope (`9bd5a672e`), proved dispatch cross-workspace STOP/START protection using real handset keywords, verified deferred queue & dead-letter recovery components (177 app tests, 122 disposable Postgres checks). Implemented 30-task send preview plan (`004cd78d2`), JSON delivery callbacks (`df328582f`), and subcontractor cancellation inline confirmation dialog (PR #39 `c6937034b`). Customer carrier registration and live customer matrix remain open.
- **Tracked in [R02](#launch-r02); dated detail:** **AI Voice — canaries are not the matrix (UPDATED 2026-09-09)**: Merged 14 production voice PRs: PR #29 (retry admission preflight counting & self-forwarding fix), PR #31 (stable signed callback URLs & diagnostic candidate booleans), PR #35 (receipt retry bounds, abandoned receipt recovery, fallback duration limits), PR #36 (10-min call limits in measurement mode & confirmed transfer history), PR #37 (customer registration guard before SMS egress), PR #38 (dispatch write contract restoration), PR #40 (operational exception surfacing), PR #41 (end answered transfers without voicemail), PR #42 (transfer recipient announcement delay), PR #43 (admission error timeout bounding), PR #44 (pending receipt exposure & operator recovery), PR #45 (spoken references & note readback fidelity), PR #48 (response delay reduction & timing diagnostics), and PR #49 (call opening polish & note draft distinction). Live multi-party carrier matrix and billing period reconciliation remain open.
- **Paid-ad readiness — tracked in [R11](#launch-r11).** The false trial CTA was removed, and [source guards](test/claims-substantiation.test.ts) prohibit free-trial/trial-period claims. Landing-page indexing, conversion, speed, attribution and offer-register acceptance remain open; source changes are not deployed-page verification.
- **Contractor-lifecycle dry run — tracked in [R12](#launch-r12).** Inspect current production candidates without sending: recipient, step, subject, account age and trigger order. Confirm test/demo accounts are excluded, no first message starts mid-sequence, and every CTA resolves. The September 8 reference to the next 14:00 UTC run is historical.
...
- **Tracked in [recorded scope decisions](#launch-decisions); dated detail:** **Dashboard ships both stylesheets** — 375KB of redundant CSS, deferred on purpose because a Next 14 `not-found` bug blocked the fix. The tree is on Next 15 now; re-test whether the blocker still exists.
- **Superseded — the Voice ledger has a recorded consumer.** [September 8 evidence](docs/six-sku-release-readiness-2026-09-08.md) records a committed two-minute reservation and settled call; September 9 verifies paid grants and refund reversals. This does not prove every credit resource or exhaustion path. Remaining Voice reconciliation is in [R02](#launch-r02).
- **Tracked in [recorded scope decisions](#launch-decisions); dated detail:** **Referral engine is unmerged** on `agent/referral-engine-port`. Nothing depends on it, but the **merge** is what turns referrals on; leaving it stranded loses the work.
- **Tracked in [R11](#launch-r11); dated detail:** **Size the cross-device attribution gap before concluding a campaign failed** — signup conversion fires at `/welcome`, reachable only by clicking an emailed magic link, so desktop-request/phone-open converts with no ad-click context. This cannot be fixed client-side; size it from server-side events.
- **Tracked in [recorded scope decisions](#launch-decisions); dated detail:** **Do not build the Flex monthly refill yet** — adversarial review returned *broken* on two of three lenses with seven blockers, including a migration that aborts on production while its PG17 harness certifies the opposite. The product fork is unresolved: top-up-to-N grants `greatest(0, target − available)` and therefore gives a dormant account **nothing**, since it still holds its untouched 50-credit starter balance. If a refill ever ships, `test/pricing-plans.test.ts:173-179` hard-pins three strings that become false in the same commit — rewrite those guards, do not delete them.

### Orderings where the wrong sequence is what causes the harm

- **Tracked in [R10](#launch-r10); dated detail:** **Follow the recorded orderings.** Each has an incident behind it. (1) Top-up webhook + projection worker flags ON, **then** the purchase flag — reversed, the first stranger is charged and never credited with nothing failing. (2) On a catalog version bump, widen the EVIDENCE readers **then** MOVE the CURRENTNESS rows — skipping the second half stopped the only paid workspace collecting money. (3) Migration **before** the deploy that reads the column, never after. (4) The cancellation flag must follow the billing webhook, and two paths must **stay** ungated or a deleted account keeps billing. (5) Fix the alert channel **before** the alert drill. (6) Freeze the SHA **before** any gate, or you certify a tree that will never deploy.
...
<a id="0-current-audit-decision-2026-08-31"></a>

## 0. Historical Audit Decision (2026-08-31)

**Historical August 31 decision: NOT READY.** The release-gate description below belongs to that snapshot. Current remaining acceptance is tracked in R01–R12 above.
...
- **Recovery scope — tracked in [R06](#launch-r06).** The earlier claim that only capture was verified is obsolete. Staging restore, production correction, independent pack download and isolated downloaded-pack restoration have dated evidence. Hosted recovery from that pack, independent key retrieval and infrastructure/provider recovery remain unverified. See [current recovery scope](docs/runbooks/security-recovery-2026-09-09.md).
...
- **Tracked in [R02](#launch-r02); dated detail:** **Complete the live AI Voice matrix after the successful customer canary (canary evidence: 2026-09-04 at 19:41 UTC / 15:41 ET; completion corrected 2026-09-06)**: the customer canary and RLS repair below are completed evidence, not a pass for every remaining acceptance requirement.
...
- [x] **Email reconciler scheduled execution verified (2026-09-09).** Production run `e7162394-5a37-45e2-8fb3-e45c44db4f36` at 06:23:20 UTC succeeded with checked=0/errors=0. This proves scheduling against empty inventory; active-domain reconciliation remains a canary gate.
- **Superseded — one-workspace email-domain rollout is enabled.** The [committed production canary record](https://github.com/wideeyephoto/lets-get-quoted/blob/59949e2bd/docs/contractor-domains-canary-2026-09-09.md) confirms BrokePipes enrollment and a Verified binding. Broad enrollment and final lifecycle sign-off remain gated by [R04](#launch-r04); do not enable additional workspaces from the old flag instruction.
...
- **Tracked in [R06](#launch-r06); dated detail:** **Backup, PITR & Restore Drill (sign-off withdrawn 2026-09-09)**:
...
- **Tracked in [R06](#launch-r06); dated detail:** **Disaster Recovery & Supabase PITR Drill (sign-off withdrawn 2026-09-09)**:
  - PITR remains disabled by the user’s keep-Free decision. Offsite backups target 12 hours while this PC/Drive are available; full-disaster RTO remains unestablished.
  - The encrypted capture was restored into staging. Database/Auth/Storage acceptance passes after a corrective migration, including 35/35 real RLS tests. The verified migration is applied to production. Twice-daily encrypted Drive backups and user-confirmed Dashlane escrow are established; independent cloud-download authentication passes; provider and infrastructure recovery remain unverified. See [offsite recovery](docs/runbooks/dr-offsite-recovery.md).
  - The [dated drill record](docs/runbooks/dr-drill-record-2026-09-09.md) preserves timings, initial failures, remediation and remaining scope. The broader sign-off stays open.
