...
- [x] **Local partial provider-timeout patch**: explicit timeouts were added to named SignalWire, AI, Maps/Geocoding, QuickBooks, and other fetch paths.
- [ ] **Third-Party Failure Matrix**: finish the egress inventory—known OpenAI public-lead classification, QuickBooks company lookup, and photo-proxy paths remain outside the claimed coverage—then inject timeout, DNS failure, 429, 5xx, malformed response, delayed success, duplicate, and out-of-order callbacks for every provider.
