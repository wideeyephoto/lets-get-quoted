-- Only verified refund writers rotate this marker. Existing rows are not backfilled.
alter table public.payments add column refund_notice_event_id uuid;
alter table public.owner_event_notices drop constraint owner_event_notices_source_type_check;
alter table public.owner_event_notices add constraint owner_event_notices_source_type_check check(source_type in ('job_feed','messaging_registration_event','change_order','warranty_claim','review_feedback_request','quick_stop','payment_refund'));
...
create function public.record_payment_refund_owner_notice() returns trigger
...
begin
  if new.id is distinct from old.id or new.account_id is distinct from old.account_id
    ...
    or (to_jsonb(new)?'charge_model' and coalesce(to_jsonb(new)->>'charge_model','')<>'destination')
    or new.refunded_amount is null or new.refunded_amount<=coalesce(old.refunded_amount,0)
    or new.refunded_amount>new.amount or new.amount::text='NaN' then
    raise exception 'Invalid refund notice transition';
  end if;
  insert into public.owner_event_notices(account_id,source_type,source_id,event_kind,source_payload)
    values(new.account_id,'payment_refund',new.refund_notice_event_id,'payment_refund_recorded',jsonb_build_object(
      'title','Payment refund recorded',
      'body','A refund of $'||to_char(new.refunded_amount-coalesce(old.refunded_amount,0),'FM999999999990.00')||' was recorded for this payment. Total refunds recorded: $'||to_char(new.refunded_amount,'FM999999999990.00')||'. Check the payment for details.',
      'payment_id',new.id,'payment_intent',new.stripe_payment_intent,'payment_amount',new.amount,
      'refund_amount',new.refunded_amount-coalesce(old.refunded_amount,0),'refunded_total',new.refunded_amount));
  return new;
end $$;
revoke all on function public.record_payment_refund_owner_notice() from public,anon,authenticated;
create trigger record_payment_refund_owner_notice after update on public.payments
  for each row when (new.refund_notice_event_id is not null and new.refund_notice_event_id is distinct from old.refund_notice_event_id)
  execute function public.record_payment_refund_owner_notice();
