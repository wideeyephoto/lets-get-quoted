-- Operator alert coverage for settlements resolved terminally as failed / no_stripe_customer.
--
-- Adds workspace_overage_settlements where state = 'failed' to scan_operational_failures().
-- This ensures terminal failures (e.g. no_stripe_customer on unbillable Flex workspaces,
-- or card refusals that Stripe confirms) do not fail silently.
...
begin
  ...
  with signals as (
    ...
    select 'settlement:'||s.id as source_key, 'billing' as category, s.id::text as reference,
      ...
      'Overage settlement '||s.state||'; account='||s.account_id||'; error='||coalesce(s.last_error, 'unknown')||'; chargeable_cents='||s.chargeable_cents as detail,
      'Inspect the overage settlement and account billing subscription. For no_stripe_customer, attach a valid payment method or Stripe customer. For failed, resolve terminal billing error.', '/admin/billing-operations' as admin_path
