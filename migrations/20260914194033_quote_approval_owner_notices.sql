-- Only new marked client-link approval events enqueue. Existing events are not backfilled.
...
create function public.record_quote_approval_owner_notice() returns trigger
language plpgsql security definer set search_path='' as $$
begin
  if new.source_table is distinct from 'jobs' or new.source_id is distinct from new.job_id
    or new.meta->>'acceptance_source' is distinct from 'client_link' or new.visibility is distinct from 'client' then
    raise exception 'Invalid quote approval notice source';
  end if;
  perform 1 from public.jobs j where j.id=new.job_id and j.account_id=new.account_id
    and coalesce(j.quoted_amount,0)=coalesce(new.amount,0) for share;
  if not found then raise exception 'Quote approval source changed'; end if;
  insert into public.owner_event_notices(account_id,source_type,source_id,event_kind,source_payload)
    values(new.account_id,'job_feed',new.id,'quote_approved',jsonb_build_object(
      'title','Quote approval recorded','body',coalesce(new.body,''),'job_id',new.job_id,
      'quote_amount',coalesce(new.amount,0),'approval_title',new.title));
  return new;
end $$;
revoke all on function public.record_quote_approval_owner_notice() from public,anon,authenticated;
create trigger record_quote_approval_owner_notice after insert on public.job_feed
  for each row when (new.kind='quote_approved' and new.meta->>'owner_email_notice'='quote_approval_v1')
  execute function public.record_quote_approval_owner_notice();
