-- Drain legacy offer-expiration email producers before applying this trigger.
...
create function public.record_quick_stop_expiry_notice() returns trigger
language plpgsql security definer set search_path='' as $$
begin
  if new.id is distinct from old.id or new.account_id is distinct from old.account_id
    or new.payment_deadline_at is null or new.payment_deadline_at>clock_timestamp() then raise exception 'Quick Stop payment window has not expired'; end if;
  insert into public.owner_event_notices(account_id,source_type,source_id,event_kind,source_payload)
    values(new.account_id,'quick_stop',new.id,'quick_stop_expired',jsonb_build_object('title','Quick Stop payment window expired',
      'body',new.client_name||E'\nThe payment window for this Quick Stop offer has expired. Open Quick Stops to review the request and payment status before making another offer.',
      'client_name',new.client_name,'payment_id',new.payment_id,'payment_deadline',extract(epoch from new.payment_deadline_at)::text));
  return new;
end $$;
revoke all on function public.record_quick_stop_expiry_notice() from public,anon,authenticated;
create trigger record_quick_stop_expiry_notice after update on public.extra_stop_requests
  for each row when (old.status='awaiting_customer_payment' and new.status='offer_expired') execute function public.record_quick_stop_expiry_notice();
