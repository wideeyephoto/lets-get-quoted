...
create function public.record_margin_owner_notice(p_account_id uuid,p_job_id uuid,p_revenue numeric,p_total_cost numeric,p_floor_pct numeric,p_title text,p_body text)
returns jsonb language plpgsql security invoker set search_path='' as $$
declare snapshot jsonb; event_id uuid:=gen_random_uuid(); notice_saved boolean;
begin
  if nullif(btrim(p_title),'') is null or nullif(btrim(p_body),'') is null then raise exception 'Invalid margin warning'; end if;
  perform pg_advisory_xact_lock(hashtextextended('margin-warning:'||p_account_id::text||':'||p_job_id::text,0));
  snapshot:=public.current_margin_warning(p_account_id,p_job_id);
  if snapshot is null or snapshot->>'warning'<>'true' or (snapshot->>'revenue')::numeric is distinct from round(p_revenue,2)
    or (snapshot->>'total_cost')::numeric is distinct from round(p_total_cost,2) or (snapshot->>'floor_pct')::numeric is distinct from p_floor_pct then
    raise exception 'Margin evidence changed'; end if;
  insert into public.job_feed(id,account_id,job_id,kind,title,body,visibility,author,amount,published_at)
    values(event_id,p_account_id,p_job_id,'margin_alert',p_title,p_body,'internal','Margin Sentinel',p_revenue-p_total_cost,clock_timestamp());
  notice_saved:=not exists(select 1 from public.owner_event_notices where account_id=p_account_id and event_kind='margin_alert'
    and source_payload->>'job_id'=p_job_id::text and created_at>clock_timestamp()-interval '4 hours');
  if notice_saved then
    insert into public.owner_event_notices(account_id,source_type,source_id,event_kind,source_payload)
      values(p_account_id,'job_feed',event_id,'margin_alert',snapshot||jsonb_build_object('job_id',p_job_id,'title',p_title,'body',p_body));
  end if;
  return jsonb_build_object('feed_id',event_id,'notice_saved',notice_saved);
end $$;
revoke all on function public.record_margin_owner_notice(uuid,uuid,numeric,numeric,numeric,text,text) from public,anon,authenticated;
grant execute on function public.record_margin_owner_notice(uuid,uuid,numeric,numeric,numeric,text,text) to service_role;
create index owner_margin_notice_cooldown_idx on public.owner_event_notices(account_id,(source_payload->>'job_id'),created_at desc) where event_kind='margin_alert';
