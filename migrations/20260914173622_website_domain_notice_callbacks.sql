...
alter table public.website_domain_connection_notices
...
create function public.confirm_website_domain_connection_notice(
...
declare n public.website_domain_connection_notices; s public.website_domain_notice_snapshots;
...
begin
  ...
  select * into n from public.website_domain_connection_notices where id=p_id for update;
  if not found then return 'missing'; end if;
  if n.account_id is distinct from p_account_id then return 'conflict'; end if;
  select * into s from public.website_domain_notice_snapshots where notice_id=p_id;
  ...
  update public.website_domain_connection_notices set provider_id=p_provider_id,
  ...
  update public.website_domain_connection_notices set callback_status=p_status,
  ...
  if p_status='delivered' then
    ...
    update public.website_domain_connection_notices set state='manual_review',last_error='delivery_'||p_status where id=p_id;
    ...
    update public.website_domain_connection_notices set state='accepted' where id=p_id;
  end if;
  return 'confirmed';
exception when unique_violation then return 'conflict';
end $$;
revoke all on function public.confirm_website_domain_connection_notice(uuid,uuid,text,text,text,timestamptz,text) from public,anon,authenticated;
grant execute on function public.confirm_website_domain_connection_notice(uuid,uuid,text,text,text,timestamptz,text) to service_role;

create or replace function public.finish_website_domain_connection_notice(
...
begin
  ...
  if p_provider_id is not null then
    if length(btrim(p_provider_id)) not between 1 and 256 or p_provider_id<>btrim(p_provider_id)
      or not exists(select 1 from public.website_domain_notice_snapshots where notice_id=p_id) then return false; end if;
    update public.website_domain_connection_notices set state='accepted',provider_id=p_provider_id,
