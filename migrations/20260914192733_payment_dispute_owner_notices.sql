-- Verified current dispute writers only; no historical notices are backfilled.
alter table public.payments add column dispute_notice_event_id uuid;
alter table public.owner_event_notices drop constraint owner_event_notices_source_type_check;
alter table public.owner_event_notices add constraint owner_event_notices_source_type_check check(source_type in ('job_feed','messaging_registration_event','change_order','warranty_claim','review_feedback_request','quick_stop','payment_refund','account_connect','payment_dispute'));
...
create function public.record_payment_dispute_owner_notice() returns trigger
language plpgsql security definer set search_path='' as $$
declare kind text;
begin
  if new.id is distinct from old.id or new.account_id is distinct from old.account_id
    or new.stripe_payment_intent is distinct from old.stripe_payment_intent or new.amount is distinct from old.amount
    or new.stripe_payment_intent is null or new.stripe_dispute_id is null
    or (old.stripe_dispute_id is not null and old.stripe_dispute_id is distinct from new.stripe_dispute_id)
    or old.status not in ('paid','disputed') or old.dispute_status in ('won','lost')
    or (to_jsonb(new)?'charge_model' and coalesce(to_jsonb(new)->>'charge_model','')<>'destination') then
    raise exception 'Invalid dispute notice transition';
  end if;
  if new.status='paid' and new.dispute_status='won' then return new; end if;
  if new.status='disputed' and new.dispute_status in ('needs_response','under_review') then
    if old.status='disputed' and old.stripe_dispute_id=new.stripe_dispute_id then raise exception 'Repeated dispute opening'; end if;
    kind:='payment_dispute_opened';
  elsif new.status='refunded' and new.dispute_status='lost' then kind:='payment_dispute_lost';
  else raise exception 'Invalid dispute notice outcome'; end if;
  insert into public.owner_event_notices(account_id,source_type,source_id,event_kind,source_payload)
    values(new.account_id,'payment_dispute',new.dispute_notice_event_id,kind,jsonb_build_object(
      'title',case when kind='payment_dispute_opened' then 'A payment dispute is open' else 'Payment dispute outcome recorded' end,
      'body',case when kind='payment_dispute_opened' then 'A payment dispute is open. Review its current status and any evidence deadline in Stripe.'
        else 'Stripe reports that the dispute was resolved in the customer''s favor. Review the payment and invoice records.' end,
      'payment_id',new.id,'payment_intent',new.stripe_payment_intent,'payment_amount',new.amount,'dispute_id',new.stripe_dispute_id));
  return new;
end $$;
revoke all on function public.record_payment_dispute_owner_notice() from public,anon,authenticated;
create trigger record_payment_dispute_owner_notice after update on public.payments
  for each row when (new.dispute_notice_event_id is not null and new.dispute_notice_event_id is distinct from old.dispute_notice_event_id)
  execute function public.record_payment_dispute_owner_notice();
