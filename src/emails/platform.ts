...
export function renderPlatformEmail(input: PlatformEmailContent, recipient?: Partial<PlatformCampaignRecipient>, unsubscribeScope?: 'platform'): string {
  const replyTo = input.replyTo?.trim() || 'hello@letsgetquoted.com';
  const unsubscribeUrl = buildUnsubscribePageUrl(unsubscribeScope || recipient?.accountId || 'platform', recipient?.email || 'contractor@example.com');
  ...
  return renderBrandedEmail({
    ...
    footerHtml: `<p style="margin:12px 0 0;color:#64748b;font-size:12px;line-height:1.6"><a href="${escapeHtml(unsubscribeUrl)}" style="color:#475569;text-decoration:underline">${unsubscribeScope === 'platform' ? 'Unsubscribe from platform updates and announcements' : 'Unsubscribe from onboarding tips and platform announcements'}</a></p>`,
  });
}

export function renderPlatformEmailText(input: PlatformEmailContent, recipient?: Partial<PlatformCampaignRecipient>, unsubscribeScope?: 'platform'): string {
  const text = [input.heading, input.body, input.ctaLabel && input.ctaUrl ? `${input.ctaLabel}: ${input.ctaUrl}` : '',
    ...
    `Unsubscribe: ${buildUnsubscribePageUrl(unsubscribeScope || recipient?.accountId || 'platform', recipient?.email || 'contractor@example.com')}`,
  ].filter(Boolean).join('\n\n');
