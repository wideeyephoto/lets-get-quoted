...
function backTo(id: string, query: string): never {
  redirect(`/admin/extra-stops/${id}?${query}`);
}

// Manual refund on an Extra Stop's payment. Blank amount = refund the full
...
export async function adminRefundExtraStopAction(requestId: string, formData: FormData) {
  ...
  try {
    ...
    console.error('Admin Extra Stop refund failed:', error instanceof Error ? error.message : error);
  ...
  await logExtraStopEvent(admin, req.account_id, requestId, { actor: 'system', meta: { adminRefund: true, refundedTotalCents, by: adminEmail } });
  ...
  revalidatePath(`/admin/extra-stops/${requestId}`);
...
export async function adminResolveExtraStopAction(requestId: string, formData: FormData) {
  const { admin, adminEmail } = await requireAdmin();
  ...
  if (outcome === 'no_show' || outcome === 'contractor_cancel') {
    await resolveExtraStopCancellation(admin, req.account_id, requestId, { kind: outcome === 'no_show' ? 'no_show' : 'contractor_cancel', reason });
  } else if (outcome === 'completed') {
    await admin.from('extra_stop_requests').update({ status: 'completed', completed_at: nowIso, updated_at: nowIso }).eq('id', requestId);
    await logExtraStopEvent(admin, req.account_id, requestId, { actor: 'system', from: req.status, to: 'completed', meta: { adminForced: true, by: adminEmail } });
  } else if (outcome === 'disputed') {
    await admin.from('extra_stop_requests').update({ status: 'disputed', updated_at: nowIso }).eq('id', requestId);
    await logExtraStopEvent(admin, req.account_id, requestId, { actor: 'system', from: req.status, to: 'disputed', meta: { adminForced: true, reason, by: adminEmail } });
  } else {
