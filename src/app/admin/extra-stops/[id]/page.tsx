...
export default async function AdminExtraStopDetailPage({
  params,
  searchParams,
}: {
  params: { id: string };
  ...
  const detail = await getExtraStopAdminDetail(admin, params.id);
  if (!detail) notFound();

  const r = detail.request;
  const status = r.status as ExtraStopStatus;
  const canRefund = Boolean(r.payment_id && r.paid_at && (r.refund_cents ?? 0) < (r.fee_cents ?? 0));

  return (
    <>
      <Link href="/admin/extra-stops" className={styles.backLink}>← Extra Stops</Link>
      ...
      <header className={styles.pageHead}>
        ...
        <div className={styles.actionRow} style={{ marginTop: '.6rem' }}>
          <span className={`${styles.pill} ${styles.accent}`}>{EXTRA_STOP_STATUS_LABEL[status] ?? status}</span>
      ...
      <div className={styles.detailGrid}>
        ...
        <div>
          <ExtraStopAdminActions requestId={r.id} canRefund={canRefund} feeLabel={money(r.fee_cents)} />
        </div>
      </div>
    </>
  );
