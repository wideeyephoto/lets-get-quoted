...
function statusPill(status: string) {
  ...
  return <span className={`${styles.pill} ${cls}`}>{EXTRA_STOP_STATUS_LABEL[status as ExtraStopStatus] ?? status}</span>;
}
...
export default async function AdminExtraStopsPage({ searchParams }: { searchParams: { f?: string } }) {
  const { admin } = await requireAdmin();
  const active = FILTERS.find((f) => f.key === searchParams.f) ?? FILTERS[0];
  const rows = await listExtraStopRequestsForAdmin(admin, { statuses: active.statuses, limit: 150 });

  return (
    <>
      <header className={styles.pageHead}>
        <p className={styles.eyebrow}>Governance</p>
        <h1 className={styles.title}>Extra Stops</h1>
        <p className={styles.lead}>Every Extra Stop across all accounts. Open a request to see its full timeline and issue refunds or resolve disputes.</p>
      </header>

      <div className={styles.filterTabs}>
        {FILTERS.map((f) => (
          <Link key={f.key} href={`/admin/extra-stops?f=${f.key}`} className={`${styles.filterTab} ${f.key === active.key ? styles.on : ''}`}>
            {f.label}
          </Link>
        ))}
      </div>
      ...
      <section className={styles.panel}>
        {rows.length === 0 ? (
          <p className={styles.emptyState}>No Extra Stops in this view.</p>
        ) : (
