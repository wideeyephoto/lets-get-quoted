...
// this page. Removing a tag was destructuring adminEmail and then using it for
...
export async function addAccountTagAction(accountId: string, formData: FormData) {
  ...
  await logAdminAction(admin, adminEmail, { action: 'account_tag_add', accountId, targetType: 'account', targetId: accountId, meta: { tag } });
  revalidatePath(`/admin/accounts/${accountId}`);
...
export async function removeAccountTagAction(accountId: string, formData: FormData) {
  ...
  if (tagId) {
    ...
    await logAdminAction(admin, adminEmail, { action: 'account_tag_remove', accountId, targetType: 'account', targetId: accountId, meta: { tag_id: tagId } });
  }
  revalidatePath(`/admin/accounts/${accountId}`);
