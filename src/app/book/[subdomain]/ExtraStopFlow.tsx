...
// Self-contained Extra Stop path for the public Book page. Runs its OWN intake
// (that IS the AI intake for Extra Stop): structured questions + photos, a live
...
export default function ExtraStopFlow({
  subdomain,
  siteId,
  businessName,
  days,
  ...
  async function checkEligibility() {
    ...
    setVerdict(null);
    try {
      const res = await fetch('/api/public/leads/extra-stop-qualify', {
        method: 'POST',
  ...
  async function submit() {
    ...
    try {
      ...
      const result = await submitExtraStopRequestAction(fd);
      if (result.ok) {
        setDone(true);
    ...
    return (
      <section className="panel workspace-section-card" style={{ marginTop: '1rem', borderColor: 'rgba(25,214,174,.4)' }}>
        ...
        <p className="workspace-details-copy" style={{ marginTop: '.5rem' }}>
          ...
          Extra Stop fee. You only pay after you approve the time and price — nothing is booked until then.
        </p>
      </section>
    );
  }
    ...
    <section className="panel workspace-section-card" style={{ marginTop: '1rem' }}>
      ...
      {verdict && !verdict.unsafe && !verdict.needsPhotos && verdict.eligible === false ? (
        <p className="payment-banner muted" style={{ marginTop: '1rem' }}>
          {verdict.reason || 'This job isn’t a fit for an Extra Stop.'} You can still request a regular booking above.
        </p>
      ) : null}
        ...
        <>
          ...
          <div className="field full" style={{ marginTop: '.5rem' }}>
            <button type="button" className="btn primary" onClick={submit} disabled={submitting}>
              {submitting ? 'Sending…' : 'Request an Extra Stop'}
            </button>
          </div>
        </>
      ) : null}
