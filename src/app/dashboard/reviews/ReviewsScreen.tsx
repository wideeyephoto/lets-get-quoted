...
/**
 * Reputation and feedback, given the summary.
 *
 * Split out of page.tsx so the logged-out demo renders the same screen — see the
 * note on CampaignsScreen.
 *
 * The policy paragraph in the hero is the reason this page is shaped the way it
 * is, and is exactly the kind of thing a hand-drawn demo copy drops: it explains
 * that we deliberately do NOT screen customers by rating, because Google
 * restricts the Business Profile of anyone who does. A demo that quietly omitted
 * it would be selling a product with a compliance stance it does not have.
 */
          ...
          <div style={{ marginTop: '0.75rem' }}>
            {readOnly ? (
              <span className="status-badge status-complete">
                Auto review requests: {reviewsOn ? 'ON' : 'OFF'}
              </span>
            ) : (
                  ...
                  <div key={item.id} className="review-feedback-card">
                    <div className="review-feedback-top">
                      ...
                      <span className="review-feedback-meta">
                        ...
                        {item.jobId ? <> · <Link href={`${basePath}/jobs/${item.jobId}`}>Open job →</Link></> : null}
                      </span>
                    </div>
