...
const HERO_TRADES: Record<string, HeroTradeDemo> = {
  plumbers: {
    ...
    unitSuffix: 'baths',
    defaultOptions: [
      { name: 'Rheem 199k BTU High-Efficiency Tankless', cost: 1850, checked: true },
      { name: 'Gas Line Inspection & Expansion Tank', cost: 420, checked: true },
      { name: 'Emergency Same-Day Priority Dispatch', cost: 250, checked: false },
    ],
    quoteItems: [
      { name: 'Rheem RTGH-95DVLN Condensing Unit', qty: '1 unit', total: 1850 },
      { name: 'Gas line adapt, expansion tank & shutoff', qty: '1 pkg', total: 420 },
      { name: 'Certified Master Plumber Labor & Haul Away', qty: '5 hrs', total: 1250 },
    ],
    crewTech: 'Dave & Jason (Master Techs)',
  ...
  hvac: {
    ...
    unitSuffix: 'sq ft',
    defaultOptions: [
      { name: 'Bosch 20 SEER2 Variable Speed Condenser', cost: 5800, checked: true },
      { name: '10 kW Electric Heat Strip & Transition Plenum', cost: 950, checked: true },
      { name: 'Ecobee Smart Thermostat Pro with Room Sensors', cost: 350, checked: true },
    ],
    quoteItems: [
      { name: 'Bosch BOVA-60HDN1 Variable Inverter', qty: '1 unit', total: 5800 },
      { name: 'Air handler transition & copper lineset', qty: '1 lot', total: 1450 },
      { name: 'Complete system install, vacuum & test', qty: '1 day', total: 2200 },
    ],
  ...
  roofers: {
    ...
    defaultOptions: [
      { name: 'GAF Timberline HDZ Architectural Shingles', cost: 7200, checked: true },
      { name: 'Ice & Water Shield on Eaves & Valleys', cost: 1100, checked: true },
      { name: '6" Seamless Aluminum Gutters & Leaf Guards', cost: 1650, checked: false },
    ],
    quoteItems: [
      { name: 'GAF Timberline Shingles + Synthetic Underlayment', qty: '28 sq', total: 8400 },
      { name: 'Ice & Water shield, starter strip, ridge vent', qty: '1 pkg', total: 1850 },
      { name: 'Tear-off, dump trailer & 6-tech crew labor', qty: '1 day', total: 3950 },
    ],
  ...
  electricians: {
    ...
    defaultOptions: [
      { name: 'Square D QO 200A Main Breaker Panel', cost: 1950, checked: true },
      { name: 'Tesla Universal Wall Connector 48A (60A Breaker)', cost: 750, checked: true },
      { name: 'Whole-Home Surge Protective Device (Type 2)', cost: 380, checked: true },
    ],
    quoteItems: [
      { name: 'Square D 200A 42-Space Meter-Main Combo', qty: '1 panel', total: 1950 },
      { name: '60A EV dedicated run with 6/3 Romex conduit', qty: '45 ft', total: 850 },
      { name: 'Utility disconnect coordination & permit inspect', qty: '1 job', total: 1100 },
    ],
    crewTech: 'Kevin R. (Licensed Journeyman)',
  ...
  landscapers: {
    ...
    defaultOptions: [
      { name: 'Belgard Dimensions Slate Pavers', cost: 4600, checked: true },
      { name: 'Built-In Freestanding Gas Fire Pit', cost: 1850, checked: true },
      { name: 'Low-Voltage LED Paver Step Lighting', cost: 850, checked: false },
    ],
    quoteItems: [
      { name: 'Belgard Slate pavers + polymeric sand lock', qty: '600 sq ft', total: 5400 },
      { name: '6" crushed aggregate base with geotextile wrap', qty: '14 tons', total: 1650 },
      { name: 'Excavation, grading, compaction & mason labor', qty: '3 days', total: 3200 },
    ],
...
export default function ForMockupExperience() {
  ...
  const calculatedSubtotal = Math.round(unitCount * heroData.baseRate + optionsTotal);
  ...
  return (
    <div className={styles.pageWrapper}>
      ...
      <main className={styles.pageContainer}>
        {/* =========================================================================
            1. HERO SECTION: SPLIT 2-COL WITH TRADE ENGINE SIMULATOR
            ========================================================================= */}
        <section className={styles.hero} aria-labelledby="hero-title">
          ...
          {/* Right Column: Live Interactive Trade Engine Simulator */}
              ...
              {activeSimulatorTab === 'intake' && (
                <div className={styles.fadeIn}>
                  <div className={styles.intakeStepBar}>
                    <span className={styles.intakeStepTag}>CUSTOMER WEBSITE EXPERIENCE</span>
                    <span className={styles.intakeTime}>Estimated in 45 seconds</span>
                  </div>
                  ...
                  <div className={styles.estimateResultBox}>
                    <div className={styles.calcPriceCol}>
                      <span className={styles.resultKicker}>AI INSTANT ESTIMATE RANGE</span>
                      <strong className={styles.calcRange}>${money.format(estimateLow)} – ${money.format(estimateHigh)}</strong>
                      <small>20% Deposit required upon acceptance: ${money.format(depositAmount)}</small>
                    </div>
                    <button
                      type="button"
                  ...
                  <div className={styles.quoteLineItems}>
                    {heroData.quoteItems.map((item) => (
                      <div key={item.name} className={styles.lineItemRow}>
                        ...
                        <span className={styles.itemPrice}>${money.format(item.total)}</span>
                      </div>
                    ))}
                  </div>
                ...
                <div className={styles.fadeIn}>
                  <div className={styles.dispatchCard}>
                    <div className={styles.dispatchHeader}>
                      ...
                      <div>
                        <span className={styles.dispatchKicker}>DISPATCHED CREW</span>
                    ...
                    <div className={styles.jobRouteDetails}>
                      <div className={styles.routeStep}>
                        ...
                        <div>
                          ...
                          <p>Customer: {heroData.customerName} (SMS reminders active)</p>
                        </div>
                      </div>
                      <div className={styles.routeStep}>
                        <span className={styles.stepDot} />
                        <div>
                          <strong>Work Scope & Materials</strong>
                          <p>{heroData.serviceTitle} · Materials verified in stock</p>
                        </div>
                      </div>
                    </div>
                    ...
                    <div className={styles.crewQuickActions}>
                      ...
                      <span className={styles.actionPill}>📷 Attach Before Photo</span>
              ...
              {activeSimulatorTab === 'payout' && (
                <div className={styles.fadeIn}>
                  <div className={styles.payoutHeader}>
                    ...
                    <h4>${money.format(calculatedSubtotal)} Collected Online</h4>
                    <p>Deposited directly to your business checking account via Stripe</p>
                  </div>
                  ...
                  <div className={styles.payoutWaterfall}>
                    ...
                    <div className={`${styles.waterfallRow} ${styles.totalDepositRow}`}>
                      <div>
                        ...
                        <small>Direct to checking · Zero monthly fee</small>
                      </div>
