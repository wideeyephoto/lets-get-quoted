...
// Global sweep of expired Extra Stop offers: releases lapsed payment holds and
...
// CRON_SECRET auth as the other crons. (The Extra Stops dashboard also runs an
...
export async function GET(request: Request) {
  ...
  try {
    const summary = await sweepExtraStopOffers(createAdminClient());
    return NextResponse.json(summary);
  } catch (error) {
    console.error('Extra Stop sweep cron failed:', error);
