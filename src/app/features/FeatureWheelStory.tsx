...
import { wireCommandCenter } from './command-center-motion';
...
export default function FeatureWheelStory() {
  ...
  useEffect(() => {
    ...
    // headings and bullets (see the "Sample data" labels). The deck's own
    // .cc-screen mockups are handled by wireCommandCenter below, so that they
    // stay hidden wherever the deck is rendered.
    scope.querySelectorAll<HTMLElement>('.fw-mock').forEach((el) => {
    ...
    // Shared with the homepage's standalone deck — see command-center-motion.ts.
    cleanups.push(wireCommandCenter(scope, REDUCE));
