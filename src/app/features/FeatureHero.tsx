...
// Features hero: "Quote it. Sign it. Get paid." The right-hand dashboard panel's
...
// tile jumps to its view and pauses. Copy + CTAs are the real content; the
// panel is decorative. Everything respects prefers-reduced-motion.
export default function FeatureHero() {
  ...
  useEffect(() => {
    ...
    if (!root) return;
    const win = root.querySelector<HTMLElement>('.fh-win');
    if (!win) return;
    const REDUCE = window.matchMedia('(prefers-reduced-motion:reduce)').matches;

    // count-up the paid figure
