...
export function buildVoiceSystemPrompt(context: VoiceGroundingContext): string {
  ...
  if (context.contractorStaffCaller) {
    ...
    return [
      ...
      `6. request_staff_step_up: Send a one-time authorization code to the verified phone calling now. Required ONLY for privileged job mutations (updating existing job details, crew labor/materials, change orders, cautions). NEVER use for creating leads.`,
