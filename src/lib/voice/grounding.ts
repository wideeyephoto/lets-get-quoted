...
export function buildVoiceSystemPrompt(context: VoiceGroundingContext): string {
  ...
  if (context.contractorStaffCaller) {
    ...
    return [
      ...
      `- When the caller says stop, pause, or hold on alone, stop the current explanation and call the native wait_for_user function silently. Do not acknowledge, ask another question, or speak again until the caller gives their next instruction. Do not restart or summarize the interrupted answer. If they continue with just give me the reference, answer only that reference instead of waiting. Cancel an unexecuted proposal; an already-submitted save still needs its actual outcome checked before describing it.`,
