...
export function buildVoiceSystemPrompt(context: VoiceGroundingContext): string {
  ...
  if (context.contractorStaffCaller) {
    ...
    return [
      ...
      `1. request_staff_step_up: Send a one-time authorization code to the verified phone that placed this call.`,
      `2. verify_staff_step_up: Verify the six-digit code the caller reads from that text.`,
      `3. update_job_details: Update job scope, quote line items, schedule date/time, tasks, or status (e.g. "We finished the rough-in on Miller's job, add 4 recessed lights for $650, schedule final for Tuesday").`,
      `4. create_or_update_lead: Create a new customer lead (e.g. "Take a new lead for John Davis at 142 Elm St, roof leak, phone 555-0199, needs inspection Friday").`,
      `5. log_crew_time_and_materials: Log hours worked, materials purchased/used, and cost notes for a job.`,
      `6. create_job_change_order: Record extra unforeseen work or scope changes requiring a change order.`,
      `7. append_job_caution_or_note: Add an internal note, safety warning, gate code, pet caution, or special request to a job or client record (e.g. "Add a caution on the Miller job: gate code is 4821 and watch out for the dog", or "Note on Davis: delicate historic brick, use soft wash only").`,
      ``,
      `[BEHAVIOR & CONVERSATION FLOW]`,
      `- Listen carefully to the contractor's spoken instructions.`,
      `- Before calling any tool that saves a dispatch, job, lead, labor, material, change-order, caution, or note mutation, call request_staff_step_up. Do not call the mutation until this active call is verified.`,
