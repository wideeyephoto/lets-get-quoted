...
export async function sendExtraStopOffer(supabase: SupabaseClient, accountId: string, requestId: string): Promise<void> {
  ...
  if (request.client_phone) {
    ...
    const feeLabel = `${fmtMoneyCents(request.fee_cents)} Extra Stop fee${request.diagnostic_fee_cents ? ` (+ ${fmtMoneyCents(request.diagnostic_fee_cents)} diagnostic)` : ''}`;
    await sendExtraStopOfferSms({
      accountId,
...
// double-confirm. No-op for any payment that isn't a live Extra Stop offer.
export async function confirmExtraStopPayment(admin: SupabaseClient, paymentId: string): Promise<void> {
  ...
  await logExtraStopEvent(admin, accountId, confirmed.id as string, {
    actor: 'stripe',
  ...
  try {
    ...
    if (ownerEmail) {
      await sendContractorAlertEmail({
        ...
        businessName,
        subject: '✅ Extra Stop confirmed & paid',
        heading: 'An Extra Stop is confirmed',
        bodyLines: [
          `${confirmed.client_name} paid the Extra Stop fee.`,
          `Arrival: ${when}.`,
    ...
    console.error('Extra Stop confirm owner email failed:', error instanceof Error ? error.message : error);
  }
}
