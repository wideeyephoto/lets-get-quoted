  ...
  for (const notice of (claimed.data ?? []) as Notice[]) {
    ...
    try {
      ...
      providerId = await sendOwnerEventNoticeEmail({
        ...
        bodyLines: [notice.source_payload.body], ctaLabel: notice.source_type === 'account_connect' ? 'Review payment setup' : ['payment_refund','payment_dispute'].includes(notice.source_type ?? '') ? 'View payments' : notice.source_type === 'quick_stop' ? 'View Quick Stops' : messaging ? 'Open messaging dashboard' : notice.source_payload.job_id ? 'Open the job' : 'Open dashboard',
