  ...
  const doFetch = async (): Promise<Jwk[] | null> => {
    try {
      const res = await (options.fetchImpl ?? fetch)(jwksUrl(), {
        // Next patches global fetch in the server runtime and can answer an
        // identical request from its data cache without leaving the process.
        // That would pin the key set past this module's own TTL and, worse,
        // make `force` — the path that heals a rotation — a no-op. A key set is
        // a read, and a read is never a cacheable fetch: same rule as
        // noStoreFetch in lib/auth, which exists because a cached read once
        // made a worker reprocess one row ten times and report success.
        cache: 'no-store',
