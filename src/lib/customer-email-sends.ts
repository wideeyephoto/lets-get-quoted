...
export async function sendCustomerEmail(
  ...
  if (!context.accountId || !context.kind || !context.idempotencyKey || !resend.key) {
    throw new Error('Email could not be submitted: customer email intent or provider configuration is unavailable.');
  }
...
export async function executeCustomerEmailClaim(
  admin: SupabaseClient, resend: EmailProvider, accountId: string, data: Claim & { provider_id?: string; reason?: string }, recovery?: RecoveryExecution
): Promise<CustomerEmailReceipt> {
