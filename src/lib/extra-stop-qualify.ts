// Extra Stop qualification. Two layers, in order:
...
import { ExtraStopSettings } from './extra-stop';

export type ExtraStopComplexity = 'simple' | 'moderate' | 'complex';

export type ExtraStopExclusionRule = {
  key: string;
  label: string;
  unsafe: boolean;
  patterns: RegExp[];
...
export const EXTRA_STOP_EXCLUSIONS: ExtraStopExclusionRule[] = [
  {
    key: 'gas_leak',
  ...
  },
  {
    ...
    safety: 'Suspected mold or asbestos requires certified testing and remediation. This can’t be handled as a quick Extra Stop — please contact a licensed remediation specialist.',
  },
  {
    key: 'hazmat',
...
export type ScreenResult = {
  matched: ExtraStopExclusionRule[];
  unsafe: boolean;
  safety: string | null;
  labels: string[];
};
...
export function screenHardExclusions(text: string): ScreenResult {
  ...
  const matched = EXTRA_STOP_EXCLUSIONS.filter((rule) => rule.patterns.some((p) => p.test(hay)));
...
export type ExtraStopQualifyInput = {
  issue: string;
  startedWhen?: string;
  worsening?: string;
  propertyType?: string;
...
export type ExtraStopQualifyOptions = {
...
export type ExtraStopQualification = {
  eligible: boolean;
  unsafe: boolean;
  summary: string;
  visitMinutes: number | null;
  complexity: ExtraStopComplexity | null;
...
// eligible for Extra Stop (it can still fall back to standard booking upstream).
export async function qualifyExtraStop(
  input: ExtraStopQualifyInput,
  opts: ExtraStopQualifyOptions,
): Promise<ExtraStopQualification> {
  ...
  if (screen.matched.length) {
    return {
      ...
      exclusions: screen.labels,
      safety: null,
      reason: `This looks like ${screen.labels[0].toLowerCase()}, which is outside what an Extra Stop covers.`,
    };
  }
  ...
  if (!apiKey) {
    ...
    // verify simplicity, so we don't offer Extra Stop.
    return {
  ...
  const categoryLine = opts.categories.length
    ? `The contractor ONLY accepts Extra Stops in these categories: ${opts.categories.join(', ')}. Set category to the closest one, or "other" if none fit.`
  ...
  const instructions =
    'You screen a home-services job to decide if it qualifies as an "Extra Stop": a simple, clearly-defined task that ONE technician can reasonably inspect or handle in a single short visit today. ' +
    businessLine +
    ' ' +
    categoryLine +
  ...
  try {
    ...
    const complexity: ExtraStopComplexity =
    ...
    if (visitMinutes != null && visitMinutes > opts.maxVisitMinutes) exclusions.push(`Longer than your ${opts.maxVisitMinutes}-minute Extra Stop limit`);
    ...
    if (opts.categories.length && category && category !== 'other' && !opts.categories.includes(category)) {
      exclusions.push('Outside your accepted Extra Stop categories');
    }
    ...
    console.error('Extra Stop qualification failed:', error);
...
export function qualifyOptionsFromSettings(settings: ExtraStopSettings, apiKey?: string | null): ExtraStopQualifyOptions {
  return {
