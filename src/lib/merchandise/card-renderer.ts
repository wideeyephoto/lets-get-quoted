 ...
 * Uses dynamic require so client components importing artwork renderers don't fail Webpack client builds.
 */
export function computeArtworkHash(svgContent: string): string {
  ...
  if (nodeCrypto && typeof nodeCrypto.createHash === 'function') {
    return nodeCrypto.createHash('sha256').update(svgContent.trim(), 'utf8').digest('hex');
  }
  let hash = 5381;
  const str = svgContent.trim();
  for (let i = 0; i < str.length; i++) {
    hash = ((hash << 5) + hash) ^ str.charCodeAt(i);
  }
  return (Math.abs(hash).toString(16) + '0'.repeat(64)).slice(0, 64);
}
