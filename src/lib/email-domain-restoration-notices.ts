...
import { getAccountOwnerEmail, sendSendingDomainRestoredEmail } from '@/lib/email';

type Notice = { id: string; account_id: string; domain_id: string | null; domain: string; attempted_at: string };

/** One attempt per committed connection event. Uncertain outcomes require review. */
export async function runEmailDomainRestorationNotices(admin: SupabaseClient) {
  const claimed = await admin.rpc('claim_email_domain_restoration_notices', { p_limit: 5 });
  if (claimed.error) throw new Error('Could not claim sending domain restoration notices');
  ...
  for (const notice of (claimed.data ?? []) as Notice[]) {
    ...
    try {
      ...
      const prepared = await admin.rpc('prepare_email_domain_restoration_notice', {
      ...
      providerId = await sendSendingDomainRestoredEmail({
        noticeId: notice.id,
        prepareIntent: async snapshot => {
          const saved = await admin.rpc('prepare_email_domain_restoration_snapshot', {
    ...
    const saved = await admin.rpc('finish_email_domain_restoration_notice', {
      p_id: notice.id, p_account_id: notice.account_id, p_attempted_at: notice.attempted_at,
      p_provider_id: providerId, p_error: failure,
    });
    if (saved.error || saved.data !== true) throw new Error('Could not persist sending domain restoration notice outcome');
    if (providerId) ownersNotified += 1; // Provider acceptance, not confirmed delivery.
  }
  const reviews = await admin.from('email_domain_restoration_notices')
    .select('id,account_id,last_error', { count: 'exact' }).eq('state','manual_review').order('created_at').limit(20);
  const pending = await admin.from('email_domain_restoration_notices').select('id', { count: 'exact', head: true }).eq('state','pending');
  if (reviews.error || pending.error) throw new Error('Could not read sending domain restoration notice incidents');
