...
// Stop.
//
// The page could say what an Extra Stop is and it could show requests that had
...
import { EXTRA_STOP_EXCLUSIONS, screenHardExclusions } from './extra-stop-qualify';
...
export type Candidate = CandidateInput & {
  eligible: boolean;
  /** Matched an unsafe rule — never an Extra Stop, and worth saying why. */
  unsafe: boolean;
 ...
 * and their day.
 */
export function screenExtraStopCandidates(
 ...
 */
export function extraStopRuleReference(): { unsafe: string[]; outOfScope: string[] } {
  return {
    unsafe: EXTRA_STOP_EXCLUSIONS.filter((rule) => rule.unsafe).map((rule) => rule.label),
    outOfScope: EXTRA_STOP_EXCLUSIONS.filter((rule) => !rule.unsafe).map((rule) => rule.label),
  };
}
