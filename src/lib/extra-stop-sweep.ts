...
import { logExtraStopEvent, type ExtraStopRequest } from '@/lib/extra-stop-requests';
...
// and lazily on the Extra Stops dashboard (account-scoped) so an owner's view is
...
export async function sweepExtraStopOffers(admin: SupabaseClient, accountId?: string): Promise<SweepSummary> {
  ...
  for (const row of (payExpired ?? []) as ExtraStopRequest[]) {
    ...
    await logExtraStopEvent(admin, row.account_id, row.id, { actor: 'system', from: 'awaiting_customer_payment', to: 'offer_expired', meta: { reason: 'payment_window_elapsed' } });
    ...
    try {
      ...
      if (ownerEmail) {
        await sendContractorAlertEmail({
          ...
          subject: 'Extra Stop offer expired unpaid',
          heading: 'An Extra Stop offer expired',
          bodyLines: [
      ...
      console.error('Extra Stop expiry email failed:', error instanceof Error ? error.message : error);
    }
