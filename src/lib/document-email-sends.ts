...
export async function sendDocumentEmail(
  admin: SupabaseClient, resend: Resend, context: DocumentEmailContext, message: CreateEmailOptions,
  ...
  const submit = async (): Promise<CreateEmailResponse> => {
    try {
  ...
  if (claim.phase === 'primary' && !result.data && result.error?.name === 'validation_error') {
    const { data: fallback, error: fallbackError } = await admin.rpc('fallback_document_email_send', {
      p_id: claim.id, p_account_id: context.accountId, p_token: claim.token,
  ...
  const { data: finished, error: finishError } = await admin.rpc('finish_document_email_send', {
    p_id: claim.id, p_account_id: context.accountId, p_token: claim.token, p_provider_id: providerId,
