// Which color a person's initials circle gets.
//
...
// find a row you had already scrolled past once. Color makes a row findable
// by shape rather than by reading, which is the only thing an avatar is for
// when there's no photo.
//
// STABLE, not random. The same person is the same color on the roster, in the
// detail pane, in the hours table and after a page refresh — a color that
// shuffles is worse than no color, because it invites you to rely on it and
// then lies.
//
// Keyed on the NAME, not the record id, and that choice is load-bearing. The
// initials come from the name, so hashing the name makes the color vary
// exactly when the two letters in the circle vary — which is the only moment
// the color has a job to do. Keyed on the id, "Danny Whitcombe" and "Dee
// Whitlock" landed on the same tone on a real roster: adjacent rows, both
// reading DW, in the same color, and nothing on screen told them apart. Two
// people who share a full name still collide, but they are indistinguishable
// anyway and giving them different colors would be the confusing answer.
//
 ...
 * so a name-sorted list comes out as bands of one color — the exact thing the
 * color is meant to break up.
 */
export function avatarTone(key: string | null | undefined): number {
  ...
  // names on one color while three tones went unused. This is the lowbias32
