// Single source of truth for the Quick Stop feature: the request lifecycle (its
...
export const QUICK_STOP_STATUSES: QuickStopStatus[] = [
  'requested',
  'awaiting_contractor',
  'more_information_requested',
  'contractor_declined',
...
export const QUICK_STOP_ACTIVE_STATUSES: QuickStopStatus[] = [
  'requested',
  'awaiting_contractor',
  'more_information_requested',
  'contractor_offer_sent',
...
export const DEFAULT_QUICK_STOP_LATEST_END = '20:00';
export const DEFAULT_QUICK_STOP_MAX_PER_DAY = 2;
export const DEFAULT_QUICK_STOP_MAX_VISIT_MINUTES = 60;
export const DEFAULT_QUICK_STOP_MAX_DETOUR_MILES = 10;
export const DEFAULT_QUICK_STOP_MAX_DETOUR_MINUTES = 20;
export const DEFAULT_QUICK_STOP_MIN_FEE_CENTS = 5000; // $50
export const DEFAULT_QUICK_STOP_MAX_FEE_CENTS = 25000; // $250
export const DEFAULT_QUICK_STOP_RESPONSE_DEADLINE_MINS = 30;
...
export function clampFeeCents(cents: number, settings: QuickStopSettings): number {
...
// Defensive: build a normalized QuickStopSettings from raw account columns,
...
export function quickStopSettingsFromAccount(row: AccountQuickStopRow): QuickStopSettings {
  ...
  return {
    ...
    earliestTime: normalizeHHMM(row?.extra_stop_earliest_time, DEFAULT_QUICK_STOP_EARLIEST),
    latestEnd: normalizeHHMM(row?.extra_stop_latest_end, DEFAULT_QUICK_STOP_LATEST_END),
    maxPerDay: clampInt(row?.extra_stop_max_per_day, 1, 50, DEFAULT_QUICK_STOP_MAX_PER_DAY),
    maxVisitMinutes: clampInt(row?.extra_stop_max_visit_minutes, 5, 600, DEFAULT_QUICK_STOP_MAX_VISIT_MINUTES),
    maxDetourMiles: clampNumber(row?.extra_stop_max_detour_miles, 0, 500, DEFAULT_QUICK_STOP_MAX_DETOUR_MILES),
    maxDetourMinutes: clampInt(row?.extra_stop_max_detour_minutes, 0, 600, DEFAULT_QUICK_STOP_MAX_DETOUR_MINUTES),
    minFeeCents: clampInt(row?.extra_stop_min_fee_cents, 0, 100_000_00, DEFAULT_QUICK_STOP_MIN_FEE_CENTS),
    maxFeeCents: clampInt(row?.extra_stop_max_fee_cents, 0, 100_000_00, DEFAULT_QUICK_STOP_MAX_FEE_CENTS),
    allowAfterCapacity: row?.extra_stop_allow_after_capacity !== false,
    responseDeadlineMins: clampInt(row?.extra_stop_response_deadline_mins, 1, 720, DEFAULT_QUICK_STOP_RESPONSE_DEADLINE_MINS),
...
export type QuickStopDayOption = {
  dateKey: string;
 ...
 */
export function quickStopDayOptions(
  settings: Pick<QuickStopSettings, 'weekdays' | 'daysAhead' | 'latestEnd'>,
...
export function isAllowedQuickStopDay(
  dateKey: string,
  settings: Pick<QuickStopSettings, 'weekdays' | 'daysAhead' | 'latestEnd'>,
  opts: { now?: Date; timeZone: string },
): boolean {
  return quickStopDayOptions(settings, opts).some((option) => option.dateKey === dateKey);
}

// The column set to select when loading settings — kept next to the builder so
// callers stay in sync with the table.
export const QUICK_STOP_SETTINGS_COLUMNS =
...
// decide how long to lock Quick Stop after a fresh one:
...
export function quickStopNoShowLock(priorNoShowDates: Date[], now: Date = new Date()): { untilIso: string; reason: string; tier: NoShowLockTier } {
  ...
  if (within(180) >= 2) return { untilIso: endIn(3650), reason: 'Third no-show within 180 days — Quick Stop disabled pending staff review.', tier: 3 };
  if (within(90) >= 1) return { untilIso: endIn(30), reason: 'Second no-show within 90 days — Quick Stop locked for 30 days.', tier: 2 };
  return { untilIso: endIn(10), reason: 'No-show reported — Quick Stop locked for 10 days.', tier: 1 };
}
