...
export default function NovaTemplate({ site, galleryImages = [] }: TemplateProps) {
  ...
  const themeStyle = {
    '--theme-accent': site.accent_override || '#c04dff',
    '--theme-display': site.header_font || 'var(--font-nova-display), Arial, sans-serif',
  } as CSSProperties;

  return (
    <main className={`${shared.site} ${styles.nova}`} style={themeStyle} data-button={site.button_style || 'solid'} data-mode={site.portal_mode}>
      <header className={styles.header}>
        <a className={styles.brand} href="#top">
          {site.logo_url && <img className={shared.logo} src={site.logo_url} alt="" />}
          <span>{site.company_name}</span>
        </a>
        <SiteNavLinks site={site} className={shared.navLinks} links={[{ href: '#studio', label: 'Studio' }, { href: '#work', label: 'Work' }, { href: '#contact', label: 'Connect' }]} />
        <a className={styles.menu} href="#contact" aria-label="Contact us">↗</a>
      </header>

      <section className={styles.hero} id="top">
        <img className={styles.heroImage} src={heroImage} alt="Featured modern build project" />
        <div className={styles.heroCopy}>
          <p className={shared.kicker}>Design-forward / Smart / Built fast</p>
          <h1>{site.headline || 'The future, framed and finished.'}</h1>
          <p>{site.tagline || 'Modern builds and smart-ready spaces designed for how you\u2019ll actually live tomorrow.'}</p>
        </div>
      ...
      <section className={styles.statement} id="studio">
        ...
        <h2>Part builder, part systems designer, always ahead of the curve.</h2>
