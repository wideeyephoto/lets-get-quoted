...
//
// Stored as raw inner-SVG markup (not JSX) so the same paths can power both the
// React component below AND standalone SVG strings (favicons, downloadable logo
// files) in @/lib/brand-mark, keeping a single source of truth for the glyphs.
...
export const SERVICE_ICON_KEYS = Object.keys(SERVICE_ICON_PATHS);
...
export default function ServiceIcon({ name, className }: { name: string; className?: string }) {
  return (
    <svg
      ...
      aria-hidden="true"
      dangerouslySetInnerHTML={{ __html: SERVICE_ICON_PATHS[name] ?? SERVICE_ICON_PATHS.spark }}
    />
  );
}
